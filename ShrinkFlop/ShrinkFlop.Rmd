...
SpikedComparison <-function(SmallDimension,BigDimension,NumSamples,NumInstances=2,TrialsPerInstance=2,spike = 10, RegMin = -5, RegMax =5, RegStride=1){
  ...
  results = data.frame(Regularizer = Regularizers,Gemini=0,ShrinkFlop = 0,Trivial=0, KGlasso = 0)
...
```{r, echo=FALSE}
#test sparse comparison
ans = SparseComparison(25,50,1,RegMin=-15,RegMax = 1,NumInstances =2, TrialsPerInstance  = 2, spike=100, RegStride = 1)
...
```{r}
SinglySpiked = SparseComparison(25,50,1,RegMin=-20,RegMax = 10,NumInstances =5, TrialsPerInstance  = 5, spike=100, RegStride = 1)
#this fails - the KGlasso regularization parameter can't go too high, I think.
```

```{r}
#do the plots 
#ans = SparseComparison(25,50,1,RegMin=-15,RegMax = 5,NumInstances =5, TrialsPerInstance  = 5, spike=100, RegStride = .5)
...
SpikedLearningCurve <-function(SmallDimension,BigDimension,MinSamples,MaxSamples,NumInstances=2,TrialsPerInstance=2,spike = 10,RegMin = -5, RegMax =5, RegStride=1){
  ...
  results = data.frame(Samples = c(MinSamples:MaxSamples),Gemini=0,RegSink = 0,Sink = 0, Trivial=0, KGlasso = NA)
...
LearningCurvePlot <- function(results){
  ...
  #df = melt(results, id.vars = "Regularizer",variable.name = "Algorithm")
  lp <- ggplot(data=df, aes(x=Samples, y=Error, group=Algorithm, color=Algorithm)) + geom_line() + geom_point() + scale_y_log10(breaks = 10^(-10:10),labels = trans_format("log10", math_format(10^.x)))+ mytheme
...
Setting the right regularization parameter.

```{r}
# to do this, compute for various n, d1, d2 the regularization parameters.
#for gemini, the suggestion is lambda_A = log^{1/2}(min (d1,d2))/\sqrt{n d_2}
#for tsiligkaridis, the suggestion is proportional to lambda_B = \sqrt{log M/ n d_1}, lambda_A = \sqrt{log M/n d_2} + lambda_B, I guess.
#we can then guess 1/sqrt n dependence, 

#to start, just remove KGlasso.
