...
#if you don't want to compare with the matlab KGlasso code, set use_matlab to False
#everything will be much faster if you do that :)
use_matlab = FALSE
if(use_matlab){
  #change path below to your matlab location
...
SpikedComparison <-function(SmallDimension,BigDimension,NumSamples,NumInstances=2,TrialsPerInstance=2,spike = 10, RegMin = -5, RegMax =5, RegStride=1){
  ...
  results = data.frame(Regularizer = Regularizers,Gemini=0,ShrinkFlop = 0,Trivial=0)
  ...
  for (j in 1:length(Regularizers)){
    ...
    for (l in 1:NumInstances){ 
      ...
      for(k in 1:TrialsPerInstance){
        ...
        results$ShrinkFlop[j] = results$ShrinkFlop[j]+ RegSinkError/(NumInstances*TrialsPerInstance)
        
        #operator error
        OpSinkError = norm(invA - (RegSinkEstimate/tr(RegSinkEstimate)),"I")^2
        OpResults$ShrinkFlop[j] = OpResults$ShrinkFlop[j]+ OpSinkError/(NumInstances*TrialsPerInstance)
        
        #geodesic distance
        GeoSinkError = GeodesicDistance(DetNorm(invA), DetNorm(RegSinkEstimate))^2/GeoTrivialError^2
        GeoResults$ShrinkFlop[j] = GeoResults$ShrinkFlop[j] + GeoSinkError/(NumInstances*TrialsPerInstance)
...
```{r}
DoublySpikedTest = SpikedComparison(3,5,2,RegMin=-5,RegMax = 2,NumInstances =1, TrialsPerInstance  = 1, RegStride = .5)
...
DoublySpiked = SpikedComparison(25,50,1,RegMin=-10, RegMax = 5, NumInstances =5, TrialsPerInstance  = 5, RegStride = .5)
...
```{r}
#geodesic
lp=SimplePlot(DoublySpikedTest$GeoResults)
lp + ggtitle("Error in Geodesic distance") #cool way to title on the fly :)
...
#geodesic
lp=SimplePlot(DoublySpiked$GeoResults)
lp = lp + ggtitle("Error in Geodesic distance") 
pdf("25-50-spiked-geo.pdf")
lp
dev.off()
lp

lp=SimplePlot(DoublySpiked$OpResults)
...
lp=SimplePlot(DoublySpiked$Results)
...
One side sparse, one side spiked comparison code

```{r, echo=FALSE}



SinglySparseComparison <-function(SmallDimension,BigDimension,NumSamples,NumInstances=2,TrialsPerInstance=2,sparsity = 4,spike=10, RegMin = -5, RegMax =5, RegStride=1){
  ...
  for (j in 1:length(Regularizers)){
    ...
    for (term in 1:SmallDimension*sparsity/10){
      ...
      for(k in 1:TrialsPerInstance){
        ...
        RegSinkFactor = .00005
        tic("regularized sinkhorn")
        RegSinkEstimate = regsinkhorn(X,tol = .01, reg = Regularizers[j]*RegSinkFactor)[[1]]
        toc()
...
Test + plot sparse comparison

```{r}
ans3 = SinglySparseComparison(11,11,5,RegMin=-5,RegMax = 5,NumInstances =1, TrialsPerInstance  = 1, spike=10, RegStride = 1)
lp = SimplePlot(ans3$GeoResults)
pdf("SparsePlotTest.pdf")
lp
dev.off()
lp
```
...
Run sparse comparison

```{r, echo=FALSE}
ans = SinglySparseComparison(25,50,1,RegMin=-15,RegMax = 1,NumInstances =2, TrialsPerInstance  = 2, spike=100, RegStride = 1)
...
rename RegSink to ShrinkFlop

```{r}
colnames(ans$Results) = c("Regularizer","Gemini","ShrinkFlop","Trivial","Kglasso")
colnames(ans$OpResults) = c("Regularizer","Gemini","ShrinkFlop","Trivial","Kglasso")
colnames(ans$GeoResults) = c("Regularizer","Gemini","ShrinkFlop","Trivial","Kglasso")
...
ans = SinglySparseComparison(50,50,5,RegMin=-10,RegMax = 10,NumInstances =2, TrialsPerInstance  = 2, spike=300, RegStride = 2)
lp = SimplePlot(ans$GeoResults)
...
Testing write tables; only run if using matlab connection
...
```{r, echo=FALSE}
ans3 = DoublySparseComparison(25,50,1,RegMin=-10,RegMax = 2,NumInstances =5, TrialsPerInstance  = 10, RegStride = 0.5)
lp = SimplePlot(ans3$GeoResults)
pdf("SparsePlot.pdf")
lp
dev.off()
...
```{r}
#rename RegSink to ShrinkFlop
colnames(ans3$Results) = c("Regularizer","Gemini","ShrinkFlop","Trivial","Kglasso")
colnames(ans3$OpResults) = c("Regularizer","Gemini","ShrinkFlop","Trivial","Kglasso")
colnames(ans3$GeoResults) = c("Regularizer","Gemini","ShrinkFlop","Trivial","Kglasso")
...
```{r}
#with the files 
#ans3 = DoublySparseComparison(25,50,1,RegMin=-10,RegMax = 2,NumInstances =5, TrialsPerInstance  = 5, RegStride = 0.5)
...
Spiked Learning curves
```{r}

SpikedLearningCurve <-function(SmallDimension,BigDimension,MinSamples,MaxSamples,NumInstances=2,TrialsPerInstance=2,spike = 10,RegMin = -5, RegMax =5, RegStride=1){
  
  results = data.frame(Samples = c(MinSamples:MaxSamples),Gemini=0,RegSink = 0,Sink = 0, Trivial=0)
  ...
  for(j in MinSamples:MaxSamples){
    ...
    for (l in 1:NumInstances){ 
      ...
      for(k in 1:TrialsPerInstance){
        ...
        #gemini
        #set gemini penalties
        #gemini reg should depend on it too.
        GeminiRegFactor = 1*sqrt(log(SmallDimension)/(BigDimension*j))
        tic("geminiB")
...
res =SpikedLearningCurve(25,25,1,50,NumInstances = 2,spike=100,TrialsPerInstance = 2)
```

```{r}
#rename RegSink to ShrinkFlop
colnames(res$Results) = c("Samples","Gemini","ShrinkFlop","Flip-Flop","Trivial")
colnames(res$OpResults) = c("Samples","Gemini","ShrinkFlop","Flip-Flop","Trivial")
colnames(res$GeoResults) = c("Samples","Gemini","ShrinkFlop","Flip-Flop","Trivial")
...
LearningCurvePlot(res$GeoResults)
...
LearningCurvePlot <- function(results){
  ...
  #df = melt(results, id.vars = "Regularizer",variable.name = "Algorithm")
  lp <- ggplot(data=df, aes(x=Samples, y=Error, group=Algorithm, color=Algorithm))+ geom_line() + geom_point() +  scale_y_log10(breaks = 10^(-10:10),labels = trans_format("log10", math_format(10^.x)))
#+ mytheme #for some reason theme was broken - maybe fonts?
...
```{r}
#do the plots 
#only do cols 1-5 because we didn't include kglasso

#geodesic
lp=LearningCurvePlot(res$GeoResults[,1:5])
lp = lp + ggtitle("Error in Geodesic distance") 
pdf("25-25-spiked-lc-geo.pdf")
lp
dev.off()
lp

lp=LearningCurvePlot(res$OpResults[,1:5])
lp = lp + ggtitle("Error in operator norm")
pdf("25-25-spiked-lc-op.pdf")
lp
dev.off()
lp

lp=LearningCurvePlot(res$Results[,1:5])
lp = lp + ggtitle("Error in Frobenius norm") 
pdf("25-25-spiked-lc-frob.pdf")
lp
dev.off()
lp
...
SparsityCurve <-function(SmallDimension,BigDimension,NumSamples, MinSparse,MaxSparse,NumInstances=2,TrialsPerInstance=2){
  ...
  results = data.frame(Sparsity = c(MinSparse:MaxSparse),Gemini=0,RegSink = 0,Sink = 0, Trivial=0)
  if(use_matlab){
    results['KGlasso']=0
...
```{r, echo=FALSE, message=FALSE, result=hide}
#test doing the learning curves

res =SparsityCurve(5,5,1,40,60)
```

Plotting sparsity curves
  ...
  for(j in MinSamples:MaxSamples){
    ...
    for (l in 1:NumInstances){ 
      ...
      for(k in 1:TrialsPerInstance){
        ...
        #set sinkhorn penalties
        tic("sinkhorn")
        SinkEstimate = sinkhorn(X,tol = .01*2^(-j/2))[[1]]
        toc()
        ...
        #regularized Sinkhorn
        #set sinkhorn penalties
        RegSinkFactor = .0005*2^(-j)
        tic("regularized sinkhorn")
        RegSinkEstimate = regsinkhorn(X,tol = .01*2^(-j/2), reg = RegSinkFactor)[[1]]
        toc()
        ...
        if(use_matlab){
          ...
          setVariable(matlab, dat = dat)
          #evaluate(matlab, "SmallDimension, class(BigDimension),class(NumSamples)")
          evaluate(matlab,"[C,D] = KGL_R(dat,SmallDimension,BigDimension,NumSamples,.4/NumSamples^(1/2),.4/NumSamples^(1/2),30,.01);")
          KGlassoEstimate = getVariable(matlab,"C")$C
...
Doing sparse learning curve plots

```{r, echo=FALSE, message=FALSE, result=hide}
#doing the learning curves

sparseres =LearningCurve(25,25,1,50,NumInstances = 2,sparsity=10,TrialsPerInstance = 2)
LearningCurvePlot(sparseres$Results)
LearningCurvePlot(sparseres$GeoResults)
```
...
#geodesic
lp=LearningCurvePlot(sparseres$GeoResults)
lp = lp + ggtitle("Error in Geodesic distance") #cool way to title on the fly :)
pdf("25-50-sparse-lc-geo.pdf")
lp
dev.off()
lp

lp=LearningCurvePlot(sparseres$OpResults)
lp = lp + ggtitle("Error in operator norm")
pdf("25-50-sparse-lc-op.pdf")
lp
dev.off()
lp

lp=LearningCurvePlot(sparseres$Results)
lp = lp + ggtitle("Error in Frobenius norm") 
pdf("25-50-sparse-lc-frob.pdf")
lp
dev.off()
lp
...
Computational timing stuff 

```{r}

Timings<-function(BigDimension, SmallDimension, NumSamples, NumInstances = 1, TrialsPerInstance = 1){
      spike=10
      #define the covariances
      for (l in 1:NumInstances){
        A = diag(SmallDimension)
        ...
        for(i in 1:NumSamples){
          ...
          RegSinkFactor = .05*10^(-2)
          tic("regularized sinkhorn")
          RegSinkEstimate = regsinkhorn(X,tol = .01, reg = RegSinkFactor)[[1]]
          toc()
          
          GeminiRegFactor = 1*sqrt(log(SmallDimension)/(BigDimension))*10^(-1.5)
          tic("geminiB")
          out <- GeminiBmult(X, GeminiRegFactor, penalize.diagonal=FALSE)
          toc()
...
```{r}
Timings(100,200,1,NumInstances =2, TrialsPerInstance  = 2)
