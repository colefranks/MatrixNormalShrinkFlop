...
\begin{theorem}\label{thm:hess-pisier} Let $X = (X_1, \dots, X_N)$ be a tuple of standard Gaussians on $(\R^{d_1}\ot \R^{d_2})^n$. Then with failure probability at most $\exp(-\Omega(d_{a} + d_{b}))$ \TODO{adjust like in the next corollary}, the random quantum channel 
$$T: X \mapsto \frac{1}{nD}\sum_{i = 1}^N X_i Z X_i^\dagger$$
has operator norm 
$O \left( \frac{d_{a}+d_{b}}{\sqrt{d_{a} d_{b} nD}} \right)$\TODO{do similar thing to the next corollary} restricted to the traceless symmetric matrices. \end{theorem}
...
$$\frac1{\sqrt{d_a d_b}} \langle Y,  \left( \nabla^2_{ab} f_{\samp} \right) Z \rangle
= O \left( \frac{d_{a}+d_{b}}{\sqrt{d_{a} d_{b} nD}} \right) \TODO{O(\lambda)}
$$
\TODO{Akshay, please make the rhs $O(\lambda)$ and then adjust the failure probability accordingly}
with probability \TODO{adjust accordingly}.
\end{corollary}
...
$$\frac{1}{\sum_{i \in \|Y_i\|^2}} Y_i Y_i^T$$ where $Y_i$ are $n D/{d_a d_b}$ i.i.d standard Gaussians on $\R^{d_a} \ot \R^{d_b}$. By \cref{lem:hess-pisier}, the operator norm of $T$ restricted to $\Sym^0_{d_b}$ is $O((d_a + d_b) \sqrt{ n D/d_ad_b} / \sum_{i \in \|Y_i\|^2} )$ \TODO{propagate from \cref{cor:off-diag-hess}} 
