...
\CF{based on new convention (see \cref{dfn:g-convexity,lem:hessian}) we may want to omit the denominator $\sqrt{d_{a} d_{b}}$ now.} Pisier's method of proof uses the trace method along with Gaussian concentration in Banach spaces.
...
\begin{theorem} [Pisier]
Let $Y$ be a standard gaussian in \CF{real, finite dimensional?} Banach space with norm $\|\cdot\|$. Then $\|Y\|$ is subgaussian with parameter $\sigma^{2} := \sup \{ \E \langle Y, \xi \rangle^{2} \mid \|\xi\|_{*} \leq 1 \}; $ that is
\[ \Pr [ | \|Y\| - \E \|Y\| | \geq t ] \leq \exp \left( - \frac{\Omega(t^{2})}{\sigma^{2}} \right).   \]
\end{theorem}

\begin{corollary}\CF{is there a constraint on $p$?}
...
\begin{theorem}
Let $P: \mat(m) \to \mat(m)$ denote the projection onto the span of $I_m$, and $\{Y_{i}\}$ are iid standard gaussians in $\mat(n,m)$ \CF{ I changed it to $\mat$ because this needs to go $\mat(m) \to \mat(n)$ if I understand correctly}. There is a constant $c > 0$ such that for all $m \leq n$ we have 
\[ \|\sum_{i=1}^{N} \alpha_{i} Y_{i} \otimes Y_{i} (I - P) \|_{op} \leq O \left( \sqrt{\sum_{i} \alpha_{i}^{2}} \right) \left( \E \|Y\|_{op} \right)^{2} \]
with probability at least $ 1- (mn)^{-c}$.
\end{theorem}
\begin{proof}
We first begin by a standard symmetrization trick to linearize: if $Y$ is a standard Gaussian on $\mat(n,m)$, then $\E Y \otimes Y (I-P) = 0$ because for any $X \in \mat(m)$ we have
$$ \E Y \otimes Y (I-P)X = \E (Y \ot Y) X - \frac{1}{m} (\tr X) \E (Y \ot Y)  I_m,$$ which is easily calculated to be zero.
...
\[ \E_{Y} \|\sum_{i=1}^{N} \alpha_{i} Y_{i} \otimes Y_{i} (I - P)\|_{op} \leq \E_{Y,Z} \|\sum_{i=1}^{N} \alpha_{i} Y_{i} \otimes Y_{i} (I - P) - \sum_{i=1}^{N} \alpha_{i} Z_{i} \otimes Z_{i} (I - P) \|_{op}  \] Now note $(Y,Z)$ has the same distribution as $(\frac{Y+Z}{\sqrt{2}},\frac{Y-Z}{\sqrt{2}})$, so the right-hand-side of the above equation is
...
Note we've lost the projection $(I-P)$, but the left and right Kraus operators are independent. Now we apply the trace method to this operator \CF{quick blurb about what the trace method is to motivate calculation, basically what's done at the end of the proof}
...
\end{align*}
Here we used independence of $(Y,Z)$. We eventually want to charge to a term $\E \|\sum_{i} \alpha_{i} Y_{i}\|_{op}$. We note expectations of gaussian monomials are non-zero and positive iff the polynomial is even. Therefore the coefficient $\alpha^{\vec{i}} \alpha^{\vec{j}}$ from all non-vanishing terms is a square, and in particular positive. So we can upper bound each term individually by the nc-Holder inequality (\cref{thm:holder} below) to find that 
...
We now consider the term $\E_{Z} \|Z_{i_{1}}^{*}\|_{2p} ... \|Z_{j_{p}}\|_{2p}$. It need not factor into a product of $2p$ expectations because some of the indices may be repeated. However, for an index $j$ repeated $k$ times we can use that $\E \|Z_j\|_{2p}^{k} = \E (\|Z_j\|_{2p}^{2p})^{k/{2p}} \leq  ( \E \|Z_j\|_{2p}^{2p} )^{k/2p}$ for $k \leq 2p$ by Jensen's inequality. Thus, we can collect like terms together:
...
Here we've used that $\|Y\|_{op}$ is subgaussian with parameter $\sigma^{2} \leq \E \|Y\|_{op}^{2} \sim (\sqrt{m} + \sqrt{n})^{2}$ up to universal constants.
So again assuming $\|\alpha\|_{2} = 1$ and choosing $p = 2c \log mn$, where the only constraint is \CF{where's this constraint come from?}
\[  p^{3/2} \ll (\sqrt{m} + \sqrt{n})^{2} \iff c \ll \frac{(\sqrt{m} + \sqrt{n})^{4/3}}{\log mn}  \] 
We can apply Markov's to get the bound: \CF{define probabilty macro}
\[ \Pr[\|\sum_{i} \alpha_{i} Y_{i} \otimes Y_{i}\|_{op} \geq e (\E \|Y\|_{op})^{2} ] \leq    \frac{m^{2} (\E\|Y\|_{op} + \frac{C \sqrt{2p}}{\sqrt{m} + \sqrt{n}} )^{4p}}{e^{2p} (\E\|Y\|_{op})^{4p}}    \]
\[ \leq \frac{m^{2} \exp \left(\frac{C' p^{3/2}}{(\sqrt{m} + \sqrt{n})^{2}} \right) }{e^{2p}} < \frac{1}{(mn)^{c}}  \]
\end{proof}
...
\AR{I may have made a calculation error but it seems we could even pick $c \sim \frac{n^{2/3}}{\log n}$ so that we get $\exp(-\frac{n^{2/3}}{\log n})$ failure probability?} \CF{ I agree - let's implement it!}
