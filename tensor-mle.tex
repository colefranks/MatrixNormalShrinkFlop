...
\item \textbf{Bound the gradient: } Show that $\nabla f_{\vec X} (I) $ is small with high probability (w.r.t. the dual norm $\|\cdot\|_{*}$). \CF{Probably need to say something about why geodesic distance and $d$ are comparable.}
...
\begin{lemma}\label{lemma:gradient-bound} Let $N_a := n D/d_a$, and suppose $N_a \geq C d_a/\eps^2$ for all $a \in [k]$. Then with probability at least $1 -  \sum_{a \in [k]} e^{-\Omega(N_a)},$ for all $a \in [k]$ we have 
$$\| \nabla_a f \|_{op} \leq \epsilon/d_a.$$
As a consequence, $\|\nabla f(I)\|_{*}^{2} := \sum_{a} d_{a} \|\nabla_a f \|_{F}^{2} \leq k \epsilon^{2}$. 
\end{lemma}

\begin{theorem}\label{thm:tensor-convexity}
Suppose the number of samples is at least $k^{2} \max_{a} d_{a}^{2}/D$. Then with failure probability $\leq 1/poly(D)$, $f_X$ is $\Omega(1)$-strongly convex at $I$ w.r.t. norm $\|Z\|^{2} := \sum_{a} \frac{1}{d_{a}} \|Z_{a}\|_{F}^{2}$. 
\end{theorem}
...
\AR{akshayproof}
\cref{lem:perturbation} shows that $f$ is $\Omega(1)$ strongly convex for $e^{Z}$ with 
We may apply \cref{lem:convex-ball} with $\kappa = $ \CF{finish once dual norms straightened out}

By \cref{lemma:gradient-bound}, we have 
\[ \forall a: \|(\nabla f)_{a}\|_{op} \leq \frac{\eps}{d_{a}} \implies \|\nabla f\|_{*}^{2} = \sum_{a} d_{a} \|(\nabla f)_{a}\|_{F}^{2} \leq k \eps^{2}  \]
...
Provided $c$ is small enough we then have 
 $$\lambda_1(YY^\dagger), \lambda_{d}(YY^\dagger) \in N(1 + c \cdot \eps).$$ 
