...
The minimizers of $\ell_\samp$ and $f_\samp$ are identical restricted to $\SPD$. However, the MLE need not have unit determinant, so it remains to show that we can use our consistency results for the minimizers of $f_\samp$ to show that the MLE itself is consistent.

%However, $\Theta$ need not have unit determinant, and the MLE may not have 


\begin{lemma}[Handling non-unit determinant]
Suppose the minimizer of $f_\samp$ is within $\eps$ of $\Theta$ in $d_F$ (resp. $d_{op}$) with probability at most $p(\eps)$ for all $\Theta \in \SPD$. Then the MLE is within $\eps + \delta$ of $d_{op}$ with probability $p(\eps) + \gamma$.  
\end{lemma}
\begin{proof}
Like the minimizer to $f_\samp$, the MLE is equivariant - that is, because $\ell_{\Sigma^{1/2} \samp} (\Upsilon) = \ell_\samp(\Sigma^{1/2} \Upsilon \Sigma^{1/2})$, for $X \sim \mathcal{N}(0, I)$ the minimizer $\hat{\Theta}_{\Sigma^{1/2}\rv}$ is has the same distribution as $\Sigma^{-1/2} \hat{\Theta}_{\rv} \Sigma^{-1/2}$. Thus, it is enough to prove the claim for $\Theta = I$. 

Recall that if $f_\samp$ is minimized at $\widehat{\Theta}$, then $\ell_\samp$ is minimized at $\Theta':= n D e^{ - f_\samp(\widehat{\Theta})} \widehat{\Theta}:=\alpha  \widehat{\Theta}$. Thus $\Theta'_a := \alpha^{1/k}\widehat{\Theta}_a$. By our bound on $f_\samp(\Theta)$, $\alpha \in (1 \pm \delta)$ with probability $1 - q(\delta)$. \CF{think $\delta = \|\nabla f\|^2;$.} Thus with probability $1 - q(\delta) + p(\eps)$, the distance of the MLE from the truth is $d(\Theta'_a;I) \in d( (1 \pm \delta)^{1/k}\widehat{\Theta}; I)$. By the approximate triangle inequality \CF{cite}, $d( (1 \pm \delta)^{1/k} \widehat{\Theta}_a; I) = O(d(\widehat{\Theta}; I) + d((1 \pm \delta)^{1/k}I; I))$. If $d = d_F$, the latter term is $O(\delta \sqrt{d_a}/k)$; if $d = d_{op}$ the latter is $O(\delta/k)$. \CF{I think the square Frobenius norm of the gradient will typically be at most $d_a(1/d_aN_a) = 1/N_a$; in the $N_a \geq d_a^2$ regime it becomes $1/d_a^2$ and in the other regime it is $1/d_a$. Either does not seem like  cause for concern.}
\end{proof}
...
\textbf{Output}: $\underline{\Sigma} \in \PD_D$ such that $d_F(\underline{\Sigma}_i, \hat{\Sigma}_i) < \eps$, for each $i \in [k]$, where $\hat{\Sigma}$ is the MLE for $\Sigma$. \\[.3ex]
...
\item
If $\norm{\nabla_a}_F^2 < \eps$, output $\left( \bigotimes_{a =1}^k \Theta_a \right)^{-1}$ and return.
...
\begin{lemma}[Descent Lemma]
	...
	$$ f_x(\Upsilon) \leq f_x(\Theta) - \dfrac{1}{6k d_1} \cdot \norm{\nabla f_x(\Theta)}_F^2 $$
\end{lemma}

\RMO{are we going to assume that $d_1 \geq d_2 \geq \cdots \geq d_k$?}\CF{No actually its $d_1\leq \dots \leq d_k$.}
...
\begin{proof}
	...
	\begin{align*}
		\frac{1}{d_a} \cdot  \log\det(d_a \cdot \rho^{(a)}) &\leq - \dfrac{1}{6 d_a} \leq - \dfrac{1}{6 d_1} \leq - \dfrac{1}{6d_1} \cdot \norm{\nabla f_x(\Theta)}_F^2
	\end{align*}
\end{proof}
...
\begin{theorem}[Restatement of \cref{thm:tensor-flipflop}]
	If $\hat{\Sigma}$ denotes the MLE estimator for $\Sigma$, then provided $n = \Omega(k^2 \cdot d_1^2/D)$, the flip-flop algorithm computes $\underline{\Sigma}$ with 
	$$ d_F(\hat{\Sigma}_i, \underline{\Sigma}_i) \leq \epsilon $$
	in $O(\log(1/\epsilon))$ iterations with probability at least 
