...
Rather than simply considering the metric induced by the Euclidean metric on the symmetric matrices, we consider the metric whose unit-speed geodesics starting at a point $\Theta \in \PD(d)$ are of the form $\Theta^{1/2} e^{Ht} \Theta^{1/2}$ for $t \in \R$ and a symmetric matrix~$H$ with $\norm H_F=1$.
...
\MW{Say that it will be convenient to write $\Theta = \lambda \Theta_1 \ot \dots \ot \Theta_k$ for $\lambda>0$ and all $\det\Theta_a=1$. The main challenge will be maximizing the log-likelihood funcion for $\lambda=1$. Indeed, one can then determine the right $\lambda$ as (formula from below).}

Thus we define
% As above, we can fix this by working with tuples of precision matrices with equal determinant:
\begin{align*}
  \P &= \{ (\Theta_1,\dots,\Theta_k) \in \PD(d_1) \times \dots \times \PD(d_k) \;:\; \det \Theta_1 = \dots = \det \Theta_k = 1 \}.
% \end{align*}
\intertext{The tangent space of this manifold is naturally identified with the real vector space}
% \begin{align*}
  \H &= \{ (H_1,\dots,H_k) \;:\; H_a \text{ symmetric $d_a \times d_a$ matrix}, \, \tr H_1 = \dots = \tr H_k = 0 \}.
\end{align*}
The embedding $(\Theta_1,\dots,\Theta_d,\lambda) \mapsto \lambda \Theta_1 \ot \dots \ot \Theta_d$ identifies $\P$ with the separable covariance matrices of determinant one, which form a totally geodesic submanifold of $\PD(D)$.
...
\end{align*}
By definition, the \emph{geodesics} through $\Theta$ are the curves
$t \mapsto \exp_\Theta(t H)$
for $t\in\R$ and $H\in\H$.
They are unit speed if $\sum_{a=1}^k \norm{H_a}_F^2 = \frac1D$.
\MW{A bit sad, should we redefine the metric on $\PD(D)$ such that this becomes $=1$?}
\end{definition}
% Indeed,
% \begin{align*}
%   \norm{\sum_{a=1}^k \sqrt{d_a} H_a \ot I_{\frac{D}{d_a}}}_F^2
% = \sum_{a=1}^k \norm{\sqrt{d_a} H_a \ot I_{\frac{D}{d_a}}}_F^2
% = \sum_{a=1}^k d_a \norm{H_a \ot I_{\frac{D}{d_a}}}_F^2
% = D \sum_{a=1}^k \norm{H_a}_F^2
% \end{align*}
...
Assuming $f$ is twice differentiable, this holds if $\partial^2_t f(\exp_\Theta(tH)) \geq 0$ for all~$H\in\H$.

Similarly, $f$ is called \emph{$\lambda$-strongly geodesically convex} at $\Theta$ for some $\lambda>0$ if the same is true for the functions $t \mapsto f(\exp_\Theta(tH))$ for all~$H\in \H$.
Assuming the function is twice differentiable, this holds if $\partial^2_t f(\exp_\Theta(tH)) \geq \lambda \sum_{a=1}^k \norm{H_a}_F^2$ for all~$H\in\H$.
\end{definition}
...
Moreover, the mapping $\Theta \mapsto A\Theta A^T$ is an isometry and it preserves the statistical distances~$D_F$ and $D_{\op}$ between the individual precision matrices as well as the global precision matrices.

We also note that the MLE obeys a certain \emph{equivariance} property under such transformations.
For all $\Theta \in \P$, $A \in \G$, and samples $x=(x_1,\dots,x_n)$, the log-likelihood satisfies
\begin{align*}
...
\MW{I'd state this above (including of MLE to this).}
...
\CF{Reprove using intermediate deterministic lemma so better bounds on failure probability or quantum expansion can be plugged in.}
...
\CF{I want to just rewrite the section using the new concentration results.}
...
\CF{reiterate that $\Phi$ is what appears in the gradient}
...
The flip-flop algorithm for this regularized estimator is nearly as simple as the unregularized flip-flop algorithm: see \cref{alg:reg-flip-flop}. The only difference that a certain multiple of the identity is added in each flip-flop step.  \CF{I need to check the regularization term carefully - it probably comes with a normalization factor} 
...
We compare the performance of the estimator with Zhou's single step estimator \CF{cite gemini} for the matrix normal model. We find that when both marginals are well-conditioned, 
