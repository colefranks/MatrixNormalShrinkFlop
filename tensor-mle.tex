...
$1 - \left(\frac {s \sqrt{nD}}{d_b}\right)^{ - \Omega(d_a)} - e^{- \Omega(nD)}$.
% for $s \geq d_b /\sqrt{n D}.$

In particular, for any small enough absolute constant $c>0$ there is $C$ such that for $n \geq C d_1^2/D$ we have $\|\nabla^2_{ab} f_{\rv}\|_0 \leq c/k$ with probability $1 - k^2 \left(\frac {\sqrt{nD}}{2d_1}\right)^{ - \Omega(d_1)}$ for all $a,b \in [k]$. 
