...
Let $\rho := \sum_i x_i x_i^T$ denote the ``sample covariance matrix'' of the data.
Then we can rewrite the objective function as
\begin{align}\label{eq:obj via rho}
  f_x(P_1,\dots,P_k) = \log \tr \rho \left( \textstyle \bigotimes_{a=1}^k P_a \right).
\end{align}
We may also consider the ``sample covariance matrix'' of an individual coordinate.
For this, analogously to the discussion in \cref{subsec:model}, we ``flatten''~$x$ by thinking of it as a $d_a \times \frac{d_1d_2d_3\dots d_kn}{d_a}$ matrix~$x^{(a)}$ for any $a\in[k]$, and define $\rho^{(a)} := x^{(a)} (x^{(a)})^T$.
It is not hard to verify that $\rho^{(a)}$ is a positive definite $d_a\times d_a$ matrix and satisfies the following property:
...
Note that $\tr \rho^{(a)} = \tr \rho = \norm x_2^2$.
In the language of linear algebra, $\rho^{(a)}$ is the \emph{partial trace} of~$\rho$ over all but one of the coordinates.
The gradient can be readily computed in terms of this data.
\MW{Careful, $\rho$ is now unnormalized (since Cole's nice text did it that way).}
...
\MW{OLD}
Define $F_x(P) := \tr \rho \, (\bigotimes_{a=1}^k P_a)$, so that $F_x(P) = e^{f_x(P)}$ by \cref{eq:obj via rho}.
  Then, for all $a\in[k]$ and any $d_a\times d_a$ matrix~$H$, we have
  \begin{align*}
    \braket{\nabla_a F_x(I,\dots,I), H}
  &= \partial_{t=0} F_x(I,\dots,I,e^{t\sqrt{d_a} H},I,\dots,I) \\
  &= \partial_{t=0} \tr \rho \, (I \ot \cdots \ot I \ot e^{t\sqrt{d_a} H} \ot I \ot \cdots \ot I) \\
  &= \sqrt{d_a} \tr \rho \, H_{(a)}
  = \sqrt{d_a} \tr \rho^{(a)} \, H
  \end{align*}
  ...
  \begin{align*}
    \nabla_a F_{\samp}
  = \sqrt{d_a} \left( \rho^{(a)} - \tr(\rho^{(a)}) \frac{I_{d_a}}{d_a} \right)
  = \sqrt{d_a} \left( \rho^{(a)} - \tr(\rho) \frac{I_{d_a}}{d_a} \right).
  \end{align*}
  Finally, since $f_x(P) = \log F_x(P)$, we obtain
  \begin{align*}
    \nabla_a f_x
  = \frac{\nabla_a F_x}{F_x(I,\dots,I)}
  % = \frac{\sqrt{d_a} \left( \rho^{(a)} - \tr(\rho) \frac{I_{d_a}}{d_a} \right)}{\tr\rho}
  = \sqrt{d_a} \left( \frac{\rho^{(a)}}{\tr\rho} - \frac{I_{d_a}}{d_a} \right),
  \end{align*}
  using that $F_x(I,\dots,I) = \tr \rho$.
