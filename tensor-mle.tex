...
\begin{proof}[Proof of \cref{thm:matrix-normal}\CF{NEW}]
As discussed in \cref{subsec:proof-sketch}, it is enough to prove \cref{thm:matrix-normal} under the assumption $\Theta_a = I_{d_a}$ for $a \in \{1,2\}$. Consider the following events.
\begin{enumerate}
...
We now would like to show that $\tilde{\Phi}$ is an $(\eps_0 \sqrt{{d_1}/{n d_2}}, 1 - \lambda/2)$ expander. The MLE is 
\end{proof}
