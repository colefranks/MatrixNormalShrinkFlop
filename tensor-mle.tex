...
Next we would like to use the $k=2$ case of \cref{thm:tensor-convexity} which shows that $f_{x}$ is at least $3/4$-strongly convex at the identity with failure probability $O \left(\frac {\sqrt{nD}}{d_1}\right)^{ - \Omega(d_1)}$. We again have the following translation by \cref{lem:hessian}
...
which by Courant-Fischer is sufficient to show $\Phi$ is a $(1-3/4)$-quantum expander. 
\end{proof}

We will prove \cref{thm:operator-cheeger} in the following section, but before doing so we can use it to prove \cref{thm:matrix-normal}.
