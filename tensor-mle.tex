...
\begin{lemma}[Fast Convergence from Initial Conditions]\label{lem:fast-convergence-initial-generic}
	...
	\begin{enumerate}
		...
		\item $\norm{\nabla f_x (I_D)} \leq \dfrac{r}{4}$
		\item $f_x$ is $\lambda$-strongly geodesically convex at $B_r(I_D)$
	\end{enumerate}
	Then, in $T = TODO$ iterations, \cref{alg:flip-flop} outputs an estimator $\otheta$ such that 
	$$ D_F(\htheta \ || \ \otheta) \leq \eps $$
\end{lemma}
...
$\delta = \dfrac{\eps \lambda}{2 \dmax^{1/2}}$. 
%$\delta = \dfrac{\eps^2}{16 k^2 \dmax^{3/2}}$. for tensor normal model
...
\begin{proof}
	...
	This in turn implies that \cref{lem:convex-ball} applies, and therefore we have that the sublevel set $\{ \Upsilon \ \mid \ f(\Upsilon) \leq f(I_D)  \}$ is contained in the ball $B_r(I_D)$. 
	...
	Thus, after $T = 24k \log(1/\delta)$ steps, \cref{lem:descent-sublevel-set} guarantees us that we will encounter a scaling $\otheta$ such that
	$$ \norm{\nabla f_x(\otheta)}_F \leq \delta = \dfrac{\eps^2}{16 k^2 \dmax^{3/2}}.$$
...
\textbf{Algorithm}:
\begin{enumerate}
\item\label{it:flip-flop step 1 matrix} Set $\otheta_a = \dfrac{1}{d_a} I_a$ for each $a \in \{1,2\}$, and $\delta = \dfrac{\eps^2}{16 d_2^{3/2}}$.
...
\begin{enumerate}
\item Provided $d^2 \leq c n$, \CF{instead just say $\min \{d^2/n, c\}$?, and likewise for the other?}
\begin{align}
\sup_{\Theta \in \PD(d)} \E[ d_F(\htheta||\Theta)^2] = \Omega(\frac{d^2}{n}).\label{eq:df-lower}
\end{align}
and provided $d \leq c n$, 
\item 
 \begin{align}
\sup_{\Theta \in \PD(d)} \E[ d_{op}(\htheta||\Theta)^2] = \Omega(\frac{d}{n}). \label{eq:dop-lower}
\end{align}
\end{enumerate}
\end{prop}
...
$$D_{KL}(P_i|| P_j) = \frac{1}{2} \tr \Theta_i^{-1} \Theta_j - \frac{1}{2}\log\det(\Theta_i^{-1} \Theta_j) - \frac{d}{2} = \frac{1}{2} \sum_{i =1}^d (\lambda_i -1 -  \log \lambda_i)$$
 ...
 Moreover, for $i \in [m]$, we have $\|\Theta_i^{-1}\|_{op} \leq 2$ and so $D_F(\Theta_j|| \Theta_i) = O( \|\Theta_i - \Theta_j\|_F) = O(\delta).$ We now have $D_{KL}(P_i|| P_j) \leq C \delta^2$ for some absolute constant $C$. By Fano's lemma, 
 $$\min_{i \in [m]} \Pr_{x \sim P_i}[T(X) =  i] \leq \frac{ - \log 2 + C n \delta^2}{d(d+1)(\log 2)/2 }.$$
If $\delta^2 = c\min\{ \frac{d^2}{n}, \frac{1}{4}\}$, the right-hand side is bounded by $\frac{1}{2}$ and the assumption $\delta \leq 1/2$ is satisfied on the other hand, we showed $\Pr_{x \sim P_i}[T(X) =  i] \geq \Pr[\|\htheta -  \Theta_i\|_F \leq \delta/2]$. Thus, 
$$\min_{i \in [m]} \Pr[ \|\htheta - \Theta_i\|_F \leq \delta/2] \leq 1/2.$$
This implies the rate lower bound in $\|\cdot\|_F$. We now deduce the lower bound on $D_F(\htheta ||\Theta_i),$  \CF{continue.}
