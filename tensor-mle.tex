...
Let $A$ be an $n \times m$ matrix with independent standard Gaussian entries.
Then, for $\max\{m,n\}$ large enough
\begin{equation*}
  \E \norm{A}_{\op} \geq \frac{1}{3} (\sqrt{n} + \sqrt{m}).
\end{equation*}
\end{lemma}
\begin{proof}
Without loss of generality, we assume that $n \geq m$.
...
Here the lower bounds follow for $n = \max\{m,n\}$ large enough since $C$ is a universal constant.
\end{proof}
