...
\usepackage{amsmath,amsthm,mathtools,braket,amssymb,bm,fullpage,xcolor,float}
...
\author{Cole Franks\thanks{Department of Mathematics, Massachusetts Institute of Technology. Email: \email{franks@mit.edu}.}
\and Rafael Oliveira\thanks{Cheriton School of Computer Science, University of Waterloo.}
\and Akshay Ramachandran\footnotemark[2]
\and Michael Walter\thanks{Korteweg-de Vries Institute for Mathematics, Institute for Theoretical Physics, Institute for Logic, Language and Computation \& QuSoft, University of Amsterdam. Email: \email{m.walter@uva.nl}. This work was supported in part by NWO Veni grant no.~680-47-459 and NWO grant OCENW.KLEIN.267.}}
\date{}
%=============================================================================
...
To remedy this issue, matrix-variate data is commonly assumed to follow the \emph{matrix normal distribution}~\cite{dutilleul1999mle,werner2008estimation}.
...
Dutilleul and later Werner et al.~proposed an iterative algorithm, known as the \emph{flip-flop algorithm}, to compute the maximum likelihood estimator (MLE)~\cite{dutilleul1999mle,werner2008estimation}.
In the same work Werner et al.\ also showed that the MLE is consistent and asymptotically normal, and showed the same for the estimator obtained by terminating the flip-flop after three steps.
...
Assuming that the covariance factors have constant condition number and that $n$ is at least $\tilde{\Omega}(\max\{d_1,d_2\})$, in~\cite{tsiligkaridis2013convergence} it was shown that the three-step flip-flop estimator has mean-squared Frobenius error of $O((d_1^2 + d_2^2)/n)$ for the full matrix $\Sigma$; they did not state a bound for the individual factors $\Sigma_1,\Sigma_2$.
...
In the extremely undersampled regime, Zhou demonstrated a single-step penalized estimator that converges even for a single matrix $(n=1)$ when the precision matrices have constant condition number, are highly sparse, and have bounded $\ell_1$ norm off the diagonal~\cite{zhou2014gemini}.
...
Allen and Tibshirani also considered penalized estimators for the purpose of missing data imputation~\cite{allen2010transposable}.
For the tensor normal model, a natural generalization of the flip-flop algorithm has been proposed to compute the MLE but is not known to converge~\cite{mardia1993spatial,manceur2013maximum}.
Assuming bounded constant condition number of the true covariances and knowledge of initializers within constant Frobenius distance of the true precision matrices, the authors of~\cite{sun2015nonconvex} propose an estimator with tight rates.
...
It was recently noted that the matrix normal and tensor MLEs are equivalent to algebraic problems about a group action called the \emph{left-right action} and the \emph{tensor action}, respectively~\cite{amendola2020invariant}.
In the computer science literature these two problems are called \emph{tensor} and \emph{operator scaling}, respectively.
It was independently pointed out in~\cite{FM20} that the Tyler's M estimator for elliptical distributions (which arises as the matrix normal problem under the additional promise that~$\Sigma_2$ is diagonal) is a special case of operator scaling.
Using the connection to the left-right action, exact sample size thresholds for the existence of the MLE were recently determined for the matrix normal model and subsequently for the tensor normal model~\cite{derksen2020matrix,derksen2020tensor}.
In the context of operator scaling, Gurvits showed much earlier that the flip-flop algorithm converges to the matrix normal MLE whenever it exists~\cite{gurvits2004classical}, and later it was shown that the number of flip-flop steps to obtain precision~$\eps$ for the tensor and matrix normal model is polynomial in the input size and~$1/\eps$~\cite{GGOW19,burgisser2017alternating} (cf.~\cite{burgisser2019towards}).
...
To this end, we consider the following dissimilarity measures:
...
\end{align*}
If $A, B$ are the precision matrices of centered Gaussians, $d_F$ is sometimes called the \emph{Mahalanobis distance}.
\end{definition}

The Mahalanobis distance is a natural dissimilarity measure on Gaussians because when any of the Mahalanobis, total variation, square of the KL-divergence, or Fisher-Rao distances is at most a small constant then they are on the same order \CF{check, cite}~\cite{barsov1987estimates}.
Considering $d_{\op}$ yields even stronger results.
...
\Cref{thm:matrix-normal}, proven later in \cref{sec:matrix-normal}, requires additional tools. We now discuss the geodesic convexity used here.
...
%These curves are in fact geodesics, or ``straight lines,'' through $\Theta$ for a natural Riemannian metric on~$\SPD$~\cite{bhatia2009positive}. \CF{fisher-rao}
...
%\CF{put somewhere}. It is well-known that various optimization algorithms applied to $f_{\samp}$ satisfy such a descent lemma~\cite{burgisser2019towards}.
%-----------------------------------------------------------------------------
...
\subsection{Net and union bound}
\CF{do the net bound like in the vershynin survey}
