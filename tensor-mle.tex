...
Suppose
\begin{#1}#2
  1 \leq \eps^2 \leq \frac{nD}{C k d_{\max}^2 \max\{k, d_{\max}\}}.
\end{#1}
...
\begin{align*}
  D_F(\htheta_a\Vert\Theta_a) &= O\left(k^{1/2} \frac{\sqrt{d_a} \, d_{\max}}{\sqrt{n D}} \eps\right) \qquad\text{ for all } a\in[k], \\
...
\end{align*}
where the Kronecker factors are chosen s.~th.~$\det\htheta_1 = \cdots = \det\htheta_k$, $\det\Theta_1 = \cdots = \det\Theta_k$.}
...
% \begin{theorem}[Tensor normal Frobenius error]\label{thm:tensor-frobenius}
% There are universal constants $C,c>0$ such that the following holds.
% Suppose $\delta \leq \smash{c / \!\! \sqrt{(k+1)d_{\max}}}$ and that the number of samples satisfies $n \geq C k \frac{d_{\max}^2}{D \delta^2}$.
% Then the MLE~$\htheta = \htheta_1 \ot \cdots \ot \htheta_k$ for $n$ independent samples of the tensor normal model with precision matrix~$\Theta = \Theta_1 \ot \cdots \ot \Theta_k$ satisfies
% \begin{align*}
%   D_F(\htheta_a\Vert\Theta_a) &= O(\sqrt{d_a} \delta) \qquad\text{ for all } a\in[k], \\
%   D_F(\htheta\Vert\Theta) &= O(k e^{2\delta k} \sqrt D \delta),
% \end{align*}
% with probability at least
% \begin{align*}
%   1 - k e^{-\Omega\bigl( \delta^2 \frac{nD}{k d_{\max}} \bigr)} - k^2 \left( \frac{\sqrt{nD}}{k d_{\max}} \right)^{-\Omega(d_{\min})},
% \end{align*}
% where the Kronecker factors are chosen s.~th.~$\det\htheta_1 = \cdots = \det\htheta_k$, $\det\Theta_1 = \cdots = \det\Theta_k$.
% \end{theorem}

% \begin{theorem}[Tensor normal Frobenius error]
...
Let~$c$ be the minimum of~$1$ and the constant from \cref{thm:ball-convexity},
and let $C$ be the maximum of~$25 \cdot 82$ and the constant from \cref{thm:ball-convexity}.
For
\begin{equation}\label{eq:def delta}
  \delta = \frac{\sqrt{Ck} \, d_{\max} }{\sqrt{nD}} \eps,
\quad\text{which satisfies}\quad
  \delta^2 \leq \frac1{\max\{k, d_{\max}\}},
\end{equation}
consider the following two events:
\begin{enumerate}
\item\label{it:grad-bd} $\norm{\nabla f_x}_F \leq \frac\delta5$.
...
\end{enumerate}
By our assumption~\eqref{eq:eps sqr assm}, both \cref{prop:gradient-bound,thm:ball-convexity} apply, with the former showing the first and the latter showing the second event; the desired success probability then follows from the union bound.

%%%
% {\color{blue}
% To verify that \cref{prop:gradient-bound} applies, note that we wish to apply it with $\eps_{prop}$, where
% \begin{align*}
%   \sqrt{82 k \eps_{prop}^2} = \frac\delta5
% \quad\Leftrightarrow\quad
%   \eps_{prop} = \frac1{\sqrt{25 \cdot 82}} \frac{ \sqrt{C} d_{\max} }{\sqrt{nD}} \eps.
% \end{align*}
% Hence we need to verify the following conditions:
% \begin{align*}
%   \eps_{prop} < 1
%   \quad\text{and}\quad
%   n \geq \frac{d_{\max}^2}{D \eps_{prop}^2}
% \end{align*}
% These are equivalent to
% \begin{align*}
%   \frac1{\sqrt{25 \cdot 82}} \frac{ \sqrt{C} d_{\max} }{\sqrt{nD}} \eps \leq 1
%   \Leftrightarrow
%   \eps^2 \leq 25 \cdot 82 \frac{nD}{C d^2_{\max}}.
% \end{align*}
% and
% \begin{align*}
%   n \geq \frac{d_{\max}^2}{D \eps_{prop}^2}
%   \Leftrightarrow
%   \eps_{prop}^2 \geq \frac{d_{\max}^2}{Dn}
%   \Leftrightarrow
%   \eps^2 \geq \frac{25 \cdot 82}C.
% \end{align*}
% The first condition is clearly implied by the upper bound in~\eqref{eq:eps sqr assm}.
% The second condition is implied by the lower bound in~\eqref{eq:eps sqr assm} (and using that $C\geq 25 \cdot 82$).
% This \cref{prop:gradient-bound} applies and event~1 holds with failure probability
% \begin{align*}
%   2(k+1)e^{-\eps_{prop}^2 \frac{nD}{8d_{\max}}}
% = 2(k+1)e^{-\frac{ \frac{C}{25\cdot82} d_{\max}^2 }{nD} \eps^2 \frac{nD}{8d_{\max}}}
% = k e^{-\Omega(\eps^2 d_{\max})}
% \end{align*}
% %%%
% For \cref{thm:ball-convexity} we simply need that
% \begin{align*}
%   n \geq C_{thm} k \frac{d_{\max}^2}D
% \end{align*}
% (by the comment below the theorem).
% This follows from our assumption, since
% \begin{align*}
%   \frac{nD}{Ck d_{\max} \max\{k, d_{\max}\}} \geq 1
% \Leftrightarrow
%   n \geq C k \frac{d_{\max}^2 \max\{k, d_{\max}\}}{D}
% \Rightarrow
%   n \geq C k \frac{d_{\max}^2}{D}.
% \end{align*}
% (since $C \geq C_{thm}$).
% Thus \cref{thm:ball-convexity} applies and event~2 holds with failure probability
% \begin{align*}
% k^2 \left( \frac{\sqrt{nD}}{k d_{\max}} \right)^{-\Omega(d_{\min})}
% \end{align*}}
%%%

By \cref{lem:convex-ball}, these two events in turn, together with our assumption on~$\delta$, imply the MLE~$\htheta$ exists, is unique, and has geodesic distance at most $\delta$ from $I_D$.
...
\leq d_a \delta^2.
\end{align*}
By \cref{eq:def delta}, $d_a \delta^2 \leq 1$, so we have
\begin{align*}
  D_F(\htheta_a\Vert I_a)
= \norm{I_a - \htheta_a}_F
\leq 2 \norm{\log\htheta_a}_F
\leq 2 \sqrt{d_a} \, \delta
...
= O\left( {\color{red}e^{2\delta k}} k^{3/2} \frac{d_{\max} }{\sqrt{n}} \eps \right)
\end{align*}
\MW{Bound $\delta k$ once we agree on it. I think we can just change $k$ to $k^2$ inside the $\max$?}
\end{proof}
...
Let $\Phi\colon\mat(d_b) \to \mat(d_a)$ be a completely positive map.
Say $\Phi$ has spectral gap $\gamma$ if
...
Because~$k = 2$, the gradient and Hessian are completely described by the operator~$\Phi^{(12)}$, compare \cref{lem:gradient,lem:hessian} with \cref{eq:hessian channel}.

\begin{theorem}[Proof of Theorem~3.22 in \cite{KLR19}]\label{thm:klr}
There is a universal constant $C>0$ such that the following holds.
...
\begin{align*}
  \max \{ \norm{P_1 - I_{d_1}}_{\op}, \norm{P_2 - I_{d_2}}_{\op} \} \leq O\Bigl(\frac{\eps \log d_{\min}}\gamma\Bigr).
\end{align*}
\end{theorem}
...
\MW{We need to state a corollary that says how close the MLEs are to~$I$ (i.e., go from $\SSPD$ to $\PD$). Can someone add this?}
...
This also improves our result on strong convexity, which will be useful in the analysis of the flip-flop algorithm.
...
\end{corollary}
\begin{proof}
\MW{Missing. Can someone fill in?}
\end{proof}
...
\end{align*}
In particular, the condition above implies that $\Phi_Y$ is $\eps \sqrt{ \frac{d_1}{nd_2}}$-balanced.
\MW{I don't see why this follows (ignoring missing factors of 2). Do you assume that $\eps$ is small? Can you add this to the proof in the appendix?}
\end{lemma}
...
\begin{customthm}{\ref{thm:matrix-normal}}[Matrix normal model spectral error]
\MatrixSpec
\end{customthm}
\begin{proof}
...
\begin{proof}
	...
	%= \det(d_a \cdot \rho^{(a)})^{1/d_a} \cdot \Theta_a^{1/2} \cdot (d_a \cdot \rho^{(a)})^{-1} \cdot \Theta_a^{1/2}
...
%=============================================================================
The original theorem in \cite{P14} is in a slightly different language:

\begin{definition}
We denote $\Pi: \mat(m) \to \mat(m)$ as the orthogonal projection onto traceless matrices in $\mat(m)$.
\end{definition}

\begin{theorem}\label{thm:Pisier-expansion}
Let $\{Y_{i}\}_{i \in [N]}$ be i.i.d. random matrices in $\mat(n,m)$ with i.i.d. standard Gaussian entries. There are constants $c,C > 0$ such that for any $t \geq 2$ we have the following norm bound with failure probability $\leq t^{-c(m+n)}$:
\[ \left\| \left(\sum_{i=1}^{N} Y_{i} \otimes Y_{i }\right) \circ \Pi \right\|_{op} \leq C \sqrt{N} t^{2} \left( \E \|Y\|_{op} \right)^{2} \]
Here the $Y$ on the right hand side is another i.i.d. standard gaussian matrix in $\mat(n,m)$
\end{theorem}
...
Therefore the term we want to bound in \cref{thm:hess-pisier} is related as:
\[ \|\Phi_{Y}\|_{0} := \sup_{Tr[H] = 0} \frac{\|\sum_{i \in [N]} Y_{i} H Y_{i}^{*}\|_{F}}{\|H\|_{F}} = \sup_{H \in \mat(m,m)} \frac{\left\| \left( \sum_{i} Y_{i} \otimes Y_{i} \right) \circ \Pi (\text{vec}(H)) \right\|_{2}}{\|\text{vec}(H)\|_{2}}      \]
In the last equality we used $\|\Pi (\text{vec}(H))\|_{2} \leq \|\text{vec}(H)\|_{2}$ since $\Pi$ is an orthogonal projection. So \cref{thm:Pisier-expansion} reduces our problem to bounding the operator norm of a single random gaussian matrix $Y$, which is well understood:
...
Therefore \cref{thm:Pisier-expansion} implies the conclusion of \cref{thm:hess-pisier}:
\[ \|\Phi_{Y}\|_{0} \leq C \sqrt{N} t^{2} (\sqrt{m} + \sqrt{n})^{2} \leq C' \sqrt{N} t^{2} (m + n)     \]

% Need to bound $Tr[\Phi(I)]$ and then this simply gives the quantum expansion result $\|\Phi\|_{0} \leq \lambda \frac{Tr[\Phi(I)]}{\sqrt{mn}}$.

\MW{Please explain this in more detail (since the point of having the appendix is as a service to the non-expert reader :-).}
Our setting required the result on rectangular matrices with strong error bounds, so we follow the proof given in \cite{P14} with these minor modifications and claim no originality .The proof of \cref{thm:Pisier-expansion} proceeds by a symmetrization trick, followed by the trace method. We will first state the necessary concentration results and then give the proof.

\begin{theorem}[\cite{P86}]
Let $Y$ be a standard Gaussian in separable Banach space with norm $\|\cdot\|$. Then $\|Y\|$ concentrates  with parameter $\sigma^{2} := \sup \{ \E \langle Y, \xi \rangle^{2} \mid \|\xi\|_{*} \leq 1 \}$:
...
The operator we want to control ($\sum_{i} Y_{i} \otimes Y_{i} \circ \Pi$) has entries which are dependent in complicated ways. We first begin by a standard symmetrization trick to linearize. A single entry of $Y \otimes Y$ is either $g g'$ for two independent standard Gaussians, or $g^{2}$ for a single standard Gaussian. In expectation we have $\E g g' = 0, \E g^{2} = 1$, and so the expected matrix is
\[ \E \sum_{i} Y_{i} \otimes Y_{i} = \{ N \delta_{u = u'} \delta_{v = v'} \}_{u,u' \in [n], v,v' \in [m]}    \]
The above term can be rewritten as $N \text{vec}(I_{n}) \text{vec}(I_{m})^{*}$ and so after projection we have
\[ \E \sum_{i} Y_{i} \otimes Y_{i} \circ \Pi = N N \text{vec}(I_{n}) \text{vec}(I_{m})^{*} \circ \Pi = 0 \]
...
Therefore we can add a copy with new iid variables $\{Z_{i}\}$: using Jensen's inequality and the convexity of the operator norm, we have
\[ \E_{Y} \left\|\left(\sum_{i=1}^{N} Y_{i} \otimes Y_{i}\right) \circ \Pi\right\|_{op} \leq
\E_{Y,Z} \left\|\left(\sum_{i=1}^{N} Y_{i} \otimes Y_{i}-\sum_{i=1}^{N} Z_{i} \otimes Z_{i}\right) \circ \Pi \right\|_{op}  \] Now note $(Y,Z)$ has the same joint distribution as $(\frac{Y+Z}{\sqrt{2}},\frac{Y-Z}{\sqrt{2}})$, so the above equation is
\begin{align}\frac{1}{2}\E &\left\|\left(\sum_{i=1}^{N} (Y_{i}+Z_{i}) \otimes (Y_{i}+Z_{i})  - \sum_{i=1}^{N} (Y_{i}-Z_{i}) \otimes (Y_{i}-Z_{i})\right)\circ \Pi \right\|_{op}\nonumber \\
& = \E \left\|\left(\sum_{i} Y_{i} \otimes Z_{i}  + Z_{i} \otimes Y_{i} \right) \circ \Pi \right\|_{op}  \nonumber\\
& \leq 2 \E \left\|\sum_{i} Y_{i} \otimes Z_{i}\right\|_{op}.   \label{eq:yz}
\end{align}
Note we've lost the projection, but removed many of the dependencies. Next we use the trace method to bound the expectation of \cref{eq:yz}; that is, we approximate the operator norm by the Schatten $p$-norm for a high enough $p$ and control these Schatten norms using concentration of moments of Gaussians.

\begin{align*}
\E \left\|\sum_{i} Y_{i} \otimes Z_{i}\right\|_{2p}^{2p}
& = \E \tr \left[ \left( \sum_{ij} Y_{i}^{*} Y_{j} \otimes Z_{i}^{*} Z_{j} \right)^{p} \right]  \\
& = \sum_{\vec{i},\vec{j} \in [N]^{p}} \E \tr [ Y_{i_{1}}^{*} Y_{j_{1}} ... Y_{i_{p}}^{*} Y_{j_{p}} \otimes Z_{i_{1}}^{*} Z_{j_{1}} ... Z_{i_{p}}^{*} Z_{j_{p}}  ]   \\
& = \sum_{\vec{i},\vec{j}} (\E_{Y} \tr [ Y_{i_{1}}^{*} Y_{j_{1}} ... Y_{i_{p}}^{*} Y_{j_{p}} ] ) (\E_{Z} \tr [ Z_{i_{1}}^{*} Z_{j_{1}} ... Z_{i_{p}}^{*} Z_{j_{p}} ] )
\end{align*}
...
So we can upper bound each term individually by the nc-Holder inequality (\cref{thm:holder}) to find:
\[\E \|\sum_{i} Y_{i} \otimes Z_{i}\|_{2p}^{2p}
\leq \sum_{\vec{i},\vec{j}} (\E_{Y} \tr [ Y_{i_{1}}^{*} Y_{j_{1}} ... Y_{i_{p}}^{*} Y_{j_{p}} ] )
( \E_{Z} \|Z_{i_{1}}^{*}\|_{2p} ... \|Z_{j_{p}}\|_{2p} )  \]
...
\[ \E \|\sum_{i} Y_{i} \otimes Z_{i}\|_{2p}^{2p}
\leq \left( \E \|Z\|_{2p}^{2p} \right) \left( \sum_{\vec{i},\vec{j}}
(\E \tr [ Y_{i_{1}}^{*} Y_{j_{1}} ... Y_{i_{p}}^{*} Y_{j_{p}} ] ) \right) \]
\[ = \left( \E \|Z\|_{2p}^{2p} \right)
\left( \E \left\| \sum_{i} Y_{i} \right\|_{2p}^{2p} \right)
= \left( \E \|Z\|_{2p}^{2p} \right) N^{p} \left( \E \|Y\|_{2p}^{2p} \right)    \]
In the last step we used unitary invariance of the joint $\{Y_{i}\}$ distribution, i.e. that $\sum_{i} Y_{i}$ has the same distribution as $\sqrt{N} Y_{1}$.
Finally we can use $\|\cdot\|_{op}$ is bounded by $\|\cdot\|_{2p}$:
%Assume wlog (by homogeneity) that $\|\alpha\|_{2} = 1$:
\[ N^{-p} \E_{Y, Z} \|\sum_{i} Y_{i} \otimes Z_{i} \|_{op}^{2p} \leq N^{-p} \E_{Y, Z} \|\sum_{i} Y_{i} \otimes Z_{i} \|_{2p}^{2p} \]
\[ \leq ( \E \|Y\|_{2p}^{2p} )^{2} \leq ( m \E \|Y\|_{op}^{2p} )^{2} \leq m^{2} \left(\E \|Y\|_{op} + O(\sqrt{\frac{2p}{\sigma^{2}}})  \right)^{4p} \]
In the third inequality we used that $Y \in \mat(n,m)$ is rank $\leq m$ and therefore $\|Y\|_{2p} \leq m \|Y\|_{op}$. In the final step we used that $\|Y\|_{op}$ is sub-Gaussian with parameter $\sigma^{2}$. Note the dual norm of $\|\cdot\|_{op}$ is $\|\cdot\|_{1}$.
\[ \sigma^{2} = \sup_{\xi} \frac{ \E \langle Y, \xi \rangle^{2} }{\|\xi\|_{1}^{2}} = \E Y_{11}^{2} \sup_{\xi} \frac{ \|\xi\|_{F}^{2} }{\|\xi\|_{1}^{2} } = 1     \]
Here we used that $\langle Y, \xi \rangle$ is distributed the same as $\|\xi\|_{F} Y_{11}$ by unitary invariance.
So we can apply Markov's inequality to get the bound:
\[ \Pr[\|\sum_{i} Y_{i} \otimes Y_{i}\|_{op} \geq (2t \E \|Y\|_{op})^{2} ] \leq    \frac{m^{2} (\E\|Y\|_{op} + C \sqrt{p} )^{4p}}{(2t \E\|Y\|_{op})^{4p}}    \]
...
The statement follows since $t \geq 2$, so that the numerator $m^{2}$ is negligible (at the cost of a small constant in the exponent).
\end{proof}
