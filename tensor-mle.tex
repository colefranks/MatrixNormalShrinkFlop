...
\leq 4 d_a \delta^2.
\end{align*}
Our assumption~\eqref{eq:eps sqr assm} implies (again choosing $C$ large enough) that $4 d_a \delta^2 \leq \frac12$, so we obtain
\begin{align*}
...
Then:
\begin{equation*}
  \E \norm{A}_{\op} \geq \frac{1}{3} (\sqrt{n} + \sqrt{m}).
\end{equation*}
\end{lemma}
\begin{proof}
The moment definition of sub-Gaussianity shows $\E \|A\|^{2} \leq \E \|A\| + O\left(\sqrt{\frac{p}{\sigma^{2}}} \right)$ \MW{square root missing? also, already have to plug in $p=2$ here.}, so we continue
\[ (\E \norm{A}_{\op})^{2} \geq \E \norm{A}_{\op}^{2} - C \sqrt{2} ,   \]
where in the last step we substituted $p=2$ and $\sigma^{2} = 1$ by \cref{l:opNormSubG}.
\MW{say what $C$ is}
We can then lower bound the second moment simply assuming $n \geq m$:
\[ \E \norm{A}_{\op}^{2} = \E \norm{A A^{T}}_{\op} \geq \norm{\E A A^{T}}_{\op} = n ,  \]
...
Here the lower bounds follow since $C'$ is a universal constant \MW{what is $C'$? hasn't been defined yet} and $n \geq m$.
\MW{how does this relate to the $1/3$ mentioned in the statement of the lemma?}
\end{proof}
...
Now we choose $q = c (\sqrt{m} + \sqrt{n})^{2}$ for small enough $c$ such that $C \sqrt{q} \leq \E \|A\|_{\op}$ (by \cref{lem:op norm lower bound}).
\MW{say that $q\geq1$ for $n$ or $m$ large enough.}
