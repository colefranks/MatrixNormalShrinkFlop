...
Nonetheless, to our knowledge there exist no nonasymptotic bounds on convergence of the MLE, or any other tractable estimator, in the dense case \CF{actually, tsiligkaridis do this in frobenius norm}. Moreover, it has not been proven that the flip-flop algorithm converges \emph{quickly} to the MLE, or even that there is a polynomial time to compute the MLE or any other \CF{asymptotically consistent?} estimator. We make significant progress on both of these open questions. As an informal summary, we show the following:
\begin{enumerate}
...
\TODO{put guts here}.
\AR{This mostly depends on notation, so I'll leave this for now and you guys can decide what the right notation is. }
\end{proof}
...
\end{lemma}
\begin{proof}
Let $M^{a}, M^{ab}$ be the matrix versions of the bipartite operators:
\[ \langle vec(X), M_{v}^{a} (vec(X)) \rangle := \langle Q_{a}, X^{2} \rangle    \]
\[ \langle vec(X), M_{w}^{a} (vec(X)) \rangle := \langle Q_{a}, X^{2} \rangle    \]
