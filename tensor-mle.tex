...
%\item $X$ for the tensor random variable, $X_1, \dots, X_n$ for each , $\vec X = (X_1, \dots, X_n)$ for the random tuple of samples. $\rho = \vec X \vec X^T/\|\vec X\|_F^2$. Lower case $x$ for samples.

\item $X$ for the random tuple $(X_1, \dots, X_n)$, $x$ for the tuple of samples $(x_1, \dots, x_n)$ when no longer random. Think of $X$ as $D \times n$ matrix, $\rho = XX^T,xx^T$ etc.
...
\item $f_{\vec X}$ for the function in \cref{dfn:function}, mostly drop $\vec X$. $\langle \cdot, \cdot \rangle$ is the $\ell_2$ inner product of vectors
...
There is $c> 0$ such that the following holds: with probability at least $1 - k^{2} n^{c 2 (d_1 + d_2)}$, the function $f_{\vec X}$ is $c$-strongly convex at all points $Z\in \PD$ with $\sum_{i = 1}^n \|\log Z_a\|_{op} < c/k.$ 
\end{theorem}
...
For input $\vec X \in \CF{where?}$ such that $d_{a}\|\rho_{X}^{(a)}\|_{op} \leq 1+\frac{1}{20}$; and perturbation $\delta := \sum_{a} (\delta_{a})_{a}$ with $\|\delta\|_{op} = \sum_{a} \|\delta_{a}\|_{op} \leq \frac{1}{20}$ giving $\{X'_{i} := e^{\delta} X_{i} \}$:
...
\end{definition}
\CF{I think we should change $X$ to $\vec X$ at many places in here. Alternately we can just start using $X$ to mean the entire tuple, but that conficts with it as a random variable, so idk} 
\begin{definition}
...
\CF{add quantifiers}
For $\vec X \in \CF{somewhere}$ and perturbation $\delta \in \Sym$ with $\|\delta\|_{op} \leq \frac{1}{20}$, if $X' = (e^{\delta})_c X$ for $c \in \{a,b\}$ we have 
