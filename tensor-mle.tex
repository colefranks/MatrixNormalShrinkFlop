...
If $\omega = \sum_{i=1}^N \operatorname{vec}(A_i) \operatorname{vec}(A_i)^T$ for $d_2\times d_1$ matrices~$A_1,\dots,A_N$, we have the following two equivalent ways to write the corresponding completely positive map, which we denote by $\Phi_A$:
\begin{align}\label{eq:kraus}
  \Phi_A(K) = \sum_{i=1}^N A_i K A_i^T
\quad\text{or}\quad
  \operatorname{vec}(\Phi_A(K)) = \sum_{i=1}^N (A_i \ot A_i) \operatorname{vec}(K).
\end{align}
...
\MW{There, typically $\Phi$ is trace preserving (a quantum channel) and $d_1=d_2$, so the condition is just $\norm{\Phi}_0 \leq \lambda$.}\CF{doesn't it also have to be unital?}
...
\MW{Should explain in the appendix how exactly this follows from what is stated there.}
\MW{What does the $\Omega$ depend on? Universal constant?}\CF{yes, isn't that the usual assumption?}
\MW{And how did the constant $C>0$ in the appendix vanish?} \CF{Fixed}
\MW{Why is the RHS now $\sqrt{d_1} + \sqrt{d_2}$?} \CF{probably a mistake!}
\MW{I think there was some normalization mismatch, please check.}\CF{I agree with what you have now}
...
\begin{align*}
  \norm \Phi_A \leq Ct^{2} \sqrt N \left( d_1 + d_2 \right).
\end{align*}
\end{theorem}
