...
The above lemma combined with \cref{prp:relative-lower} immediately implies \cref{thm:matrix-lower}.
\begin{proof}[Proof of \cref{thm:matrix-lower}] We begin with the proofs of \cref{it:frob-lower,it:op-lower}, as they imply the rate lower bounds. If $nd_1 \geq d_1$, then the bound follows immediately from \CF{finish}

To show \cref{it:frob-lower}, let $\delta \leq c $ be small enough that the estimator $\tilde{\Theta}$

we observe that $\tilde{\Theta}$
\end{proof}
