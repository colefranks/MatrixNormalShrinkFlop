...
\begin{fact} $\forall \|\delta\|_{op} \leq \frac{1}{2}: \|e^{\delta} - I\|_{op} \leq 2 \|\delta\|_{op}$   \end{fact}

We will follow the structure presented in the last proof by letting our input at the identity be represented $\{X_{i}\}$ and showing each block of the Hessian only changes a small amount under perturbation $\{X'_{i} := e^{\delta} X_{i}\}$. In particular we will gives bounds on each block under each component-wise perturbation $\{X'_{i} := (e^{\delta})_{a} X_{i}\}$. 

\begin{lemma} \label{atoaaRobustness}
For input $\{X_{i}\}$ and perturbation $\|\delta\|_{op} \ll 1$ giving $\{X'_{i} := (e^{\delta})_{a} X_{i} \}$, there is a universal constant $C$:
...
Letting $\delta' := e^{\delta} - I_{d_{a}}$, we can bound $\|(e^{\delta})_{a} X\|_{2} \in \|X\|_{2}(1 \pm \|\delta'\|_{op})$ so let $\|(e^{\delta})_{a} X\|_{2}^{-2} = \|X\|_{2}^{-2} (1 + \eta)$; note $|\eta| \leq O(\|\delta\|_{op})$ for small enough perturbation. Assume for the sequel that $\|Z\|_{1} = 1$:
\[ | \langle Z_{a}, (1+\eta) (I+\delta') \rho_{X} (I+\delta') - \rho_{X} \rangle | \]
\[ \leq (2\|\delta'\|_{op} + |\eta| + O(\|\delta\|_{op}^{2}) \|\rho^{(a)}\|_{op} \|Z\|_{1}    \]
\[ \leq O(\|\delta\|_{op}) \|\rho^{(a)}\|_{op}    \]
Here we used that $\|\delta\|_{op} \ll 1$.
\end{proof}

\begin{lemma} \label{btoaaRobustness}
For input $\{X_{i}\}$ and perturbation $\|\delta\|_{op} \ll 1$ giving $\{X'_{i} := (e^{\delta})_{b} X_{i} \}$, there is a universal constant $C$ (different from above):
\[ \|\rho_{X'}^{a} - \rho_{X}^{a}\|_{op} \leq C \|\delta\|_{op} \|\rho_{X}^{a}\|_{op}      \]
\end{lemma}
\begin{proof} 
Let $\delta' := (1+\eta)e^{2\delta} - I$ and $\eta$ same as above. We assume for now $Z \succeq 0$. 
\[ | \langle Z_{a}, (1+\eta) (e^{\delta})_{b} \rho_{X} (e^{\delta})_{b}^{*} - \rho_{X} \rangle| \]
...
The lemma follows by noting $\|Z\|_{1} = \|Z_{+}\|_{1} + \|Z_{-}\|_{1}$ as well as 
\[ \|\delta'\|_{op} = \|(1+\eta) e^{2 \delta} - I\|_{op} \leq |\eta|(1 + 2 \|\delta\|_{op}) + 2\|\delta\|_{op} \leq O(\|\delta\|_{op})     \]
\end{proof}

This gives a bound on the diagonal blocks of the Hessian as well as the "rank-one" term in all other blocks.

\begin{corollary} \label{diagRobustness}
For input $\{X_{i}\}$ and perturbation $\sum_{a} \|\delta_{a}\|_{op} \ll 1$ small enough:
\[ \frac{1}{d_{a}} \|\nabla^{2}_{aa} f(e^{\delta}) - \nabla^{2}_{aa} f(I)\|_{op} \leq O_{k}(\|\delta\|_{op}) \|\rho_{X}^{(a)}\|_{op}      \]
\end{corollary}
\begin{proof}
Recall $\frac{1}{d_{a}}\langle Y_{a}, \nabla^{2}_{aa} f_{X} Y_{a} \rangle = \langle \rho_{X}^{(a)}, Y^{2} \rangle - \langle \rho_{X}^{(a)}, Y \rangle^{2}$. We break our perturbation into $k$ terms and let $X =: X_{0}, e^{\delta_{1}} X_{0} =: X_{1}, ..., e^{\delta_{k}} X_{k-1} =: X_{k} = X'$.  We use $\ref{atoaaRobustness}$ for the $a$ part and $\ref{btoaaRobustness}$ for the rest:
\[ |\langle \rho_{X'}^{a} - \rho_{X}^{a}, Y^{2} \rangle| \leq \sum_{j=1}^{k} |\langle \rho_{X_{j}}^{(a)} - \rho_{X_{j-1}}^{(a)}, Y^{2} \rangle| \leq \sum_{j=1}^{k} (1+ C\|\delta_{j}\|_{op}) \|\rho_{X_{j-1}}^{(a)}\|_{op} \|Y^{2}\|_{1} \]
\[ < k \prod_{j} (1+C\|\delta_{j}\|_{op}) \|\rho_{X}^{(a)}\|_{op} \|Y\|_{F}^{2} \leq C'_{k} \|\delta\|_{op} \|\rho_{X}^{(a)}\|_{op} \|Y\|_{F}^{2}    \]
This also gives a way to bound the "rank-one" term:
...
\[ \leq C'_{k} \|\delta\|_{op} (2+C'_{k} \|\delta\|_{op}) (\|\rho_{X}^{(a)}\|_{op} \|Y\|_{1})^{2} \leq (d_{a} \|\rho_{X}^{(a)}\|_{op} C''_{k} \|\delta\|_{op}) \|\rho_{X}^{(a)}\|_{op} \|Y\|_{F}^{2}       \]
So assuming $d_{a} \|\rho_{X}^{(a)}\|_{op} \leq O(1)$, this is some some fixed function of $k$. 

In toto this gives the corollary:
\[ \frac{1}{d_{a}}\|\nabla^{2}_{aa} f(e^{\delta}) - \nabla^{2}_{aa} f(I)\|_{op} \leq (C'_{k} + C''_{k}) \|\delta\|_{op} \|\rho_{X}^{(a)}\|_{op}      \]
\end{proof}
...
$\|H_{X}^{ab} f(I)\|_{F \to F}^{2} \leq \|\rho_{X}^{(a)}\|_{op} \|\rho_{X}^{(b)}\|_{op}$
\end{lemma}
\begin{proof}
...
\end{lemma}
\begin{proof}
We will drop the superscript $(ab)$ since it doesn't change. Let $R : \mat(d_{b}) \to \mat(d_{b})$ be defined as $R(Z) := e^{\delta} Z e^{\delta}$. The subspace $\Sym_{d_{b}}^{0}$ is not invariant under $R$, but we show $R \approx I$. Let $\delta' := e^{\delta} - I$. 
\[ R(Z) - Z = \delta' Z + Z \delta' + \delta' Z \delta'    \]
\[ \implies \|R(Z) - Z\|_{F} \leq O(\|\delta'\|_{op}) \|Z\|_{F} \leq O(\|\delta\|_{op}) \|Z\|_{F}     \]
...
For input $\{X_{i}\}$ and perturbation $\|\delta\|_{op} \ll 1$ giving $\{X'_{i} := (e^{\delta})_{c} X_{i} \}$ for $c \not\in \{a,b\}$, there is a universal constant $C$ (different from above):
\[ \|H_{X'}^{ab} - H_{X}^{ab}\|_{0} \leq C \|\delta\|_{op} \|H_{X}^{ab}\|_{F \to F}    \]
\end{lemma}
\begin{proof}
Let $\delta' := e^{2 \delta} - I_{c}$
\[ \langle Y, (H_{X'} - H_{X}) Z \rangle = \langle = \langle \rho_{X}^{(abc)}, Y \otimes Z \otimes \delta' \rangle   \]
...
Here we again used that $\rho_{X}^{(abc)} \succeq 0$. This last term cannot be bounded by $\|H_{X}\|_{0}$ as $Y \succeq 0$, but $\|H_{X}\|_{F \to F}$ works for our purposes. 
...
For input $\{X_{i}\}$ and perturbation $\sum_{a} \|\delta_{a}\|_{op} \ll 1$ small enough:
\[ \frac{1}{\sqrt{d_{a} d_{b}}} \|\nabla^{2}_{ab} f(e^{\delta}) - \nabla^{2}_{ab} f(I)\|_{op} \leq O_{k}(\|\delta\|_{op}) \|\rho_{X}^{(a)}\|_{op}      \]
\end{corollary}
\begin{proof}
Recall $\frac{1}{\sqrt{d_{a} d_{b}}} \langle Y_{a}, \nabla^{2}_{aa} f_{X} Z_{b} \rangle = \langle \rho_{X}^{(ab)}, Y^{2} \rangle - \langle \rho_{X}^{(a)}, Y \rangle^{2}$. Using the same iterative strategy as $\ref{diagRobustness}$ we can show: 
\[ |\langle Y, (\rho_{X'}^{(ab)} - \rho_{X}^{(ab)}) Z \rangle| \leq C'_{k} \|\delta\|_{op} \|H_{X}\|_{F \to F} \|Y\|_{F} \|Z\|_{F}    \]
...
\[ 2 C'_{k} \|\delta\|_{op} (1+C'_{k} \|\delta\|_{op}) \|\rho_{X}^{(a)}\|_{op} \|\rho_{X}^{(b)}\|_{op} \|Y\|_{1} \|Z\|_{1}    \]
\[ \leq C''_{k} \|\delta\|_{op} (\sqrt{d_{a} d_{b}} \|\rho_{X}^{(a)}\|_{op} \|\rho_{X}^{(b)}\|_{op}) \|Y\|_{F} \|Z\|_{F}    \]
Again assuming our one-body marginals are constantly balanced, this is a fixed function of $k$. 

In toto this gives the lemma:
\[ \frac{1}{\sqrt{d_{a} d_{b}}} \|\nabla^{2}_{ab} f(e^{\delta}) - \nabla^{2}_{ab} f(I)\|_{op} \leq \|\delta\|_{op}(C'_{k} \|H_{X}\|_{F \to F}) + C''_{k} (\sqrt{d_{a} d_{b}} \|\rho_{X}^{(a)}\|_{op} \|\rho_{X}^{(b)}\|_{op})) \]
\[ \leq \|\delta\|_{op} (C'_{k} + C''_{k}) \sqrt{\|\rho_{X}^{(a)}\|_{op} \|\rho_{X}^{(b)}\|_{op}}     \]
We used $\ref{inftyto2}$ for the first term and balanced marginals for the second. 
\end{proof}

\begin{lemma} \label{convexRobustness}
If $f_{X}$ is $\alpha$-strongly convex at $I$ and $\sum_{a} \|\delta_{a}\|_{op} \ll 1/k$, then $f$ is $\geq (\alpha - O_{k}( \|\delta\|_{op}))$-strongly convex at $e^{\delta} := \otimes_{a} e^{\delta_{a}}$. 
\end{lemma}
\begin{proof}
We follow the same structure as the proof of convexity, letting $\{X'_{i} := e^{\delta} X\}$. Then by $\ref{diagRobustness}$ we have a bound on the diagonal blocks, and by $\ref{offdiagRobustness}$ we have a bound on the off-diagonal blocks:
\[ \|\nabla^2 f_{X'} - \nabla^{2} f_{X}\|_{op} \leq \|\delta\|_{op} (k C_{k} + k(k-1) C_{k}) \leq O_{k}( \|\delta\|_{op} )  \]
\end{proof}

\AR{I can fix the $k$-dependency, but I think each $C_{k}$ is $O(k)$ and therefore this one is $O(k^{3})$. Also I'm missing dependence on $\mu$, will look into that later.}
...
\begin{theorem}
Let $\Pi: \mat(m) \to \mat(m)$ denote the projection onto the span of $I_m$, and $\{Y_{i}\}$ are iid standard gaussians in $\mat(n,m)$ \CF{ I changed it to $\mat$ because this needs to go $\mat(m) \to \mat(n)$ if I understand correctly}. There is a constant $c > 0$ such that for all $m \leq n$ we have 
\[ \|\sum_{i=1}^{N} \alpha_{i} Y_{i} \otimes Y_{i} (I - \Pi) \|_{op} \leq O \left( \sqrt{\sum_{i} \alpha_{i}^{2}} \right) \left( \E \|Y\|_{op} \right)^{2} \]
with probability at least $ 1- (mn)^{-c}$. \AR{Should we put the best possible}
\end{theorem}
\begin{proof}
...
In the last step we used unitary invariance of the joint $\{Y_{i}\}$ distribution, i.e. that $\sum_{i} c_{i} Y_{i}$ has the same distribution as $\|c\|_{2} Y$ where $Y$ is a standard Gaussian on $\mat(n,m)$.
...
So again assuming $\|\alpha\|_{2} = 1$ and choosing $p = 2c \log mn$, where we choose $c$ at the end, we can apply Markov's to get the bound: \CF{define probabilty macro}
\[ \Pr[\|\sum_{i} \alpha_{i} Y_{i} \otimes Y_{i}\|_{op} \geq e (\E \|Y\|_{op})^{2} ] \leq    \frac{m^{2} (\E\|Y\|_{op} + C \sqrt{p} )^{4p}}{e^{2p} (\E\|Y\|_{op})^{4p}}    \]
Now we choose $c \ll \frac{(\sqrt{m} + \sqrt{n})^{2/3}}{\log mn}$ so that $p \leq p^{3/2} \leq o(\E \|Y\|_{op})$ and we bound: 
\[ (\E \|Y\|_{op} + C \sqrt{p})^{4p} = (\E \|Y\|_{op})^{4p} \left(1 + \frac{C \sqrt{p}}{\E \|Y\|_{op}} \right)^{4p} < (\E \|Y\|_{op})^{4p} \exp\left( \frac{4 C p^{3/2}}{\E \|Y\|_{op}} \right)  \]
This allows us to cancel the $\E \|Y\|_{op}$ and bound the probability:
\[ \leq \frac{m^{2} \exp \left(\frac{C' p^{3/2}}{\E \|Y\|_{op}} \right) }{e^{2p}} < \frac{1}{(mn)^{c}}  \]
\end{proof}
...
By the above theorem, the operator norm on the subspace $\perp$ to $(I_{a},I_{b})$ is of the order $\frac{d_{a}+d_{b}}{\sqrt{d_{a} d_{b} TD}}$ whp, so for constant expansion it suffices that $TD \gg \max_{a} d_{a}^{2}$
\end{corollary}
