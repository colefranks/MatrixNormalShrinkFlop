...
$$f(A^\dagger e^{t Z} A)$$ is convex in $t$ for all $Z \in H$ and $A\in G$. Say $f$ is $\lambda$-geodesically \emph{strongly convex} if $\partial^2_t f(A^\dagger e^{t Z} A) \geq \lambda \|Z\|_F$. 
\end{definition} 
...
\begin{lemma} If $f$ is geodesically convex everywhere, and $\lambda$-geodesically strongly convex on a ball of radius $\kappa$ about $I$ and the geodesic gradient $\nabla f(I)$ satisfies $\|\nabla f(I)\|_F = \eps < \lambda \kappa$, then there is an optimizer $e^{Z^{*}}$ such that $\|Z^{*}\|_F \leq \eps/\lambda$. 
\end{lemma}
\begin{proof}
Take geodesic $e^{t Z}$ towards the optimum with speed $\|Z\|_{F} = 1$ so the optimizer is $e^{T Z}$; and let $g(t) := f(e^{tZ})$. 
\[ g'(0) = \langle \nabla f(I), Z \rangle \geq - \|\nabla f(I)\|_{F} \|Z\|_{F} \geq - \eps   \]
\[ 0 = g'(T) = \int_{t=0}^{T} g''(t) dt + g'(0) \geq \lambda \min\{\kappa,T\} - \eps     \]
Now if $T \geq \kappa$ we have a contradiction as the final expression is $> 0$. So it must be the case that the optimizer is within a $\eps/\lambda$ geodesic ball. 
\end{proof}
Next, we state the two theorems that allow us to apply this lemma. We prove them in the next two sections.

\begin{lemma}\label{lemma:gradient-bound} Suppose the number of samples is at least $\max_{a} d_{a}^{2} / D \epsilon^{2} $. Then with failure probability $\leq k \exp(-\Omega(\max_{a} d_{a}))$, 
$$\forall a: \| d_{a} (\nabla f(I))_{a} \|_{op} \leq \epsilon $$
as a consequence, $ \sum_{a} d_{a} \|(\nabla f(I))_{a} \|_{F}^{2} \leq k \epsilon^{2}$. 
\end{lemma}

\begin{theorem}\label{thm:tensor-convexity}
Suppose the number of samples is at least $k^{2} \max_{a} d_{a}^{2}/D$. Then with failure probability $\leq 1/poly(D)$, $f_X$ is $\Omega(1)$-strongly convex. 
\end{theorem}
