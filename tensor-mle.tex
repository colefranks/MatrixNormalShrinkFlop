...
\MW{Cole, please have a glance to see if all the squares and $d_{\min}$ vs $d_{\max}$ look right to you, they changed slightly.}
...
Let $x = (x_1,\dots,x_n)$ consist of independent standard Gaussian random variables in~$\R^D$, where $n \geq C k \frac{d_{\max}^2}D$.
Then, with probability at least $1 - k^2 ( \frac{k d_{\max}}{\sqrt{nD}} )^{\Omega(d_{\min})}$,
\begin{align*}
...
% \end{align*}
with failure probability at most~$\smash{( \frac{k d_{\max}}{\sqrt{nD}})^{\Omega(d_{\min})}}$.
% \begin{align*}
...
\begin{align*}
  e^{-\Omega(nD)}
+ k \smash{e^{-\Omega(\frac{nD}{k d_{\max}})}}
+ k^2 \left( \frac{k d_{\max}}{\sqrt{nD}} \right)^{\Omega(d_{\min})}.
\end{align*}
\MW{MARKER}
Due to our assumption on~$n$, the final term dominates and so we obtain the desired bound the failure probability.
\end{proof}
...
\MW{Say that we now proved \cref{thm:ball-convexity}.}
