...
\begin{fact} For all Hermitian $d\times d$ matrices $\delta $ such that $ \|\delta\|_{op} \leq \frac{1}{2}$, we have $$ \|e^{\delta} - I\|_{op} \leq 2 \|\delta\|_{op}.$$   \end{fact}
...
\[ \|\rho_{X'}^{a} - \rho_{X}^{a}\|_{op} =O( \|\delta\|_{op} \|\rho_{X}^{a}\|_{op}  )    \]
\end{lemma}
\begin{proof} By definition, $\|\rho_{X'}^{(a)} - \rho_{X}^{(a)}\|_{op} = \sup_{\|Z\|_{1} \leq 1} \langle Z_{a}, \rho_{X'} - \rho_{X} \rangle $. 

Letting $\delta' := e^{\delta} - I_{d_{a}}$, we can bound $\|(e^{\delta})_{a} X\|_{2} \in \|X\|_{2}(1 \pm \|\delta'\|_{op})$ \CF{what is $X$ and is $2$-norm Euclidean here?} so let $\|(e^{\delta})_{a} X\|_{2}^{-2} = \|X\|_{2}^{-2} (1 + \eta)$; note that $|\eta| = O(\|\delta\|_{op})$ provided $c$ is small enough. Assuming without loss of generality that $\|Z\|_{1} = 1$, we have \CF{added $a$ subscript on $(I + \delta');$ please check}
\[ | \langle Z_{a}, (1+\eta) (I+\delta')_a \rho_{X} (I+\delta')_a - \rho_{X} \rangle | \]
\[ \leq (2\|\delta'\|_{op} + |\eta| + O(\|\delta\|_{op}^{2}) \|\rho^{(a)}\|_{op} \|Z\|_{1}    \]
\[ = O(\|\delta\|_{op}) \|\rho^{(a)}\|_{op}    \]
Here we used that $\|\delta\|_{op} \leq c$.
\end{proof}
...
\begin{proof}
Recall from \cref{lem:hessian} that $\frac{1}{d_{a}}\langle Y_{a}, \nabla^{2}_{aa} f_{X} Y_{a} \rangle = \langle \rho_{X}^{(a)}, Y^{2} \rangle - \langle \rho_{X}^{(a)}, Y \rangle^{2}$. We treat the perturbation as the composition of $k$ perturbations; let $X_{0}:=X, X_{1}:= (e^{\delta_{1}})_1 X_{0}, ..., $ and $X_{k}:=(e^{\delta_{k}})_1 X_{k-1} = X'$. We first handle the term $\langle \rho_{X}^{(a)}, Y^{2} \rangle$. We use $\ref{atoaaRobustness}$ to handle $(e^{\delta})_a$ and $\ref{btoaaRobustness}$ for the rest:
\begin{align*}
 |\langle \rho_{X'}^{a} - \rho_{X}^{a}, Y^{2} \rangle| 
 &\leq \sum_{j=1}^{k} |\langle \rho_{X_{j}}^{(a)} - \rho_{X_{j-1}}^{(a)}, Y^{2} \rangle| 
\underset{\ref{atoaaRobustness},\;\ref{btoaaRobustness}}{\leq} \sum_{j=1}^{k} (1+ C\|\delta_{j}\|_{op}) \|\rho_{X_{j-1}}^{(a)}\|_{op} \|Y^{2}\|_{1} \\
& < k \prod_{j=1}^k (1+C\|\delta_{j}\|_{op}) \|\rho_{X}^{(a)}\|_{op} \|Y\|_{F}^{2} \leq C'_{k} \|\delta\|_{op} \|\rho_{X}^{(a)}\|_{op} \|Y\|_{F}^{2}.   \end{align*}
We next bound the rank-one term $\langle \rho_{X}^{(a)}, Y \rangle^{2}$:
