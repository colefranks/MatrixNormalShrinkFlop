...
\begin{theorem}[\CF{KLR}]\label{thm:klr} There is a constant $c > 0$ such that the following holds for all $d_1 \leq d_2$. Let $\samp$ be a tuple of vectors in $\R^{d_1} \ot \R^{d_2}$ with $\|x\| = 1$.
...
$$\left\| \frac{\|x\|_F}{\sqrt{nD}}\;\widehat{\Theta}_1(\samp) - I_{d_1}\right\|_{op}, \left\|\frac{\|x\|_F}{\sqrt{nD}}\;\widehat{\Theta}_2(\samp) - I_{d_2}\right\|_{op} \leq \frac{\eps \log d_1}{\lambda}.$$
\end{theorem}
By \cref{prp:xnorm}, $|\frac{\|X\|_F}{\sqrt{nD}} - 1 | \leq \eps$ with probability $e^{- \Omega(\eps^2 nD)}$, so this additional factor will be negligible in our case. \cref{thm:klr}, along with what we've shown so far, already implies a preliminary version of \cref{thm:matrix-normal}. In the proof of \cref{thm:tensor-convexity}, we've already shown that $\Phi$ is an $(\eps \sqrt{d_2/n d_1}, 1 - c)$-quantum expander with failure probability 
...
$Y_i = \frac{1}{\sqrt{d_i}} X_i \Phi_X^*(I_{d_1})^{-1/2} = \frac{1}{\sqrt{d_i}}  X_i (\sum_{i = 1}^n X_i^\dagger X_i)^{-1/2}.$ 
...
\item \label{it:norm} $|1 - \frac{\|X\|_F}{\sqrt{nD}}| \leq \frac{\eps_0}{\sqrt{d_2}}. $ 
\end{enumerate}
...
We now use a lemma relating the quantum expansion of $\tilde{\Phi}$ and $\Phi_\samp$; this is analogous to \cref{convexRobustness} guaranteeing the strong convexity of $f$ in a neighborhood of the origin. The operator $\tilde{\Phi}$ is a \emph{scaling} of $\Phi_\samp$, that is for some ``scalings'' $g \in \GL(d_1), h \in \GL(d_2)$ for all $Z$ we have $\tilde{\Phi}(Z) = g^\dagger \Phi_\samp( h Z h^\dagger) g$. In our case we may take $h = \frac{\|x\|_F}{\sqrt{d_2}}\Phi_\samp^*(I_{d_2})^{-1/2}$ and $g = \frac{1}{\|x\|_F} I_{d_1}$. By Lemma 4.4 in \cite{FM20}, if $\| h^\dagger h - I_{d_1}\|_{op} \leq \delta$,  then $\tilde{\Phi}$ is an $(\eps \sqrt{d_2/n d_1} + O(\delta), 1 - \lambda + O(\delta))$-quantum expander. Fortunately, $h^\dagger h$ is bounded by the balancedness of $\Phi_\samp$: $h^\dagger h - I = \frac{\|x\|_F^2}{d_2} ( \Phi^*(I_{d_1}))^{-1} - I_{d_2}$. By the balancedness of $\Phi_\samp$ we already know that 
$$\|\frac{d_2}{\|x\|_F^2}\Phi_\samp^*(I_{d_1}) - I_{d_2}\|_{op} \leq \eps_0 \sqrt{d_2/n d_1};$$
by \cref{prp:xnorm}, $\|x\|_F^2$ 
...
By our choice of $n$, we have $\eps \sqrt{d_1/nd_2} \leq c\lambda^2\log d_1$ if we take $\eps_0 = \eps/\log(d_1)$ for $\eps \leq c\lambda^2$. By \cref{thm:klr}, the MLE $\Theta'_1(y), \Theta'_2(y)$ for the tuple $y_i =  x_i \Phi^*(I_{d_1})^{-1/2}/\sqrt{d_1}$ satisfies 
