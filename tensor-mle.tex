...
Above we were trying to bound competing forces on the largest eigenvalue of $\mu$ for all time to make sure our scaling remains inside a small $\|\cdot\|_{op}$ ball. Here we will show it is enough to have small forces initially. 
...
\begin{theorem}
If we have the following condition initially, then we have strong convergence. 
$\forall a,b: \sup_{u \in \R^{a}} d_{a} d_{b} \langle \rho^{ab}, u u^{*} \otimes \mu_{b} \rangle \ll (k \log D)^{-c} $
\end{theorem}
