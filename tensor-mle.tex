...
In particular, $f_{\vec X}$ is $\Omega(1)$ at all points $Z$ with 



strongly convex with probability $\CF{good}$ 

\end{theorem}
and $\sum_{a} \|\delta_{a}\|_{op} \ll 1/k$, then $f$ is $\geq (\alpha - O_{k}( \|\delta\|_{op}))$-strongly convex at $e^{\delta} := \otimes_{a} e^{\delta_{a}}$. \\
...
with probability at least $ 1- \exp(-c(m+n))$. \AR{Should we put the best possible}
\end{theorem}
\begin{proof}
...
\[ \Pr[\|\sum_{i} \alpha_{i} Y_{i} \otimes Y_{i}\|_{op} \geq (2e \E \|Y\|_{op})^{2} ] \leq    \frac{m^{2} (\E\|Y\|_{op} + C \sqrt{p} )^{4p}}{(2e \E\|Y\|_{op})^{4p}}    \]
Now we choose $C\sqrt{p} \leq \E \|Y\|_{op} = \sqrt{m} + \sqrt{n}$, i.e. $p = \left( \frac{\sqrt{m} + \sqrt{n}}{C} \right)^{2}$: 
\[ \leq \frac{m^{2} (2\E \|Y\|_{op})^{4p}}{(2e \E \|Y\|_{op})^{4p}} \leq \frac{m^{2}}{\exp(\frac{4}{C^{2}} (\sqrt{m} + \sqrt{n})^{2})}    \]
