...
\MW{Added:} By replacing $r$ with any smaller~$r'>\frac{2\eps}\lambda$, we see that the sublevel set is in fact contained in the ball of radius~$\frac{2\eps}\lambda$.
...
When the samples $x=(x_1,\dots,x_n)$ are independent standard Gaussians, then $\rho^{(a)}$ is distributed as $\frac1{nD} Y Y^T$, where~$Y$ is a random $d_a \times N_a$ matrix with independent standard Gaussian entries, where~$N_a = \frac{nD}{d_a}$.
...
\begin{align*}
  \nabla^2_{a0} f_x = \nabla^2_{0a} f_x &= \sqrt{d_a} \left( \rho^{(a)} - \frac{\tr \rho}{d_a} I_{d_a} \right) \\
  \nabla^2_{00} f_x &= \tr \rho.
\end{align*}
for all $a \in [k]$.
\MW{Caution the reader about the $a0$ components!}
\end{lemma}
\begin{proof}
...
If $\omega = \sum_{i=1}^N \operatorname{vec}(A_i) \operatorname{vec}(A_i)^T$ for $d_2\times d_1$ matrices~$A_1,\dots,A_N$, we have the following two equivalent ways to write the corresponding completely positive map:
\begin{align*}
  \Phi(K) = \sum_{i=1}^N A_i K A_i^T
\quad\text{or}\quad
  \operatorname{vec}(\Phi(K)) = \sum_{i=1}^N (A_i \ot A_i) \operatorname{vec}(K).
\end{align*}
Conversely, any linear map $\Phi$ of this form is completely positive.
The matrices $A_1,\dots,A_N$ are known as Kraus operators of~$\Phi$.
We denote by $\Phi^*\colon\mat(d_2)\to\mat(d_1)$ the adjoint of $\Phi$ with respect to the Hilbert-Schmidt inner product; it has $A_1^T,\dots,A_N^T$ as Kraus operators.
...
\leq \frac{1-\lambda}{\sqrt{d_1d_2}} \tr \Phi(I_{d_1})
\end{align*}
...
\begin{align*}
  \norm*{\frac{\nabla^2 f_x}{\tr \rho} - I}
\leq \bigl(k-1\bigr) \lambda + \bigl(\sqrt k + 1\bigr) \eps.
\end{align*}
\end{lemma}

The proof of \cref{lem:expansion-convexity} uses a basic lemma that gives upper and lower bounds on a block matrix in terms of block diagonal matrices and the Loewner order.

\begin{lemma}\label{lem:block-matrix}
Let $M$ be a square symmetric block matrix with blocks $M_{ij}$ of size~$d_i \times d_j$.
Then,
\begin{align*}
  \bigoplus_i \left(M_{ii} - I_{d_i} \cdot \sum_{j \neq i} \norm{M_{ij}}_{\op} \right)
\preceq M
\preceq \bigoplus_i \left(M_{ii} + I_{d_i} \cdot \sum_{j \neq i} \norm{M_{ij}}_{\op} \right)
\end{align*}
\end{lemma}
\begin{proof}
...
\begin{align*}
  ...
  M_{11} + I_{d_1} \cdot \norm{M_{12}}_{\op} & 0 & \quad M_{13} & \cdots \\
  0 & M_{22} + I_{d_2} \cdot \norm{M_{12}}_{\op} & \quad M_{23} & \\
  M_{31} & M_{23} & \quad M_{33} &\\
  \vdots & & & \;\;\;\;\ddots
  \end{bmatrix}.
\end{align*}
By sequentially apply this inequality to all other $2\times 2$ principal block submatrices until we obtain the desired upper bound.
