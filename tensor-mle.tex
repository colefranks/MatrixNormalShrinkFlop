...
The proof strategy of this section is different from the tensor normal model case, as now, the number of samples is not large enough to guarantee that the initial conditions from \cref{lem:fast-convergence-initial-generic} will apply with high probability.

Thus, we will first have to obtain a scaling... \RMO{write more later}
...
There exist absolute constants $\Gamma, \gamma > 0$ and $\lambda \in (0,1)$ such that the following holds.
	...
	with probability at least $TODO$ we have that the following conditions hold:
	\begin{enumerate}
		\item $|\nabla_0 f_x(I)| \leq \frac{\gamma}{100}$
...
\begin{proof}
Condition 1 follows from \cref{prop:gradient-bound} with parameters...
