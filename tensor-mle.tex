...
\CF{temporary}
\begin{itemize}
\item Number of samples $n$, dimensions $d_1, \dots, d_n$. Whatever works for product of these, etc. 
...
\item $\PD_k$ for $k\times k$ pd's, $\Herm_k$ for $k\times k$ Hermitians. I don't think stats folks use this, I just really like them. Other suggestions welcome.
...
\end{align}
\end{definition}
\begin{theorem}[Matrix normal model spectral error]\label{thm:matrix-normal} The MLE for $\Sigma_1, \Sigma_2$ from $n$ independent samples satisfies 
$$ d_{op}(\hat{\Sigma}, \Sigma) = O(\TODO{}). $$
with probability $\TODO{}$.
\CF{presumably there will be some dimension-dependent factors}.
\end{theorem}

\begin{theorem}[Tensor normal Frobenius error]\label{thm:tensor-frobenius} The MLE for $\bigotimes_{a = 1}^{k} \Sigma_a$ from $n$ independent samples satisfies 
$$ d_F(\hat{\Sigma}, \Sigma) = O(\TODO{}). $$
with probability $\TODO{}$.
\CF{presumably there will be some dimension-dependent factors}.
\end{theorem}


\begin{theorem}[Matrix flip-flop]\label{thm:matrix-flipflop} If $\hat{\Sigma}$ denotes the MLE estimator for $\Sigma$, then provided $n \geq \TODO{}$ the flip-flop algorithm computes $\underline{\Sigma}$ with 
...
\TODO{Gaussian proof of moment map concentration}

\subsection{Gradients and Hessians}
\TODO{Hessian descriptions}

\section{Matrix normal models}
...
\TODO{Proof of \cref{thm:tensor-frobenius}. Prove bounds on $d_{F}(\hat{\Sigma}, I)$ assuming $X$ standard normal.}
...
\[ \Pr[ \langle \sum_{t} g_{t} g_{t}^{*}, X \otimes Y \rangle \geq \lambda \frac{\|X\|_{F} \|Y\|_{F}}{\sqrt{d_{a} d_{b}}} ] \leq 
\begin{cases} 
