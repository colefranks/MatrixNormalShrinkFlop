...
\begin{proof}
We have the following translation by \cref{lem:gradient}: \CF{use $\tr$ not $Tr$, also all through the robustness stuff}
...
and looking at the proof of \cref{thm:tensor-convexity} \CF{where? how? If needed, add the below inequality to the proof of theorem 13 and use \cref{} to refer to it here.}, we see that we actually have the bound:
\[ \sup_{\langle Y, I_{1} \rangle = \langle Z, I_{2} \rangle = 0} \frac{\langle Y, \Phi(Z) \rangle}{\|Y\|_{F} \|Z\|_{F}} \leq \frac{\tr[\Phi(I_{2})]}{4 \sqrt{d_{1} d_{2}}}       \]
...
However, to lower bound the rates for estimating $\Theta_1$, we must supply invertible $B$ with this property. Let the first $d_1$ columns of $B$ have entries i.i.d uniform in $[-1,1]$, and let the remaining entries be i.i.d uniform in $[-\delta, \delta]$. If $\delta = 0$, then with access to the random variable $Y_\delta:=(\sqrt{\Sigma_1} X_1 B^T, \dots, \sqrt{\Sigma_1} X_n B^T)$ by the argument above $\Theta_2$ has access to at most $nd_1$ samples. However, we claim that as $\delta \to 0$, the distribution of $Y_\delta$ tends to that of $Y_0$ in total variation distance. Thus the distribution of $\Theta(Y_\delta) \to \Theta(Y_0)$ in total variation distance.This means that 


%Then $\widehat{\Theta}_1$ is an estimator that has access to only $n d_1$ samples of a Gaussian. 

 and let the last $d_2-d_1$ columns of $B$ have entries uniform in $[-\delta,\delta]$
