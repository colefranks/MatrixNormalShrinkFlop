...
We begin by stating that a lemma that proves that when the gradient of the log-likelihood function is small, then it means we are close to the MLE.
...
$\delta = \dfrac{\eps^2}{16 k^2 \dmax^{3/2}}$.
...
\begin{proof}
	...
	$\varepsilon = \frac{\gamma}{4 \sqrt{(k+1) \dmax}}$, 
	if the number of samples satisfies 
	$n \geq \dfrac{350 k^2 \dmax^3}{\gamma^2 \cdot D}$ then with probability at most
...
\RMO{making the observation that this flip-flop keeps the trace of $\rho$ equals 1 - this will help with the descent lemma (makes it simpler)}
