...
\[ \leq (\|\delta'\|_{op}(2 + \|\delta\|_{op}) + |\eta|(1+3\|\delta'\|_{op})) \|\rho^{(a)}\|_{op} \|Z\|_{1}    \]
...
For input $\{X_{i}\}$ and perturbation $\|\delta\|_{op} \ll 1$ giving $\{X'_{i} := (e^{\delta})_{b} X_{i} \}$ for $b \neq a$, there is a universal constant $C$ (different from above):
...
For input $\{X_{i}\}$ and perturbation $\delta := \sum_{a} (\delta_{a})_{a}$ with $\|\delta\|_{op} := \sum_{a} \|\delta_{a}\|_{op} \ll 1$ small enough:
...
& \leq \left( \prod_{j=1}^k (1+C\|\delta_{j}\|_{op}) - 1 \right) \|\rho_{X}^{(a)}\|_{op} \|Y\|_{F}^{2} \leq C'_{k} \|\delta\|_{op} \|\rho_{X}^{(a)}\|_{op} \|Y\|_{F}^{2}.   \end{align*}
...
\[ \leq C' \|\delta\|_{op} (2+C' \|\delta\|_{op}) (\|\rho_{X}^{(a)}\|_{op} \|Y\|_{1})^{2} \leq (d_{a} \|\rho_{X}^{(a)}\|_{op} C'' \|\delta\|_{op}) \|\rho_{X}^{(a)}\|_{op} \|Y\|_{F}^{2}       \]
So assuming $d_{a} \|\rho_{X}^{(a)}\|_{op} \leq O(1)$, this is some fixed constant. 

In toto this gives the corollary:
\[ \frac{1}{d_{a}}\|\nabla^{2}_{aa} f(e^{\delta}) - \nabla^{2}_{aa} f(I)\|_{op} \leq (C' + C'') \|\delta\|_{op} \|\rho_{X}^{(a)}\|_{op}    \]
...
\[ \frac{1}{\sqrt{d_{a} d_{b}}} \|\nabla^{2}_{ab} f(e^{\delta}) - \nabla^{2}_{ab} f(I)\|_{op} \leq O(\|\delta\|_{op}) \|\rho_{X}^{(a)}\|_{op}      \]
\end{corollary}
\begin{proof}
...
Again assuming our one-body marginals are $O(1)$-balanced, this is a fixed function of $k$. 
