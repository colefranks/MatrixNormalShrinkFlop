...
\RMO{it would be great to have a preprocessing of the input to assume that the initial trace is 1, so that we get $\nabla_0 f_x  = 0$. For now I will assume that trace is one because it simplifies the descent lemma.}
...
$\delta = \dfrac{\eps \lambda}{2 \dmax^{1/2}}$. \RMO{TODO: put the dependency on the strong convexity parameter, and remove the placeholder $\lambda$ here.}
...
Finally, we can use the Markov inequality to see that the event
\begin{align*}
