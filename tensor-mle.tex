...
\begin{prop}\label{eq:op norm bound}
\begin{equation}
  \frac{1}{3} (\sqrt{n} + \sqrt{m}) \leq \E \norm{A}_{\op} \leq \sqrt{n} + \sqrt{m}. 
\end{equation}
\end{prop}
The upper bound is from Theorem 5.32 in \cite{vershynin2010introduction}. We will show the lower bound later in this section when we discuss a few more properties of Gaussians. 
...
\begin{proof}[Proof of \cref{eq:op norm bound}]We can now also show the lower bound in \cref{eq:op norm bound}. The moment definition of sub-Gaussianity shows $\E \|A\|^{2} \leq \E \|A\| + O\left(\sqrt{\frac{p}{\sigma^{2}}} \right)$, so we continue
...
Here the lower bounds follow since $C'$ is a universal constant and $n \geq m$.  \end{proof}
...
Now we choose $q = c (\sqrt{m} + \sqrt{n})^{2}$ for small enough $c$ such that $C \sqrt{q} \leq \E \|A\|_{\op}$ (by \cref{eq:op norm bound}). \AR{Changed the choice of constant, hope \MW{} doesn't mind}
