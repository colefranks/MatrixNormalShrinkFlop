...
\MW{How about moving this to right after \cref{lem:convex-ball}, since it's also g-convexity general nonsense?}
...
We first explain how \cref{thm:Pisier-expansion}, along with the following estimate of the operator norm of a standard Gaussian $n \times m$ random matrix~$A$ (see \MW{ref} in \cite{P86}),
\begin{equation}\label{eq:op norm bound}
  \E \norm{A}_{\op} = \Theta(\sqrt{n} + \sqrt{m}),
\end{equation}
...
\begin{theorem}[\MW{ref} in \cite{P86}]\label{thm:banach conc}
Let $A$ be a standard Gaussian in separable Banach space with norm $\|\cdot\|$.
Then $\|A\|$ concentrates with parameter $\sigma^2 := \sup \{ \E \langle \xi, A \rangle^{2} \mid \|\xi\|_{*} \leq 1 \}$, where $\norm{\xi}_*$ denotes the dual norm.
That is,
\[ \Pr\Bigl( \abs[\big]{\norm A - \E \norm A} \geq t\Bigr) \leq \exp\Bigl( - \frac{\Omega(t^2)}{\sigma^{2}}\Bigr).   \]
Another equivalent definition of sub-Gaussianity is \MW{ref for equivalence}
\[ \forall p \geq 2: (\E \|A\|^{p})^{\frac{1}{p}} \leq \E \|A\| + O \left( \sqrt{\frac{p}{\sigma^{2}}} \right).  \]
\end{theorem}
...
\end{align*}
by Jensen's inequality and the convexity of $\norm{\cdot}_{\op}^p$ \MW{(!)}.
Now note $(A_i,B_i)$ has the same distribution as $(\frac{A_i+B_i}{\sqrt2},\frac{A_i-B_i}{\sqrt2})$, so the right-hand side is equal to
\begin{align*}
&\quad \frac12 \E \norm*{\left( \sum_{i=1}^N (A_i + B_i) \otimes (A_i + B_i) - \sum_{i=1}^N (A_i - B_i) \otimes(A_i - B_i) \right) \circ \Pi}_{\op}^p \\
...
\MW{We need the $2p$-th powers of this result\dots}
...
Now, a monomial of independent standard Gaussians random variables is nonnegative, and positive iff each variable appears an even number of times.
\MW{Remind me how we use the 2nd half of the preceding sentence? Isn't the trace just a sum of monomials and that is all we need?}
Thus, whenever $\E \tr ( A^T_{i_1} A_{j_1} \cdots A^T_{i_q} A_{j_q} )$ is nonzero, it is positive, so we can upper bound the sum term by term as
\begin{align*}
...
\leq 1,
\end{align*}
where we first used that $\braket{\xi,A}$ is distributed the same as $\norm\xi_F A_{11}$ by orthogonal invariance \MW{Why is this not a completely general argument?}, and then that the trace norm dominates the Frobenius norm.
...
% \leq 2 \E \norm*{\sum_{i=1}^N A_i \ot B_i}_{\op}^{2q}
\leq 2 N^q m^2 \Bigl( \E\norm{A}_{\op}^{2q} \Bigr)^2
\leq 2 N^q m^2 \Bigl( \E\norm A_{\op} + C \sqrt{q} \Bigr)^{4q}.
\end{align*}
for some universal constant~$C>0$.

\MW{Please check if the proof is now correct (and there is no easier way):}
Finally, we can use the Markov inequality to see that, for any~$C'>0$, the event
\begin{align*}
  \norm*{\left(\sum_{i=1}^N A_i \otimes A_i \right) \circ \Pi}_{\op} \leq (2C')^2 t^2 \sqrt{N} \bigl( \E \norm{A}_{\op} \bigr)^2
\end{align*}
holds up to failure probability at most
\begin{align*}
  \frac{\E \norm*{\left(\sum_{i=1}^N A_i \otimes A_i \right) \circ \Pi}_{\op}^{2q}}{\left( (2C')^2 t^2 \sqrt{N} \bigl( \E \norm{A}_{\op} \bigr)^2 \right)^{2q}}
\leq % \frac{2 N^q m^2 \Bigl( \E\norm A_{\op} + C \sqrt{q} \Bigr)^{4q}}{(2C')^{4q} t^{4q} N^q \bigl( \E \norm{A}_{\op} \bigr)^{4q}} =
  \frac{2 m^2 \Bigl( \E\norm A_{\op} + C \sqrt{q} \Bigr)^{4q}}{(2C')^{4q} t^{4q} \bigl( \E \norm{A}_{\op} \bigr)^{4q}}.
\end{align*}
Now we choose $q = (\sqrt m + \sqrt n)^2 \geq m + n$.
In view of \cref{eq:op norm bound}, we can moreover choose~$C'\geq1$ to be a universal constant such that $C \sqrt q \leq C' \, \E \norm{A}_{\op}$.
Then, the failure probability can be bounded as follows:
\begin{align*}
  \frac{2 m^2 \Bigl( \E\norm A_{\op} + C \sqrt{q} \Bigr)^{4q}}{(2C')^{4q} t^{4q} \bigl( \E \norm{A}_{\op} \bigr)^{4q}}
\leq
% \frac{2 m^2 \Bigl( (1 + C') \E\norm A_{\op} \Bigr)^{4q}}{(C')^{4q} t^{4q} \bigl( \E \norm{A}_{\op} \bigr)^{4q}} =
2 m^2 \left( \frac{1 + C'}{2 C' t} \right)^{4q}
\leq 2 m^2 t^{-4(m+n)}
= t^{-\Omega(m+n)}
\end{align*}
In the last step we used that $t\geq2$, so the prefactor can be absorbed at the cost of slightly changing the constant in the exponent.
\end{proof}
...
%\[ \sum_{i=1}^{nD/d_{a}d_{b}} \frac{1}{nD} Y_i \otimes Y_i   \]
...
\begin{align} \sum_{i = 1}^n Y_i Y_i^{\dagger} - \frac{d_2}{d_1} I_{d_1} = \sum_{i = 1}^n X_i \left( \sum_{j}  X_i^{\dagger} X_i\right)^{-1} X_i^\dagger - \frac{d_2}{d_1} I_{d_1}.\label{eq:1marg}
\end{align}
