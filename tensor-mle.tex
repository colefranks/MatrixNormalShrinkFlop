...
% k^2 \left( \frac{\sqrt{nD}}{k d_{\max}} \right)^{-\Omega(d_{\min})}
% \end{align*}}
% I think we need $\delta \leq r = c/\sqrt{ k d_{max}}$ to apply \cref{lem:convex-ball}. This is not guaranteed by $\delta^2 \leq 1/\max\{k, d_{max}}$.
%%%
...
\end{align}
Note that, by construction, $\Phi_Y^*(I_{d_1})$ is proportional to $I_{d_2}$ and $\tr \Phi_Y(I_{d_2}) = \|Y\|^2 = nD$. Thus $\Phi_Y$ is $\delta$-balanced if and only if $\|\frac{1}{nD} \Phi_Y(I_{d_2}) - \frac{1}{d_1} I_{d_1}\|_{op} \leq \delta.$ 
...
\end{align*}
By the remarks preceding the lemma, the above condition implies $\Phi_Y$ is $\eps \sqrt{\frac1{nD}}$-balanced.
\end{lemma}
