...
\begin{lemma} If $f$ is $\lambda$-geodesically strongly convex on a ball of radius $\lambda/2$ about $I$ and the geodesic gradient $\nabla f(I)$ satisfies $\|\nabla f(I)\|_F \leq \eps \leq c \lambda^2$, then there is an optimizer $Z$ such that $\|\log Z\|_F \leq \eps/\lambda$. 
\end{lemma}
\begin{proof}
\TODO{basic}.
\end{proof}
Next, we state the two theorems that allow us to apply this lemma. We prove them in the next two sections.
\begin{lemma}\label{lemma:gradient-bound} Suppose the number of samples is at least \TODO{}. Then with probability \TODO{}, 
$$\| \nabla f(I)\|_{op} \leq \TODO{}$$
as a consequence, $\|\nabla f(I)\|_F \leq \TODO{}$. 
\end{lemma}

\begin{theorem}\label{thm:tensor-convexity}
Suppose the number of samples is at least \TODO{}. Then with probability \TODO{}, $f_X$ is $\TODO{\lambda}$-strongly convex. 
\end{theorem}
