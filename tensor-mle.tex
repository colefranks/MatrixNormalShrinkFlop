...
\MW{Sadly now we have $k+1$-tuples, so $\norm{H}_F$ looks terrible :D Use $_2$?} We extend these definitions to tuples of matrices $A=(A_1,\dots,A_k)$ and denote them by the same symbol, i.e., $\norm{A}_F = (\sum_{a=1}^k \norm{A_a}_F^2)^{1/2}$ and similarly for the inner product.
...
\MW{No $1/k$ in front of $H_0$ I think.}
\MW{Can we also divide by $D$ here in the exponent, instead of in the objective function?}
...
\begin{align*}
  \exp_\Theta(H) &=   e^{\frac{1}{k} H_0}\cdot ( \Theta_1^{1/2} e^{\sqrt{d_1} H_1} \Theta_1^{1/2}) \ot  \dots \ot (\Theta_k^{1/2} e^{\sqrt{d_k} H_k} \Theta_k^{1/2}).
\end{align*}
...
\MW{What is the difference between a (totally) convex and (totally) geodesic subset?}
...
\qquad \forall H \in \H.
\end{align*}
We abbreviate $\nabla f = \nabla f(I,\dots,I)$ and write $\nabla_a f$ for the components.
As a block matrix,
\begin{align*}
  \nabla f = \left[\begin{array}{c|ccc} \nabla_0 f & \nabla_1 f & \cdots & \nabla_k f \end{array}\right],
\end{align*}
where $\nabla_0 f \in \R$, while each $\nabla_a f(\Theta)$ is a $d_a \times d_a$ traceless symmetric matrix for~$a\in[k]$.
...
For this, analogously to the discussion in \cref{subsec:model}, we ``flatten''~$x$ by thinking of it as a $d_a \times \frac{d_1d_2d_3\dots d_kn}{d_a}$ matrix~$x^{(a)}$ for any $a\in[k]$, and define $\rho^{(a)} := \frac1{nD} x^{(a)} (x^{(a)})^T$.
It is not hard to verify that $\rho^{(a)}$ is a positive semidefinite $d_a\times d_a$ matrix and satisfies the following property:
...
In the language of linear algebra, the matrix $\rho^{(a)}$ is known as the \emph{partial trace} of~$\rho$ over all but one of the coordinates.
...
&= \partial_{t=0} \tr \rho \, (I \ot \cdots \ot I \ot e^{t\sqrt{d_a} H} \ot I \ot \cdots \ot I) - \frac1D\log\det(e^{t\sqrt{d_a} H_{(a)}}) \\
&= \sqrt{d_a} \tr \rho \, H_{(a)}
= \sqrt{d_a} \tr \rho^{(a)} \, H
\end{align*}
using \cref{eq:partial trace duality} and that $\tr H_{(a)} = \tr H = 0$.
