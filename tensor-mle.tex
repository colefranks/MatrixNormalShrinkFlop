...
\textbf{Algorithm}:
\begin{enumerate}
\item\label{it:flip-flop step 1} Set $\otheta_a = \dfrac{1}{d_a} \cdot I_a$ for each $a \in [k]$, and $\delta = \dfrac{\eps^2}{16 d_k^{3/2}}$.
...
\item
If $\norm{\nabla_a}_F^2 < \delta^2/k$, output $\otheta$ and return.
...
\begin{lemma}[Initial Conditions]\label{lem:tensor-initial-conditions}
	...
	When the number of samples $n \geq \Gamma \cdot k^2 \cdot \dmax^2/D$, with probability at least $1 - k^2 \cdot \left( \dfrac{\sqrt{nD}}{k \dmax} \right)^{-\Omega(\dmin)} - 2(k+1) \cdot e^{- \Omega(nk^2D/\dmax)}$ we have that the following conditions hold:
	\begin{enumerate}
...
\RMO{making the observation that this flip-flop keeps the trace of $\rho$ equals 1}

\begin{prop}[Unit Trace]
	Let $\Theta$ and $\Upsilon$ be successive scalings produced by the flip-flop algorithm. Then $\tr \Theta = \tr \Upsilon$.
\end{prop}

\begin{proof}
	Let $\rho = \Theta^{1/2} \cdot \sum_{i=1}^n x_i x_i^\dagger \cdot \Theta^{1/2}$.
	Since $\Upsilon$ is the successive scaling from $\Theta$, we have that
	there is $a \in [k]$ such that 
	$$\Upsilon_a = \dfrac{\tr \rho}{d_a} \cdot \Theta_a^{1/2} (\rho^{(a)})^{-1} \Theta_a^{1/2} $$
	and $\Upsilon_b = \Theta_b$ for all $b \neq a$. Let $E_{(a)} = I_1 \otimes \cdots \otimes I_{a-1} \otimes (\rho^{(a)})^{-1} \otimes I_{a+1} \otimes \cdots \otimes I_k$.
	Thus, we have:
	\begin{align*}
		\tr \Upsilon &= \tr\left[ \Upsilon^{1/2} \sum_{i=1}^n x_i x_i^\dagger \cdot \Upsilon^{1/2} \right] = \tr\left[ \Upsilon \sum_{i=1}^n x_i x_i^\dagger \right] \\
		&= \tr\left[ \dfrac{\tr \rho}{d_a} \cdot \Theta^{1/2} E_{(a)} \Theta^{1/2} \cdot \sum_{i=1}^n x_i x_i^\dagger \right] = \dfrac{\tr \rho}{d_a} \cdot \tr\left[ E_{(a)} \rho \right] \\
		&= \dfrac{\tr \rho}{d_a} \cdot \tr\left[ (\rho^{(a)})^{-1} \rho^{(a)} \right] = \tr \rho
	\end{align*}
\end{proof}

Thus, since we started the flip-flop algorithm with a matrix of unit trace, the proposition above allows us to assume from now on that each scaling produced by the flip-flop algorithm will have unit trace. 
In particular, we have that $\nabla_0 f_x(\Theta) = 0$ at each iteration.
...
We first explain how \cref{thm:Pisier-expansion}, along with the following estimate of the operator norm of a standard Gaussian $n \times m$ random matrix~$A$ implies \cref{thm:hess-pisier}.
\begin{equation}\label{eq:op norm bound}
  \frac{1}{3} (\sqrt{n} + \sqrt{m}) \leq \E \norm{A}_{\op} \leq \sqrt{n} + \sqrt{m}. 
\end{equation}
The upper bound is from Theorem 5.32 in \cite{vershynin2010introduction}. We will show the lower bound later in this section when we discussed a few more properties of Gaussians. 
\MW{I think we currently also seem to use some kind of lower bound to wrap up.}
...
\begin{lemma} \label{l:opNormSubG}
Let $A$ be an $n \times m$ matrix with independent standard Gaussian entries. Then $\norm{A}_{\op}$ is sub-Gaussian with parameter $\sigma^{2} = 1$. 
\end{lemma}
\begin{proof}
...
We can now also show the lower bound in \cref{eq:op norm bound}. The moment definition of sub-Gaussianity shows $\E \|A\|^{2} \leq \E \|A\| + O\left(\sqrt{\frac{p}{\sigma^{2}}} \right)$, so we continue
\[ (\E \norm{A}_{\op})^{2} \geq \E \norm{A}_{\op}^{2} - C \sqrt{2} ,   \]
where in the last step we substituted $p=2$ and $\sigma^{2} = 1$ by \cref{l:opNormSubG}. We can then lower bound the second moment simply assuming $n \geq m$:
\[ \E \norm{A}_{\op}^{2} = \E \norm{A A^{T}}_{\op} \geq \norm{\E A A^{T}}_{\op} = n ,  \]
where the first step is by definition of $\norm{\cdot}_{\op}$, the second is by Jensen's inequality, and the third step is because $\E A A^{T} = n I_{m}$. Therefore in total we have shown
\[ \E \norm{A}_{\op} \geq \sqrt{ \E \norm{A}_{\op}^{2} - C' } = \Omega(\sqrt{n}) = \Omega(\sqrt{n} + \sqrt{m}) .   \]
Here the lower bounds follow since $C'$ is a universal constant and $n \geq m$. 
...
\begin{align*}
  \norm*{\left(\sum_{i=1}^N A_i \otimes A_i \right) \circ \Pi}_{\op} \leq (2 t)^2 \sqrt{4 N} \bigl( \E \norm{A}_{\op} \bigr)^2
\end{align*}
...
Now we choose $q = c (\sqrt{m} + \sqrt{n})^{2}$ for small enough $c$ such that $C \sqrt{q} \leq \E \|A\|_{\op}$ \MW{the $=$ is a $\geq$, brzz}(by \cref{eq:op norm bound}). \AR{Changed the choice of constant, hope you don't mind}
...
= t^{-\Omega(m+n)}
\end{align*}
In the final step we used that $q = \Omega(m + n)$ by \cref{eq:op norm bound}, as well as the fact that $t \geq 2$, so the prefactor $m^{2}$ can be absorbed at the cost of slightly changing the constant in the exponent.
...
\begin{fact} For all symmetric $d\times d$ matrices $\delta $ such that $ \|\delta\|_{op} \leq \frac{1}{2}$ \MW{why not $\leq1$?} \AR{Slightly worse constant (3 instead of 2) for $\delta \leq 1$ I think?}, we have
