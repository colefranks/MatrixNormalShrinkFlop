...
If $d( \Upsilon^{-1}; \overline{\Theta}_2) > \eps$, set $\overline{\Theta}_2 \leftarrow \Upsilon^{-1}$ and return to Step 1.
...
% &= \frac n 2 \log \det \left( \lambda \textstyle\bigotimes_{a=1}^k P_a \right) - \frac\lambda2 \sum_{i=1}^n x_i^T \left( \textstyle\bigotimes_{a=1}^k P_a \right) x_i \\
...
% We may extend $f_x$ to $\oplus_i \PD_{d_i}$ by writing
% \begin{align}
...
$$\Phi: Z \mapsto \tr_2 \rho \left(I \ot Z\right),$$
 is known as a \emph{quantum channel}. If $\rho = \sum_{i = 1}^N x_i x_i^T$ for $x_i \in \R^{d_1 \ot d_2}$, then we have the following two equivalent ways to write the corresponding channel, which we call $\Phi_{\samp}$:
...
Here $x_i$ are viewed as $d_1 \times d_2$ matrices and $x_i^\dagger$ is the matrix transpose. In our proof of strong convexity, we will show that for $\rho = \rho^{(a,b)}$ this quantum channel has a spectral gap, or rather that $\Phi_{\samp}$ has a small operator norm on the traceless symmetric matrices.
...
\begin{theorem}\label{thm:hess-pisier} Let $X = (X_1, \dots, X_N)$ be a tuple of standard Gaussians on $(\R^{d_1}\ot \R^{d_2})^n$. Then $\forall t \geq 1$ with failure probability at most $t^{-\Omega(d_{1} + d_{2})}$, the random quantum channel $\frac{1}{nD}\Phi_{X}$
...
As a linear map, define the first term to be $\Phi:Z \mapsto \sqrt{d_a d_b} \tr_b \rho^{(a,b)} (I \ot Z)$. The state $\rho^{(a,b)} = \tr_{\overline{\{a,b\}}} \rho$ is given by $\rho^{{a,b}} =  X X^\dagger/\|X\|^2$ where $X$ is viewed as an $n D/{d_a d_b} \times d_a d_b$ matrix. Equivalently, $\rho^{(a,b)}$ is distributed as
$$\frac{1}{\sum_{i} \|Y_i\|^2} Y_i Y_i^T$$ where $Y_i$ are $n D/{d_a d_b}$ i.i.d standard Gaussians on $\R^{d_a} \ot \R^{d_b}$. By \cref{thm:hess-pisier} with $2t^2 = \frac {s \sqrt{nD}}{d_a + d_b}$, the operator norm of $\Phi$ restricted to $\smallSym^0_{d_b}$ is $O(s n D /2 \sum_i \|Y_i\|^2 )$
...
Let $M$ be a square, symmetric block matrix with blocks $M_{ab}$ of dimension $d_a \times d_b$ for $a,b \in [k]$. Suppose that for all $a < b \in [k]$, $\|M_{ab}\|_{op} \leq \lambda/2k$ and for all $a \in [k]$ $\|M_{aa} - I\|_{op} \leq \lambda/2$. Then $\|M - I\|_{op} \leq \lambda$.%, and in particular $f$ is $(1 - \lambda)$-geodesically convex at the identity.
\end{lemma}
\begin{proof}
...
We first apply the inequality to the upper-left block two-by-two matrix. By our bound on $\|M_{12}\|_{op}$, we may take
   $A = (\lambda/2k) I_{d_1\times d_1}$, $B = (\lambda/2k) I_{d_2\times d_2}$ to find that
   \begin{align*}
M \succeq \begin{bmatrix}
M_{11}-  (\lambda/2k) I & 0 & \hdots &  M_{1k} \\
0 & M_{11} - (\lambda/2k) I & \ddots & \vdots \\
\vdots & \ddots & \ddots & \vdots\\
M_{k1} & \hdots &  \hdots & M_{kk}\\
  \end{bmatrix}.\end{align*}
 Applying this for all $a < b$ we find that
 $$M \succeq \bigoplus_{a \in [k]} (M_{aa} -  \lambda I/2) \succeq (1 -\lambda) I, $$
...
\section{Improvements for the matrix normal model \CF{NEW}}\label{sec:matrix-normal-ii}



We now prove \cref{thm:matrix-normal}, an improvement to \cref{thm:tensor-frobenius} in the case $k=2$. The results for $k = 2$ are stronger in that \CF{ergh} the MLE can be shown to be close to the truth in operator norm rather than the looser Frobenius norm, the errors are tight for the individual factors, and that the failure probability is inverse exponential in the number of samples rather than inverse polynomial.

The proof plan is similar to that in \cref{subsec:proof-sketch}, but with the virtually equivalent notion of quantum expansion substituted for strong convexity. As mentioned in \cref{subsec:strong-convex}, the strong convexity of $f_\samp$ is closely related to quantum expansion. We make this relationship more precise here. First we need a definition.

\begin{definition}[Quantum expansion] Let $\Phi:\mat(d_1) \to \mat(d_2)$ be a quantum channel (c.f. \cref{eq:quantum-channel}).
\begin{enumerate}
...
\end{align*}
\item $\Phi$ is an \emph{$(\eps, 1 - \lambda)$-quantum expander} if $\Phi$ is $\eps$-doubly balanced and
$$\|\Phi\|_0 \leq \frac{(1 - \lambda)}{\sqrt{d_1d_2}} \tr \Phi(I_{d_2}).$$
\end{enumerate}
A $(0, 1 - \lambda)$-quantum expander is called a $(1-\lambda)$-quantum expander.
\end{definition}
By \cref{thm:tensor-convexity}, the $(\eps, 1 - \lambda)$-quantum expansion of the channel $\Phi_\samp$ implies that $\nabla^2 f_x \geq (\lambda - O(\eps)) I.$ It is desirable to work with $(\eps, 1 - \lambda)$-quantum expanders $\Phi$ because they have a \emph{spectral gap,} i.e. $\Phi$ satisfies the condition 
\begin{align}\sigma_2(\Phi) \leq \frac{(1 - \lambda)}{\sqrt{d_1d_2}} \tr \Phi(I_{d_2}).\label{eq:spectral-gap}
\end{align}
It is known that $(\eps, 1 - \lambda)$-quantum expansion implies spectral gap $(1 - \lambda - O(\eps))$. Our main tool is the following:
...
\begin{theorem}[\CF{KLR}]\label{thm:klr}
If $\Phi$ is $\eps$-doubly balanced and has spectral gap $(1 - \lambda)$ for $\eps \leq c \lambda^2/\log d_1$, then the maximum likelihood estimator $(\widehat{\Theta}_1, \widehat{\Theta}_2) \in \SL_{d_1}\times \SL_{d_2}$ is unique and satisfies
$$\| \widehat{\Theta}_1 - I_{d_1}\|_{op}, \| \widehat{\Theta}_2 - I_{d_2}\|_{op} \leq \frac{\eps \log d_1}{\lambda}.$$
\end{theorem}

We prove in this section is that $\Phi$ is with high probability a good quantum expander, which allows us to apply the previous theorem.

\begin{theorem}\label{thm:operator-cheeger}
There are absolute constants $c, C$ such that the following holds for all $d_1 \leq d_2$. Suppose $n \geq C (d_2/d_1) \max\{\log (d_2/d_1),  \eps^{-2}\} $. Then $\Phi$ is $\eps \sqrt{d_2/n d_1}$-balanced and is an $(\eps \sqrt{d_2/n d_1}, 1 - c)$-quantum expander, with failure probability $O(e^{ - \Omega( d_2 \eps^2)}).$
\end{theorem}
...
By \cref{thm:tensor-convexity}, and the consequent improvement to \cref{thm:ball-convexity} \CF{actually make it like this}, we have the following.
...
We will prove \cref{thm:operator-cheeger} in the appendix using techniques similar to \cite{FM20} using Cheeger's inequality. As a preliminary result we can already prove that the MLE is accurate in operator norm, but with slightly worse failure probability using the tools for the tensor case:

\begin{prop} \label{thm: operator-Pisier}
There are absolute constants $c, C$ such that the following holds for all $d_1 \leq d_2$. Suppose $n \geq C (d_2/d_1) \max\{\log (d_2/d_1),  \eps^{-2}\} $. Then $\Phi$ is an $(\eps \sqrt{d_2/n d_1}, 1 - c)$ quantum expander with failure probability $O \left(\frac {\sqrt{nD}}{d_1}\right)^{ - \Omega(d_1)}$.
\end{prop}
