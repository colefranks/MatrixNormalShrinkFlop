...
There is $c> 0$ such that the following holds: with probability at least $1 - k^{2} n^{c 2 (d_1 + d_2)}$\TODO{make sure this is right. Rafael, if you use this value, make a note by it to make sure we check again.}, the function $f_{\rv}$ is $c$-strongly convex at all points $Z\in \PD$ with $\sum_{i = 1}^n \|\log Z_a\|_{op} < c/k.$ 
\end{theorem}
...
\begin{theorem}\label{thm:hess-pisier} Let $X = (X_1, \dots, X_N)$ be a tuple of standard Gaussians on $(\R^{d_1}\ot \R^{d_2})^n$. Then $\forall t \geq 1$ with failure probability at most $t^{-\Omega(d_{a} + d_{b})}$, the random quantum channel 
$$T: X \mapsto \frac{1}{nD}\sum_{i = 1}^N X_i Z X_i^\dagger$$
has operator norm 
$O \left( t^{2} \frac{d_{a}+d_{b}}{\sqrt{d_{a} d_{b} nD}} \right)$ restricted to the traceless symmetric matrices. \end{theorem}
...
\begin{corollary}[\CF{Yet again reworking this.}] \label{cor:off-diag-hess} For traceless symmetric matrices $Y \in \Sym^0_{d_a}$, $Z \in \Sym^0_{d_b}$, we have 
%$O \left( \frac{d_{a}+d_{b}}{\sqrt{d_{a} d_{b} nD}} \right)$  

$$ \langle Y,  \left( \nabla^2_{ab} f_{\rv} \right) Z \rangle
= O \left(t \|Y\|_F\|Z\|_F\right)
$$
%\TODO{Akshay, please make the rhs $O(\lambda)$ and then adjust the failure probability accordingly}
with probability %$1 - t^{\Omega (d_{a} + d_{b})} n^{- (d_a + d_b) \log \frac{\sqrt{D} }{d_{a} + d_{b}}}$.
$1 - \left(\frac{t \sqrt{nD}}{d_a + d_b}\right)^{ - \Omega(d_a + d_b)}$.

In particular, there are absolute constants $C, 0 < c < \CF{1/50}$ such that if $n \geq C d_1^2/D$ we have $\|\nabla^2_{ab} f_{\rv}\|_0 \leq c/k$ with probability $1 - k^2 n^{- \Omega(d_1 \log (\sqrt{D}/ d_1))}$ for all $a,b \in [k]$.
...
\CF{get rid of big O's in bounds, not probabilities - actually set $c$'s here?}
\begin{corollary} \label{cor:off-diag-hess} For traceless symmetric matrices $Y \in \Sym^0_{d_a}$, $Z \in \Sym^0_{d_b}$, we have 
%$O \left( \frac{d_{a}+d_{b}}{\sqrt{d_{a} d_{b} nD}} \right)$  

$$ \langle Y,  \left( \nabla^2_{ab} f_{\rv} \right) Z \rangle
= O \left( t^{2} \frac{d_{a}+d_{b}}{\sqrt{nD}} \|Y\|_F\|Z\|_F\right)
$$
%\TODO{Akshay, please make the rhs $O(\lambda)$ and then adjust the failure probability accordingly}
with probability $1 - t^{-\Omega(d_{a} + d_{b})}$ for $t \geq \CF{}$. In particular, there are absolute constants $C, 0 < c < \CF{1/50}$ such that if $n \geq C d_1^2/D$ we have $\|\nabla^2_{ab} f_{\rv}\|_0 \leq c/k$ with probability $1 - n^{- \Omega(d_1 \log (\sqrt{D}/ d_1))}$.


%\geq 1 - \exp( - \Omega(d_{a} + d_{b}) )$ for $\lambda^{2} n D \gg \max_{a} d_{a}^{2}$. 
...
$$\frac{1}{\sum_{i} \|Y_i\|^2} Y_i Y_i^T$$ where $Y_i$ are $n D/{d_a d_b}$ i.i.d standard Gaussians on $\R^{d_a} \ot \R^{d_b}$. By \cref{thm:hess-pisier}, the operator norm of $T$ restricted to $\Sym^0_{d_b}$ is $O(t^2(d_a + d_b) \sqrt{ n D} / \sum_{i \in \|Y_i\|^2} )$ 
with probability $t^{-\Omega(d_{a} + d_{b})}$. The denominator is a sum of $nD$ many i.i.d $\chi$-square random variables, and so is $\Omega(nD)$ with failure probability $e^{ -\Omega(nD)} \ll t^{-\Omega(d_{a} + d_{b})}$ \cite{W19} \CF{puts constraint on $t$}, so finally we have that the operator norm of $T$ restricted to $\Sym^0_{d_b}$ is $O \left( \frac{d_{a}+d_{b}}{\sqrt{ nD}} t^2\right)$.
...
As we have already shown in \cref{prop:gradient-bound}, we have $\norm{\nabla_a f_{\rv}}_{\op} = \|\sqrt{d_a} \rho^{(a)} - I_a/\sqrt{d_a}\|_{op} \leq \eps/\sqrt{d_a}$ with failure probability at most $e^{-D \eps^2/2Cd_a}$ , and likewise for $b$. Set $\eps^2 =  \frac{d_{a}+d_{b}}{\sqrt{ nD}} t^2$. Thus we have$$\sqrt{d_a d_b} |(\tr Y  \rho^{(a)})\bigl(\tr Z \rho^{(b)}\bigr)| \leq \left( \frac{\eps \|Y\|_{1}}{\sqrt{d_{a}}} \right) \left( \frac{\eps \|Z\|_{1}}{\sqrt{d_{b}}} \right) \leq \eps^2 \|Z\|_F\|Y\|_F $$
with failure probability at most $2 e^{ - \sqrt{D}d_2 t^2/C\sqrt{n} d_1 } \ll t^{-\Omega(d_{a} + d_{b})}.$ The bounds for the first and second term complete the proof.
...
\AR{I can make this proof look more like the convexity one, with block matrices, if you all want}
...
We next bound the radius $\kappa$ of the ball on which our function is geodesically convex. By \cref{thm:tensor-convexity}, with failure probability $\CF{1/poly(D)}$ we have that $f$ is $1-o_{d}(1)$ strongly convex at $I$. Because $\|\sqrt{d_{a}} \nabla_{a} f_{\samp}\|_{op} \leq \delta \leq c$, by \cref{convexRobustness} there is a constant $c$ such that $f$ is $1/2$ strongly convex at any point $\Theta$ such that $\sum_a \|\log \Theta_a\|_{op} \leq  c$ for all $a \in [k]$. Now note that 
