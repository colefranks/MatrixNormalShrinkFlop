...
If $P$ is positive definite and $A$ an invertible matrix then $AP A^T$ is again in positive definite.
The transformation $P \mapsto AP A^T$ is an isometry, i.e., it preserves the geodesic distance.
Importantly, the statistical distances we use are also \emph{invariant} under such transformations:
\begin{align*}
  D_F(A P A^T \Vert A P' A^T) = D_F(P \Vert P')
\end{align*}
...
\begin{align*}
  \P &= \{ (P_1,\dots,P_k) \in \PD(d_1) \times \dots \times \PD(d_k) \;:\; \det P_1 = \dots = \det P_k = 1 \}.
% \end{align*}
...
We take as our objective its logarithm, which is the following geodesically convex function \citep{burgisser2019towards}:
...
\begin{align*}
  f_x(P_1,\dots,P_k)
= \log \sum_{i=1}^n \braket{\samp_i, \left( \textstyle \bigotimes_{a=1}^k P_a \right) \samp_i}.
\end{align*}
