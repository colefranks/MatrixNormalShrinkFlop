...
\[ \forall X \perp I_{a},Y \perp I_{b}:  \langle Q^{ab}, X \otimes Y \rangle \leq \lambda \frac{\|X\|_{F} \|Y\|_{F}}{\sqrt{d_{a} d_{b}}}   \]
...
\begin{theorem} [Pisier]
Let $Y$ be a standard gaussian in Banach space with norm $\|\cdot\|$. Then $\|Y\|$ is subgaussian with parameter $\sigma^{2} := \sup \{ \E \langle Y, \xi \rangle^{2} \mid \|\xi\|_{*} \leq 1 \} $
...
\begin{theorem}
Let $P$ denote the projection onto $vec{I_{m}} \in \R^{m^{2}}$, and $\{Y_{i}\}$ are iid standard gaussians in $\R^{m \times n}$ where $m \leq n$. Then for any small enough $c$, the  following holds with FAILURE PROBABILITY $\leq (mn)^{-c}$
\[ \|\sum_{i=1}^{N} \alpha_{i} Y_{i} \otimes Y_{i} (I - P) \|_{op} \leq O \left( \sqrt{\sum_{i} \alpha_{i}^{2}} \right) \left( \E \|Y\|_{op} \right)^{2} \]
\end{theorem}
\begin{proof}
We first begin by a standard symmetrization trick to linearize:
\[\E Y^{*} I_{m} Y = m I_{n} \implies \E Y \otimes Y (I-P) = 0\]
Therefore we can add the like term with new iid variables $\{Z_{i}\}$:
...
Here we have used Jensen's because the $Z$ term is mean 0. Now note $(Y,Z)$ has the same distribution as $(\frac{Y+Z}{\sqrt{2}},\frac{Y-Z}{\sqrt{2}})$
...
Note we've lost the projection $(I-P)$, but the left and right Kraus operators are independent. Now we apply the trace method to this operator
...
Denoting $Z$ also an iid gaussian, we have $\E \|Z\|_{2p}^{k} \leq ( \E \|Z\|_{2p}^{2p} )^{k/2p}$ by Jensen's inequality, so we can collect like terms together:
...
\[ \E \|\sum_{i} \alpha_{i} Y_{i} \otimes Z_{i}\|_{2p}^{2p}
\leq \left( \E_{Z} \|Z\|_{2p}^{2p} \right) \left( \sum_{\vec{i},\vec{j}} \alpha^{\vec{i}} \alpha^{\vec{j}} (\E_{Y} Tr [ Y_{i_{1}}^{*} Y_{j_{1}} ... Y_{i_{p}}^{*} Y_{j_{p}} ] ) \right) \]
...
In the last step we used unitary invariance of the joint $\{Y_{i}\}$ distribution, so $\sum_{i} c_{i} Y_{i}$ has the same distribution as $\|c\|_{2} Y$.
...
So again assuming $\|\alpha\|_{2} = 1$ and choosing $p = 2c \log mn$, where the only constraint is 
\[  p^{3/2} \ll (\sqrt{m} + \sqrt{n})^{2} \iff c \ll \frac{(\sqrt{m} + \sqrt{n})^{4/3}}{\log mn}  \] 
We can apply Markov's to get the bound:
...
\AR{I may have made a calculation error but it seems we could even pick $c \sim \frac{n^{2/3}}{\log n}$ so that we get $\exp(-\frac{n^{2/3}}{\log n})$ failure probability?}
