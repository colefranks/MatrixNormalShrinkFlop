...
Let $\rv = (\rv_1,\dots,\rv_n)$ be independent standard Gaussian random variables in $\R^D$, where the number of samples is $N \geq k^2 \max_{a=1}^k d_a^2 / D$. Then, with probability at least $1 - 1/{\color{red}\poly(D)}$, 
...
For input $\samp \in \CF{where?}$ such that $d_{a}\|\rho_{\samp}^{(a)}\|_{op} \leq 1+\frac{1}{20}$; and perturbation $\delta := \sum_{a} (\delta_{a})_{a}$ with $\|\delta\|_{op} = \sum_{a} \|\delta_{a}\|_{op} \leq \frac{1}{20}$ giving $\samp' := e^{\delta} \samp$:
\[ \frac{1}{d_{a}} \|\nabla^{2}_{aa} f(e^{2\delta}) - \nabla^{2}_{aa} f(I)\|_{op} \leq 40 \|\delta\|_{op} \|\rho_{\samp}^{(a)}\|_{op}      \]
\end{corollary}
%\CF{technically $f(e^{\delta})$ corresponds to $e^{\delta/2} \samp$}
\begin{proof}
Recall from \cref{lem:hessian} that $\frac{1}{d_{a}}\langle Y, \nabla^{2}_{aa} f_{\samp} Y \rangle = \langle \rho_{\samp}^{(a)}, Y^{2} \rangle - \langle \rho_{\samp}^{(a)}, Y \rangle^{2}$. We treat the perturbation as the composition of $k$ perturbations; 
...
\underset{\ref{atoaaRobustness},\;\ref{btoaaRobustness}}{\leq} \sum_{j=1}^{k}  9.5 \|\delta_{j}\|_{op} \|\rho_{\samp_{(j-1)}}^{(a)}\|_{op} \|Y^{2}\|_{1} \\
& \leq \left( \prod_{j=1}^k (1+9.5 \|\delta_{j}\|_{op}) - 1 \right) \|\rho_{\samp}^{(a)}\|_{op} \|Y\|_{F}^{2} \leq 10 \|\delta\|_{op} \|\rho_{\samp}^{(a)}\|_{op} \|Y\|_{F}^{2}.   \end{align*}
The term in parenthesis is shown by induction, and in the last step we used $\|\delta\|_{op} \leq \frac{1}{20}$. We next bound the rank-one term $\langle \rho_{\samp}^{(a)}, Y \rangle^{2}$:
\[ |\langle \rho_{\samp'}^{(a)}, Y \rangle^{2} - \langle \rho_{\samp}^{(a)}, Y \rangle^{2}| = |\langle \rho_{\samp'}^{(a)}, Y \rangle - \langle \rho_{\samp}^{(a)}, Y \rangle| |\langle \rho_{\samp'}^{(a)}, Y \rangle + \langle \rho_{\samp}^{(a)}, Y \rangle|    \]
\[ \leq 10 \|\delta\|_{op} (2+ 10 \|\delta\|_{op}) (\|\rho_{\samp}^{(a)}\|_{op} \|Y\|_{1})^{2} \leq 25 \|\delta\|_{op} d_{a} \|\rho_{\samp}^{(a)}\|_{op}^{2} \|Y\|_{F}^{2}       \]

In toto this gives the corollary:

\[ \frac{1}{d_{a}}\|\nabla^{2}_{aa} f(e^{2\delta}) - \nabla^{2}_{aa} f(I)\|_{op} \leq \|\delta\|_{op}(C' \|\rho_{\samp}^{(a)}\|_{op} + C'' \|\rho_{\samp}^{(a)}\|_{op}^{2})    \]
from which the statement follows by the initial condition $\|d_{a} \rho_{\samp}^{(a)}\|_{op} \leq (1+c)$.
\end{proof}
...
\end{definition}
\begin{definition}
For input $\rho_{\samp}$ and fixed $a,b$, define
\[ \langle Y, H_{\samp}^{ab} Z \rangle := \langle (Y_{a})(Z_{b}), \rho_{\samp} \rangle = \langle Y \otimes Z , \rho_{\samp}^{(ab)} \rangle     \]
\end{definition}
We use a lemma from \cite{KLR19}.
\begin{lemma}[\cite{KLR19}] \label{inftyto2}
$\|H_{\samp}^{ab}\|_{F \to F}^{2} \leq \|\rho_{\samp}^{(a)}\|_{op} \|\rho_{\samp}^{(b)}\|_{op}$
\end{lemma}
...
\CF{add quantifiers}
For $\samp \in \CF{somewhere}$ and perturbation $\delta \in \Sym$ with $\|\delta\|_{op} \leq \frac{1}{20}$, if $\samp' = (e^{\delta})_c \samp$ for $c \in \{a,b\}$ we have 
%$\{\samp'_{i} := (e^{\delta})_{c} \samp_{i} \}$ for $c \in \{a,b\}$:
\[ \|H_{\samp'}^{ab} - H_{\samp}^{ab}\|_{0} \leq 4.5 \|\delta\|_{op} \|H_{\samp}^{ab}\|_{F \to F}    \]
\end{lemma}
\begin{proof}
We will drop the superscript $(ab)$ since it doesn't change. By taking adjoints, we can assume w.l.o.g. that $c = b$. Let $R : \mat(d_{b}) \to \mat(d_{b})$ be defined as $R(Z) := (1+\eta)^{2} e^{\delta} Z e^{\delta}$ for $\eta$ defined by our normalization $\|(e^{\delta})_{b} \samp\|_{2}^{-1} =: (1+\eta) \|\samp\|_{2}^{-1}$. The subspace $\Sym_{d_{b}}^{0}$ is not invariant under $R$, but we show $R \approx I$. Let $\delta' := (1+\eta) e^{\delta} - I$. 
\[ \|R(Z) - Z\|_{F} \leq 2 \|\delta' Z\|_{F} + \|\delta' Z \delta'\|_{F} \leq (2 \|\delta'\|_{op} + \|\delta'\|_{op}^{2}) \|Z\|_{F}    \]
So using the definition of $\|\cdot\|_{0}$:
\[ \sup_{Y \in \Sym_{d_{a}}^{0},Z \in \Sym_{d_{b}}^{0}} \frac{\langle Y, (H_{\samp'} - H_{\samp}) Z \rangle}{\|Y\|_{F} \|Z\|_{F}} = \frac{ \langle Y, H_{\samp} (R(Z) - Z) \rangle }{\|Y\|_{F} \|Z\|_{F}} \]
...
For input $\{\samp_{i}\}$ and perturbation $\|\delta\|_{op} \leq \frac{1}{20}$ giving $\samp' := (e^{\delta})_{c} \samp$ for $c \not\in \{a,b\}$:
\[ \|H_{\samp'}^{ab} - H_{\samp}^{ab}\|_{0} \leq 19 \|\delta\|_{op} \|H_{\samp}^{ab}\|_{F \to F}    \]
\end{lemma}
\begin{proof}
Let $\delta' := (1+\eta) e^{2 \delta} - I_{c}$ for $\eta$ defined by normalization $\|(e^{\delta})_{c} \samp\|_{2}^{-1} =: (1+\eta) \|\samp\|_{2}^{-2}$
\[ \langle Y, (H_{\samp'} - H_{\samp}) Z \rangle = \langle \rho_{\samp}^{(abc)}, Y \otimes Z \otimes \delta' \rangle   \]
...
Here we again used that $\rho_{\samp}^{(abc)} \succeq 0$. This last term cannot be bounded by $\|H_{\samp}\|_{0}$ as $Y \succeq 0$; but $\|H_{\samp}\|_{F \to F}$ is good enough for our purposes. 

To finish the lemma we decompose $Y = Y_{+} - Y_{-}, Z = Z_{+} - Z_{-}$ and bound
\[ |\langle Y, (H_{\samp'} - H_{\samp}) Z \rangle| \leq \|\delta'\|_{op} \|H_{\samp}\|_{F \to F} \sum_{s,t \in \{+,-\}} \|Y_{s}\|_{F} \|Z_{t}\|_{F}   \]
...
\end{corollary}
\begin{proof}
Recall $\frac{1}{\sqrt{d_{a} d_{b}}} \langle Y, \nabla^{2}_{aa} f_{\samp} Z \rangle = \langle \rho_{\samp}^{(ab)}, Y^{2} \rangle - \langle \rho_{\samp}^{(a)}, Y \rangle^{2}$. Using the same iterative strategy as $\ref{diagRobustness}$ we can show: 
\[ |\langle Y, (\rho_{\samp'}^{(ab)} - \rho_{\samp}^{(ab)}) Z \rangle| \leq 20 \|\delta\|_{op} \|H_{\samp}\|_{F \to F} \|Y\|_{F} \|Z\|_{F}    \]
...
\[ |\langle \rho_{\samp'}^{(a)}, Y \rangle \langle \rho_{\samp'}^{(b)}, Z \rangle - \langle \rho_{\samp}^{(a)}, Y \rangle \langle \rho_{\samp}^{(b)}, Z \rangle|    \]
\[ \leq |\langle \rho_{\samp'}^{(a)} - \rho_{\samp}^{(a)}, Y \rangle \langle \rho_{\samp'}^{(b)}, Z \rangle| + |\langle \rho_{\samp}^{(a)}, Y \rangle \langle \rho_{\samp'}^{(b)} - \rho_{\samp}^{(b)}, Z \rangle|   \]
\[ 40 \|\delta\|_{op} (1+20\|\delta\|_{op}) \|\rho_{\samp}^{(a)}\|_{op} \|\rho_{\samp}^{(b)}\|_{op} \|Y\|_{1} \|Z\|_{1}    \]
\[ \leq 80 \|\delta\|_{op} (\sqrt{d_{a} d_{b}} \|\rho_{\samp}^{(a)}\|_{op} \|\rho_{\samp}^{(b)}\|_{op}) \|Y\|_{F} \|Z\|_{F}    \]

In toto this gives the lemma:
\[ \frac{1}{\sqrt{d_{a} d_{b}}} \|\nabla^{2}_{ab} f(e^{2 \delta}) - \nabla^{2}_{ab} f(I)\|_{op} \leq \|\delta\|_{op}((20 \|H_{\samp}\|_{F \to F}) + 80 (\sqrt{d_{a} d_{b}} \|\rho_{\samp}^{(a)}\|_{op} \|\rho_{\samp}^{(b)}\|_{op})) \]
\[ \leq 100 \|\delta\|_{op} \sqrt{\|\rho_{\samp}^{(a)}\|_{op} \|\rho_{\samp}^{(b)}\|_{op}}     \]
