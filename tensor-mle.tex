...
$$ d_{F}(\hat{\Sigma}_i, \Sigma_i) = O(\TODO{}) $$
...
$$f(A^\dagger e^{t Z} A)$$ is convex in $t$ for all $Z \in H$ and $A\in G$. Say $f$ is $\lambda$-geodesically \emph{strongly convex} w.r.t norm $\|\cdot\|$ if $\partial^2_t f(A^\dagger e^{t Z} A) \geq \lambda \|Z\|^{2}$. We will use the norm $\| \cdot\|$ given by the inner product $\langle X, Y \rangle = \sum_{a \in [k]} \frac{1}{d_a} \tr X_a Y_a $ for $X, Y \in H$. 
\end{definition} 
With this definition in place, we are able to state the rather simple plan. \CF{change $\Sigma$ to $\Theta$ in what follows}
\begin{enumerate}
...
\item \textbf{Show strong convexity: } Show that, with high probability, $f_{\vec X}$ is $\Omega(1)$-geodesically strongly convex near $I$ w.r.t. $\|\cdot\|$.
\item \textbf{Bound the gradient: } Show that $\| \nabla f_{\vec X} (I) \|$ is small with high probability.
...
\end{proof}
Next, we state the two theorems that allow us to apply this lemma. We prove them in the next two sections. \CF{probably need to define geodesic gradient}
...
$$\| \nabla_a f \|_{op} \leq \eps/d_a.$$
As a consequence, $\|\nabla f(I)\|^{2} = \sum_{a} d_{a} \|\nabla_a f \|_{F}^{2} \leq k \eps^{2}$. 
\end{lemma}

\begin{theorem}\label{thm:tensor-convexity}
Suppose the number of samples is at least $k^{2} \max_{a} d_{a}^{2}/D$. Then with failure probability $\leq 1/poly(D)$, $f_X$ is $\Omega(1)$-strongly convex at $I$ w.r.t. $\|\cdot \|$.
\end{theorem}
...
\begin{proof}[Proof of \cref{thm:tensor-frobenius}] By \cref{lemma:gradient-bound}, we have 
\[ \forall a: \|(\nabla f)_{a}\|_{op} \leq \frac{\eps}{d_{a}} \implies \|\nabla f\|^{2} = \sum_{a} d_{a} \|(\nabla f)_{a}\|_{F}^{2} \leq k \eps^{2}  \]
By \cref{thm:tensor-convexity} we have that $f$ is $1-o_{d}(1)$ strongly convex in norm $\|\cdot\|$, and further by \cref{lem:perturbation} $f$ is $\Omega(1)$ (for concreteness say $1/2$) strongly convex at any point $e^{Z}$ with $\forall a: \|Z_{a}\|_{op} \leq \delta \ll 1/k$. 
We can show that this region of strong convexity contains a large ball in the norm $\|\cdot\|$:
\[ \exists a: \|Z_{a}\|_{op} > \delta \implies \|Z\|^{2} \geq \frac{1}{d_{a}} \|Z_{a}\|_{F}^{2} \geq \frac{\delta^{2}}{d_{a}}  \]
Now to apply \cref{lem:convex-ball}, we need $\lambda \kappa > \eps$, where $\lambda$ is the strong convexity we can maintain in a $\kappa$-ball, and $\epsilon$ is the size of the gradient in the dual norm. We have shown above that we can maintian $\Omega(1)$-strong convexity in norm $\|\cdot\|$ in a ball of size $\Omega(1/\max_{a} \sqrt{d_{a}})$ again in norm $\|\cdot\|$. Therefore we need $\epsilon^{2} = \|\nabla f\|_{*}^{2} < O(1/\max_{a} d_{a})$. By concentration, to achieve this bound we need $nD \gg \frac{\max_{a} d_{a}^{2}}{\min_{a} 1/d_{a}} = \max_{a} d_{a}^{3}$. So applying \cref{lem:convex-ball} we get that the optimizer $\Theta$ is in a ball of size $O(\eps)$ in the norm $\|\cdot\|$. In other words, $\|\log \Theta\| = O(\eps)$. As $\|\Theta_a\|_{op} \leq c$ because $\Theta$ is in the $\kappa$ ball, we have $\|I - \Theta_a\|_F = O( \| \log \Theta_a\|_F) = O( \|\log \Theta\| \sqrt{d_a}) = O( \eps \sqrt{d_a}).$\end{proof}


\CF{
Let $\delta_a = \|Z_a\|_{op}$. Then $k \sum_a  \delta_a \leq k^{3/2} \max \sqrt{d_a} \|Z\|$. \cref{thm:tensor-convexity} tells us $f$ is $\Omega(1)$-strongly convex in a $\kappa:=\Omega(1/\sqrt{k^{3}\max d_a} )$ radius $\| \cdot \|$ ball about $I$ with inverse polynomial failure probability. By \cref{lemma:gradient-bound}, with failure probabilty at most $\sum_a \exp ( - N_a \eps^2)$, $\| \nabla f\|_* \leq \eps \sqrt{k}$. Provided $\eps = c/k^2 \max \sqrt{d_a}$, \cref{lem:convex-ball} shows there is an optimizer $\Theta$ of $f$ in a $\| \cdot \|$ ball of radius $\eps$. }
%As $\|Z_a\|_{op} \leq c$ on the $\kappa$ ball, we have $\|I - \Theta_a\|_F \leq \| \log \Theta_a\|_F \leq \|\log \Theta\|/\sqrt{d_a}$. }
...
\begin{definition}[Geodesic gradient and Hessian]
For $Q \in P$, the geodesic gradient $\nabla f(Q)$ is the unique element in $H$ such that 
$$\langle X, H \rangle = \partial_t f(\sqrt{Q}e^{Xt}\sqrt{Q})|_{t = 0}$$
for all $X \in H$. The Hessian $\nabla^2 f(Q)$ is the linear operator on $H$ such that 
$$\langle Y,  \nabla^2 f(Q) X \rangle = \partial_s \partial_t f(\sqrt{Q}e^{Xs + Yt}\sqrt{Q})|_{s,t = 0}$$
for all $X, Y \in H$. 
\end{definition}
...
defined by $\tr \rho(A \otimes I_{\bar{S}}) = \tr A \rho^S$ for all $A \in H.$
\end{definition}


\begin{fact}[Geodesic gradients and Hessians]
Let $\rho$ denote $\frac{\vec X \vec X^\dagger}{\|\vec X\|^2}$, and for $S \subset [k]$ let $\rho^S$ denote the partial trace \CF{should probably move the partial trace def elsewhere}
$$\tr_{\bar{S}} \rho \in \bigotimes_{i \in S} \PD_{d_i}.$$
The geodesic gradient $\nabla f := (\nabla_1 f, \dots, \nabla_k f) \in P $ at the identity is given by
\begin{align}
\nabla_a f &= \rho^a - \frac{1}{d_{a}} I_{a}
\end{align}
for $a \in [k]$. 
...
so that 
\[ \partial_{t=0} F(I_{\overline{a}} \otimes e^{tY}) = \tr  (I_{\overline{a}} \otimes e^{tY}  Y)\tilde{\rho}  |_{t=0} = \tr  (I_{\overline{a}} \otimes Y) \tilde{\rho} , \]
when $a\neq b$,
...
\[ \langle \nabla_a F, Z \rangle = \langle \tilde{\rho}^a, Z \rangle  \]
and 
\begin{align*} \langle Z, (\nabla^{2}_{aa}) F Z \rangle &=  \langle \tilde{\rho}^a, Z^{2} \rangle  \\
 \langle Y, (\nabla^{2} F) Z \rangle &= \langle \tilde{\rho}^{ab}, Z \otimes Y \rangle  \end{align*}

Now to calculate the same for $f$, note that $f = \log F - \sum_{a \in [k]} \frac1{d_a} \log\det(\Theta_a)$. Further note that $\nabla \log F = \frac{\nabla F}{F}$, $\nabla^{2} f = \frac{\nabla^{2} F}{F} - \frac{(\nabla F)(\nabla F)^{\dagger}}{F^{2}}$, and that the geodesic gradient of $\log \det \Theta_a$ is $I_{a}$ and the geodesic Hessian is zero. Plugging in the values of $F, \nabla F, \nabla^2 F$ from above completes the proof. \end{proof}
...
\begin{proof}[Proof of \cref{lemma:gradient-bound}]Let $N = n D/d_a$ and $d = d_a$. Again, $\nabla_a f = \rho^a - I_a/d_a$ where $\rho^a$ is distributed as $Y Y^\dagger/\|Y\|_F^2$, where $Y$ is a $d \times N$ matrix with independent Gaussian entries.
