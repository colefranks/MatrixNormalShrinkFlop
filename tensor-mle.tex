...
\CF{also confused. Why does $1/\sqrt{d_a d_b}$ become $1/d_a$ and $1/d_b$, resp?}
...
\begin{proof}[Rigorous proof of \cref{thm:tensor-convexity}]
We use the inequality $\begin{bmatrix} 0 & K \\ K^{*} & 0 \end{bmatrix} \succeq - \begin{bmatrix} A & 0 \\ 0 & B \end{bmatrix}$ for any $A,B \succeq 0$ such that $\|A^{-1/2} K B^{-1/2}\|_{op} \leq 1,$ which may be proved by computing Schur complements. 

Let $\underline{\nabla}^2 f$ denote the gradient in the Hilbert-Schmidt inner product, or $\underline{\nabla}^2_{ab} f = \frac{1}{d_a} \nabla_{ab}^2 f$. By \cref{eq:expansion-thing} (\CF{which needs to actually be proved}), with probability $\CF{good}$, the map $Z \to \tr_2 \rho^{(a,b)} (I \ot Z)$ has operator norm at most $\lambda/\sqrt{d_a d_b}$ restricted to traceless $Z$ and $\|\rho^{(a)} - I_{d_a}/{d_a}\|_{op} \leq \eps/d_a, \|\rho^{(b)} - I_{d_b}/d_b\|_{op} \leq \eps/d_b$. Thus for traceless $Z$ we have $|\tr Z \rho^{(b)}| \leq  \eps \|Z\|_F /\sqrt{d_b}$, and hence
$$(\tr Y \rho^{(a)})\bigl(\tr Z \rho^{(b)}\bigr) \leq \eps^2 \|Z\|_F\|Y\|_F /\sqrt{d_a d_b} .$$
Together, we have 
$\|\underline{\nabla}_{ab}^2 f \|_{op} \leq (\lambda + \eps^2)/\sqrt{d_a d_b}.$ Thus we may apply the inequality with $A = (\lambda + \eps^2) I_{d_a}/d_{a}$, $B = (\lambda + \eps^2)I_{d_b}/d_{b}$. For $a = 1, b = 2$, we have \begin{align*}
  \underline{\nabla}^2 f \succeq \begin{bmatrix}
  \underline{\nabla}_{11}^2 f - \frac{\lambda + \eps^2}{d_1} I_{d_1} & 0 & \hdots &  \underline{\nabla}_{1k}^2 f  \\
0 & \underline{\nabla}_{22}^2 f - \frac{\lambda + \eps^2}{d_2} I_{d_2} & \ddots & \vdots \\
\vdots & \ddots & \ddots & \vdots\\
  \underline{\nabla}_{k1}^2 f & \hdots &  \hdots & \underline{\nabla}_{kk}^2 f \\
