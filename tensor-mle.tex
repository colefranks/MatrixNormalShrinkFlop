...
If $f$ is $\lambda$-strongly convex at $I$, then at perturbation $\otimes_{a} e^{Z_{a}}$ bounded by $\forall a: \|Z_{a}\|_{op} \leq \delta \ll 1$, $f$ $\lambda - O(k\delta)$-strongly convex. 
\end{lemma}
...
\begin{fact}[Geodesic gradients]
Let $\rho$ denote $\frac{\vec X \vec X^\dagger}{\|\vec X\|^2}$, and for $S \subset [k]$ let $\rho^S$ denote the partial trace 
$$\tr_{\bar{S}} \rho \in \bigotimes_{i \in S} \PD(d_i).$$
...
\begin{align}\langle Y, (\nabla^{2}_{aa} f) Y \rangle &=  \langle \rho^{a}, Y^{2} \rangle - \left\langle \nabla_a f, Y \right\rangle^{2} \\
\langle Z, (\nabla^{2}_{ab} f) Y \rangle &= \langle \rho^{ab}, Z \otimes Y \rangle  - \langle \nabla_a f, Y \rangle \langle \nabla_b f, Z \rangle
\end{align}
for $a \neq b \in [k]$, $Y \in \Herm(d_a), Z \in \Herm(d_b)$. 
\end{fact}
\begin{proof}
...
and \CF{does the next line make sense when $a=b$?}
\begin{align*}\partial_{s=0} \partial_{t=0} F(I_{\overline{ab}} \otimes e^{sY} \otimes e^{tZ}) &=  \partial_{s=0} \tr  (I_{\overline{b}} \otimes e^{sY}) (I_{\overline{a}} \otimes Z) \tilde{\rho} \\
...
Now to calculate the same for $f$, note that $f = \log F - \sum_{a \in [k]} \frac1{d_a} \log\det(\Theta_a)$. Further note that $\nabla \log F = \frac{\nabla F}{F}$, $\nabla^{2} f = \frac{\nabla^{2} F}{F} - \frac{(\nabla F)(\nabla F)^{\dagger}}{F^{2}}$, and that the geodesic gradient of $\log \det \Theta_a$ is $I_{a}$ and the geodesic Hessian is zero. This completes the proof. \end{proof}
...
\AR{This mostly depends on notation, so I'll leave this for now and you guys can decide what the right notation is. }\CF{just use \cref{eq:projective-likelihood} as the definition, we can fix the notation later as long as we are proving the right thing.}
...
\]
We can rewrite the Hessian using shorthand $\{M_{a}\}$ for the diagonal blocks and $\{M_{ab}\}$ for off-diagonal blocks:
\[ \nabla^{2} = \sum_{a} E_{aa} \otimes M_{a} + \sum_{a \neq b} E_{ab} \otimes M_{ab}  \]
