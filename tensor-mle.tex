...
There are absolute constants $c, C$ such that the following holds for all $d_1 \leq d_2$. If the entries of the $n$ Kraus operators of $\Phi$ are drawn from standard Gaussians, then $\Phi$ is $C\sqrt{d_1/d_2}$-balanced and a $c - C (d_1/d_2)$ quantum expander with probability $e^{ - c n d_1}$ provided $n \geq C\max\{\log (d_1 d_2),  d_2/d_1\}$. 
\end{theorem}
...
$$\ch(\Phi):=\min_{\Pi_1, \Pi_2: \rk \Pi_1 \leq d_1/2} \phi(\Pi_1,\Pi_2)$$
...
$$\phi(\Pi_1,\Pi_2) := \frac{\cut(\Pi_1, \Pi_2)}{\min\{\vol(\Pi_1,\Pi_2), \vol(I_{d_1} - \Pi_1, I_{d_2} - \Pi_2)\}}$$
where 
...
\begin{itemize}
\item if $F_2 \geq (4/9) n d_1 d_2$, then \cref{eq:vol} holds with probability at least $1 - e^{-c n d_1 d_2}$. 
\item else, \cref{eq:cut} holds with probability at least $1 - e^{-c F_2}$. 
\end{itemize}
\end{lemma}


\begin{proof}
Recall from \cref{fact:chi} that, $\cut(\Pi_1, \Pi_2), \vol(\Pi_1, \Pi_2), \vol(I_{d_1}, I_{d_2})$ are jointly distributed as $X, X + 2Y, 2X + 2Y + 2Z$ for $X, Y, Z$ independent $\chi^2$ random variables with $F_1, F_2, F_3$ degrees of freedom, respectively. Thus it is enough to show that if
\begin{itemize}
\item If $nr_1 r_2 \geq (4/9) n d_1 d_2$, then with probability $1 - e^{- c n d_1 d_2}$ we have $Y > Z$, and 
\item if $nr_1 r_2 \leq (2/3) n d_1 d_2$, then with probability $1 - e^{- cF_1}$ we have $X \geq (2/3) F_1$ and $X + 2Y \leq (4/3) (F_1 + 2 F_2).$ 
\end{itemize}
Both follow from standard results for concentration of $\chi^2$ random variables \cite{W19}. To prove the first item, first note that $F_1 + 2 F_2 \geq (4/3)(F_1 + F_2 + F_3)$, because 
\begin{align*}
...
\end{align*}
In particular, $F_2 \geq (2/3)(F_2 + F_3)$. Thus, with probability $1 - e^{- c F_2}$, $Y \geq (5/9) (F_2 + F_3)$ and $Y + Z \leq (10/9) (F _2 + F_3),$ so $Y > Z$ with probability $1 - e^{- c F_2} \geq 1 - e^{- c n d_1 d_2}$. The second item is straightfoward. 
\end{proof}
...
\CF{finish proof of \cref{thm:operator-cheeger}; the plan is to use that when $r_1 r_2 < (4/9) d_1 d_2$, the size of the net is $O(\log \delta)$ times the degrees of freedom when $n = 1$. On the other hand if $r_1 r_2 < (4/9) d_1 d_2$ then the failure probability is on the order of $e^{-n d_1 d_2}$, which is also small enough. So making $n = O(\log \delta)$ suffices; finally we need to show that $\cut$ and $\vol$ are lipschitz-continuous enough that we may choose $\delta = poly(d_1,d_2)^{-1}$. Both steps seem relatively straightforward.}
