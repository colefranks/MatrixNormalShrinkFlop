...
\end{align}
We may also consider the ``second sample moments'' of an individual coordinate.
...
$1 - 2e^{-\eps^2 nD/8} - \sum_{a=1}^k 2 e^{-\eps^2 N_a/2}$:
\MW{Replace $\eps$ by $\eps/9$.}
\begin{align*}
...
&=\abs{\tr \rho - 1} \leq \eps,
\end{align*}
with failure probability at most $2e^{-\eps^2 nD/8} + \sum_{a=1}^k 2 e^{-\eps^2 N_a/2}$.
\end{proof}
...
\MW{CHECK}
There are constants $C,c>0$ such that the following holds. For $n \geq C k^2 d_1^2/D$, with probability at least $1 - k^2 \left(\frac {\sqrt{nD}}{kd_1}\right)^{ - \Omega(d_1)}$, $f$ is $1/2$-strongly convex at any point $Z \in \SPD$ such that $\sum_a \|\log Z_a\|_{op} \leq c$ for all $a \in [k]$.
\end{theorem}
...
Similarly as for the gradient, we can compute the components of the Hessian in terms of a partial trace.
...
Then $\rho^{(ab)}$ is a positive semidefinite $d_ad_b \times d_ad_b$ matrix, which satisfies the defining property that
...
\begin{proof}
  ...
  \end{align*}
  using \cref{eq:two body partial trace duality}.
...
\MW{Simplify, don't talk about a general $\omega$.}
...
In our proof of strong convexity, we will show that strong convexity follows if all the completely positive maps $\Phi^{(ab)}$ arising from the partial traces $\rho^{(ab)}$ are good \emph{quantum expanders}.
...
\begin{align*}
  \norm*{\frac{\Phi(I_{d_1})}{\tr \Phi(I_{d_1})} - \frac{I_{d_2}}{d_2}}_{\op} \leq \frac\eps{d_2}
\quad\text{and}\quad
  \norm*{\frac{\Phi^*(I_{d_2})}{\tr \Phi(I_{d_2})} - \frac{I_{d_1}}{d_1}}_{\op} \leq \frac\eps{d_1}.
\end{align*}
...
\leq \lambda \frac{\tr \Phi(I_{d_1})}{\sqrt{d_1d_2}}
\end{align*}
A $(0, \lambda)$-quantum expander is called a \emph{$\lambda$-quantum expander}.
\end{definition}

\MW{Could already at this point translate the conditions into ones involving $\rho^{(ab)}$.}

Quantum expanders play an important role in quantum information theory and quantum computation \TODO{cite}.
\MW{There, typically $\Phi$ is trace preserving (a quantum channel) and $d_1=d_2$, so the latter condition is just $\norm{\Phi}_0 \leq \lambda$.}\CF{doesn't it also have to be unital?}\MW{Yes, definitely, I was just talking about the inequality involving $\lambda$.}
...
\MW{What does the $\Omega$ depend on? Universal constant?}\CF{yes, isn't that the usual assumption?}\MW{Not sure, since on the other hand we write ``There exists an absolute constant $C>0$.'' Or is there some subtlety that I am missing? Happy to use $O$, $\Omega$ consistently as you say (and to say so in the Notations subsection).}

\begin{theorem}[\cite{pisier2012grothendieck,P14}]\label{thm:hess-pisier}
There is an absolute constant $C>0$ such that the following holds. Let $A_1,\dots,A_N$ be independent $d_2\times d_1$ random matrices with independent standard Gaussian entries.
Then, for every $t >0$, with probability at least~$1 - t^{-\Omega(d_{1} + d_{2})}$, the completely positive map $\Phi_A$, defined as in \cref{eq:kraus}, satisfies
\begin{align*}
  \norm \Phi_A \leq Ct^{2} \sqrt N \left( d_1 + d_2 \right). \text{\MW{Should this be $\sqrt{d_1} + \sqrt{d_2}$?}}
\end{align*}
\end{theorem}
...
When the samples $x=(x_1,\dots,x_n)$ are independent standard Gaussians in $\R^D$, then~$\rho^{(ab)}$ is distributed as $\frac1{nD} ZZ^T$, where $Z$ is a random $d_a d_b \times \frac{nD}{d_ad_b}$ matrix with independent standard Gaussian entries, as follows from the discussion above \cref{eq:two body partial trace duality}.
Therefore, the random completely positive maps $\Phi^{(ab)}$ we are interested in have the same distribution as~$\frac1{nD}\Phi_A$, where the $A_1,\dots,A_N$ are independent $d_a \times d_b$ standard Gaussian matrices and~$N=\frac{nD}{d_ad_b}$.
...
For the latter, observe that by \cref{lem:gradient},
\begin{align*}
  \norm*{\frac{\Phi^{(ab)}(I_{d_b})}{\tr \Phi^{(ab)}(I_{d_b})} - \frac{I_{d_a}}{d_a}}_{\op}
= \frac1{\tr\rho} \norm*{\rho^{(a)} - \frac{\tr \rho}{d_a} I_{d_a}}_{\op}
...
\end{align*}
and similarly for the adjoint, hence the completely positive maps $\Phi^{(ab)}$ are $\eps$-doubly balanced if and only if, for all $a\in[k]$,
\begin{align*}
  % \frac1{1 + \nabla_0 f_x} \frac1{\sqrt{d_a}} \norm*{\nabla_a f_x}_{\op} \leq \frac\eps{d_a}
  \sqrt{d_a} \norm*{\nabla_a f_x}_{\op} \leq \left( 1 + \nabla_0 f_x \right) \eps.
\end{align*}
Thus, double balancedness can be controlled using the gradient bounds in \cref{prop:gradient-bound}.
...
\MW{CHECK}
There is an absolute constant $C$ such that the following holds. Let $\rv = (\rv_1,\dots,\rv_n)$ be independent standard Gaussian random variables in $\R^D$, where the number of samples $n$ is at least $C k^2 d_{\max}^2/D$ for $k \geq 2$. Then we have
