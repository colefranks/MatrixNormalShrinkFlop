...
\end{align}
It is known (Lemma A.3 in \cite{FM20}) that $(\eps, \lambda)$-quantum expansion implies spectral gap $(\lambda - O(\eps))$ , so the notions are equivalent for our purposes. 

%$\sigma_1(\Phi) = \langle I_{d_1}/\sqrt{d_1}, \Phi(I_{d_1}/\sqrt{d_2}) \rangle$


\begin{theorem}[\CF{KLR}]\label{thm:klr} Let $d_1 \leq d_2$.
...
\cref{thm:klr}, along with what we've shown so far, already implies a preliminary version. In the proof of \cref{thm:tensor-convexity}, we've already shown that $\Phi$ is an $(\eps \sqrt{d_2/n d_1}, 1 - c)$-quantum expander with failure probability 
...
There are absolute constants $c, C$ such that the following holds for all $d_1 \leq d_2$. Suppose $n \geq C (d_2/d_1) \max\{\log (d_2/d_1),  \eps^{-2}\} $. Then $\Phi$ is an $(\eps \sqrt{d_2/n d_1}, 1 - c)$-quantum expander, with failure probability $O(e^{ - \Omega( d_2 \eps^2)}).$ \CF{check $d_2$ vs $d_1$.}
\end{theorem}
...
This also improves our result on strong convexity. By \cref{lem:expansion-convexity}, and the consequent improvement to \cref{thm:ball-convexity}, we have the following.
...
We now discuss the concentration bound. For $\Phi$ as in \cref{eq:matrix-normal-operator}, we can right-multiply $X_i$ by the result of one step of the flip-flop algorithm on the larger factor $\Theta_2$; the result gives rise to a new operator $\tilde{\Phi}$ which satisfies 
...
\begin{corollary}[Akshay corollary]\label{cor:flipflop-concentration}
If $X$ is an $n$-tuple of i.i.d. standard Gaussians on $\R^{d_1}\ot \R^{d_2}$ and $n \geq d_2/d_1$, then for $\tilde{\Phi}$ as in \cref{eq:one-step} we have
...
\end{corollary}
The proof of this lemma uses a result of \CF{cite} on the inner product of two random projections, combined with a standard net argument. With this bound in hand, we may now prove \cref{thm:matrix-normal}.

\CF{I have screwed something up with the $1 - \lambda$'s in the other places...}
...
$$n \geq C (d_2/d_1) \max\{\log (d_2/d_1),  \eps_0^{-2}\} $$
 with probability $1 - O(e^{ - \Omega( d_1 \eps_0^2)})$ the operator $\Phi$ is a $(\eps_0  \sqrt{{d_2}/{n d_1}},1-c)$-quantum expander. By \cref{cor:flipflop-concentration}, $\tilde{\Phi}$ is $\eps_0 \sqrt{{d_1}/{n d_2}}$-balanced with probability $1 - O(e^{ - \Omega( d_1 \eps_0^2)});$ by the union bound both events occur with probability $1 - O(e^{ - \Omega( d_1 \eps_0^2)});$ condition on both events.
 
We now use a lemma relating the quantum expansion of $\tilde{\Phi}$ and $\Phi$; this is analogous to \cref{convexRobustness} guaranteeing the strong convexity of $f$ in a neighborhood of the origin. The operator $\tilde{\Phi}$ is a \emph{scaling} of $\Phi$, that is $\tilde{\Phi}(X) = g^\dagger \Phi( h X h^\dagger) g$ for some $g \in \GL(d_1), h \in \GL(d_2)$. In our case case $h = \frac{1}{\sqrt{d_2}}\Phi^*(I_{d_2})^{-1/2}$ and $g = I_{d_1}$. By Lemma 4.4 in \cite{FM20}, if $\|h^\dagger h - I_{d_1}\|_{op} \leq \delta$,  then $\tilde{\Phi}$ is an $(\eps \sqrt{d_2/n d_1} + O(\delta), 1 - c + O(\delta))$-quantum expander. Fortunately, $h^\dagger h$ is bounded by the balancedness of $\Phi$: $h^\dagger h - I = (d_2 \Phi^*(I_{d_1}))^{-1} - I_{d_2}$. By the balancedness of $\Phi$ we already know that $\|d_2 \Phi^*(I_{d_1}) - I_{d_2}\|_{op} \leq \eps_0 \sqrt{d_2/n d_1}$; provided $\eps_0 \sqrt{d_2/nd_1} \leq c' < 1$ we have $\|(d_2 \Phi^*(I_{d_1}))^{-1} - I_{d_2}\|_{op} = O( \eps_0 \sqrt{d_2/n d_1})$. Thus $\tilde{\Phi}$ is an $(\eps_0 \sqrt{d_1 / n d_2}, 1 - c/2)$-quantum expander provided $c'$ is small enough compared to $c$.
 
By our choice of $n$, we have $\eps \sqrt{d_1/nd_2} \leq c^2/\log d_1$ if we take $\eps_0 = \eps/\log(d_1)$ for $\eps \leq c^2$. By \cref{thm:klr}, the MLE $\Theta'_1, \Theta'_2$ for the tuple $Y_i = \frac{1}{d_1} X_i \Phi^*(I_{d_1})^{-1/2}$ satisfies 
...
\end{align*} where the penultimate inequality is by the approximate triangle inequality and approximate symmetry for the relative spectral error \CF{cite}. Our choice $\eps_0 = \eps/\log(d_1)$ gives the failure probability $e^{- \Omega(\eps^2 d_1)/\log^2 d_1}$. By setting the constant $C$ in the statement of the theorem large enough, all the constraints will be satisfied.
\end{proof}
...
%-----------------------------------------------------------------------------
$ $
\CF{what follows will be condensed to cite-able results to prove \cref{thm:matrix-normal}}
