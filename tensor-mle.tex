...
\MW{MOVE OVER TO SECTION 2 WHEN DONE.}
There are universal constants $C,c>0$ such that the following holds.
Suppose $\delta \leq \smash{\frac c {\sqrt{(k+1)d_{\max}}}}$ and that the number of samples satisfies $n \geq C k \frac{d_{\max}^2}{D \delta^2}$.
...
\begin{align*}
  D_F(\htheta_a\Vert\Theta_a) &= O(\sqrt{d_a} \delta) \text{ for all } a\in[k], \\
  D_F(\htheta\Vert\Theta) &= O(\sqrt D \delta),
\end{align*}
with probability at least
\begin{align*}
...
\end{align*}
\MW{And now reparameterize $\delta$ so that $\Theta_{\max}$ has the right decay and $\eps$ takes a constant range.}
\end{proof}
