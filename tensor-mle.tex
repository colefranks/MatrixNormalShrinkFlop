...
There is a constant $c>0$ such that the following holds. If $f_{\samp}$ is $\alpha$-strongly convex at $I$; \CF{use english, not $\forall$} $\forall a: \|d_{a} \rho_{\samp}^{(a)} - Tr[\rho_{\samp}^{(a)}] I_{a}\|_{op} = \|\sqrt{d_{a}} \nabla_{a} f_{\samp}\|_{op} \leq c$; and $\sum_{a} \|\delta_{a}\|_{op} \leq c$: then $f$ at $e^{\delta} := \otimes_{a} e^{\delta_{a}}$ is $\geq (\alpha - k C \|\delta\|_{op})$-strongly convex for some universal constant $C$. 
\end{lemma}
...
For input $\samp\CF{\in?}$ and perturbation \CF{$\delta \in ?$ such that }  $\|\delta\|_{op} \leq \frac{1}{20}$ \CF{if} $\samp' := (e^{\delta})_{a} \samp$ \CF{then we have}
