...
Consider any estimator $\htheta$ for the precision $\theta$. Then 
\begin{align*}
\sup_{\Theta \in \PD(d)} \E[ d_F(\htheta||\Theta)^2] &= \Omega(\frac{d^2}{n}).\\
\text{ and } \sup_{\Theta \in \PD(d)} \E[ d_{op}(\htheta||\Theta)^2] &= \Omega(\frac{d}{n}).
\end{align*}
\end{prop}
The proof uses Fano's method, summarized by the following lemma. \CF{cite something that isn't lecture notes??}
\begin{lemma}[Local Fano's method]
Let $P_i$ for $i \in [n]$ be a finite set of probability distributions on a set $\mathcal X$, and let $T: \mathcal X \to [n]$ be an estimator for $i$. Then 
$$ \sup_i \Pr_{x \sim P_i}[T(x) \neq i] \geq 1 - \frac{ - \log 2 + \frac{1}{n^2}\sum_{i,j \in [n]} D_{KL}(P_i|| P_j)}{\log n}.$$
\end{lemma}
