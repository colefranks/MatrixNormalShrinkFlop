...
and let $0<\eps<1$.
Suppose $nD \geq \left(\frac{d_{\max}}{\eps}\right)^2$.
...
Let $\Phi\colon\mat(d_a) \to \mat(d_b)$ be a completely positive map.
...
\leq \lambda \frac{\tr \Phi(I_{d_a})}{\sqrt{d_ad_b}}
\end{align}
...
\MW{Could already at this point translate the conditions into ones involving $\rho^{(ab)}$, $\rho^{(a)}$, $\rho^{(b)}$. But maybe ok like it is\dots}
...
\begin{align*}
  \norm \Phi_A \leq Ct^{2} \sqrt N \left( d_a + d_b \right). \text{\MW{Should this be $\sqrt{d_a} + \sqrt{d_b}$?}}
\end{align*}
\end{theorem}
...
Let $x = (x_1,\dots,x_n)$ consist of independent standard Gaussian random variables in~$\R^D$, where \MW{CHECK}
\begin{align*}
  n \geq C k^2 d_{\max}^2/D
\end{align*}
for $k\geq2$.
Then with probability $1 - k^2 \left(\frac {kd_{\min}}{\sqrt{nD}}\right)^{ \Omega(d_{\min})}$ we have
\begin{align*}
