...
\TODO{
\begin{enumerate}
\item \CF{see where high sparsity causes the Gemini etc to breaks down.}
\item \CF{make a "learning curve" showing all 3 estimators as a function of samples.}
\end{enumerate} }
...
In this subsection we examine how the number of samples affects the error. In \cref{fig:lc} we see that for $1$ sample, the Gemini and regularized Flip-Flop estimators are comparable. The error of the unregularized Flip-flop estimator is much higher. As the number of samples increases, however, the regularized Flip-flop estimator performs better, and for even more samples the unregularized Flip-Flop e
