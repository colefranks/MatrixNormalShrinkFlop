...
\MW{I think these are unit speed if $\norm{H}_F^2=1/D$. Does anything become nicer if we incorporate this factor below, e.g.\ in the definition of strong convexity?}
\end{definition}
...
Assuming the function is twice differentiable, this holds if $\partial^2_t f(\exp_\Theta(tH)) \geq \lambda \norm{H}_F^2$ for all~$H\in\H$, where $\norm{H}_F^2 := \sum_{a=1}^k \norm{H_a}_F^2$.
\end{definition}
