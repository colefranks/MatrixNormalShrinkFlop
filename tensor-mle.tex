...
Recall that if $f_\samp$ is minimized at $\widehat{\Theta}$, then $\ell_\samp$ is minimized at $\Theta':= n D e^{ - f_\samp(\widehat{\Theta})} \widehat{\Theta}:=\alpha  \widehat{\Theta}$. Thus $\Theta'_a := \alpha^{1/k}\widehat{\Theta}_a$. By our bound on $f_\samp(\Theta)$, $\alpha \in (1 \pm \delta)$ with probability $1 - q(\delta)$. Thus with probability $1 - q(\delta) + p(\eps)$, the distance of the MLE from the truth is $d(\Theta'_a;I) \in d( (1 \pm \delta)^{1/k}\widehat{\Theta}; I)$. By the approximate triangle inequality \CF{cite}, $d( (1 \pm \delta)^{1/k} \widehat{\Theta}_a; I) = O(d(\widehat{\Theta}; I) + d((1 \pm \delta)^{1/k}I; I))$. If $d = d_F$, the latter term is $O(\delta \sqrt{d_a}/k)$; if $d = d_{op}$ the latter is $O(\delta/k)$. \CF{how large are we thinking delta is?}
\end{proof}
