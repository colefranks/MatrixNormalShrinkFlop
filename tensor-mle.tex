...
\begin{align*}
  B_r(P) = \{ \exp_P(H) : \norm H_F \leq r \}.
\end{align*}
\MW{Balls are convex in nonpositive curvature, right?}
...
\MW{The following definitions are buggy; the zero'th order definition of convexity does not make sense pointwise. Define it in a convex set? Or take the differential definition as a pointwise one?}
...
Further assume the gradient is bounded as $\norm{\nabla f(P)}_F \leq \eps < \lambda  r / 2$.
Then the sublevel set $\{Q \in \P : f(Q) \leq f(P)\}$ is contained in the ball $B_{\eps/\lambda}(P) \subset B_r(P)$ \MW{It looks like we don't prove the smaller inclusion!}, $f$ has a unique minimizer $\cramped{\hat P}$ which is contained in $B_{\eps/\lambda}(P)$, and
