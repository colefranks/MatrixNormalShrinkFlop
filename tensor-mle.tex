...
\item $\PD_d$ for $d \times d$ pd's, $\Herm_d$ for $d \times d$ Hermitians. I don't think stats folks use this, I just really like them. Other suggestions welcome. $\Theta$ for big tensor product psd, $\Theta_a$ for individual psd's. \MW{$\operatorname{Sym}_d$ for real symmetric matrices?} \MW{$\Theta$ looks like big O notation}
...
\MW{Does this mean that $\tilde L$ is the log-likelihood for the family of pushforwards of normal distributions on $\R^n$ onto the unit sphere / projective space? `Projective likelihood function' sounds fancyful :)}
\end{remark}
...
\MW{The `bare' result would be $\frac D{d_i}$, right? Hwo do you renormalize it away?}
...
\MW{Not sure if those should be called `Riemannian' gradient and Hessian.}
