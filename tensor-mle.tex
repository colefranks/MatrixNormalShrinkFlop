...
\CF{I've shifted away from rates and towards whp bounds, which we need to do unless we supply a tail bound.}
...
This is a natural dissimilarity measure on Gaussians because when any of~$D_F$, total variation distance, square of the KL-divergence, or Fisher-Rao distance is at most a small constant then they are all on the same order \CF{check} \citep{barsov1987estimates} \CF{mention that this is also called stein's loss and there is work (Ledoit-Wolf) on estimating the covariance matrix in Stein's loss)}.
...
\item In the tensor normal model, for some $n_0 = O_k( \max\{d_i^3\}/ \prod_i d_i)$, if $n \geq n_0$ then the MLE for the precision matrix $\Theta$ has error $O_k( \frac{d_{\max}}{\sqrt{n}} )$ in $D_F$ \MW{with probability \dots}. \MW{$O_k$ notation might not be clear} We also give a tight bound for the error of the largest Kronecker factor $\Theta_i$.
...
We extend these definitions to tuples $A=(A_0;A_1,\dots,A_k)$, where~$A_0\in\R$ and the $A_a$ for $a\in[k]$ are matrices and denote them by the same symbol, i.e., $\norm{A}_F = (\abs{A_0}^2 + \sum_{a=1}^k \norm{A_a}_F^2)^{1/2}$ and similarly for the inner product. For functions $f,g:S \to \R$ for any set $S$, we say $f = O(g)$ if there is a constant $C > 0$ independent of $x$ such that $f(x) \leq C g(x)$ for all $x \in S$, and similarly $f = \Omega(g)$ if there is a constant $c > 0$ independent of $x$ such that $f(x) \geq c g(x)$ for all $x \in S$. 
...
\end{align*}
with probability at least  $1 - O(e^{ - \Omega( d_1 \eps^2)})$. \CF{ I commented out your convention, because what I proved uses the convention above the lemma.}}
...
\RMO{Preprocessing the input to assume that the initial trace is 1, so that we get $\nabla_0 f_x  = 0$. This simplifies our descent lemma.}
...
\textbf{Algorithm}:
\begin{enumerate}
\item\label{it:flip-flop preprocessing} Preprocess the input by scaling sample vectors $x_i$ by $\dfrac{1}{\tr \rho} \cdot x_i$, where $\rho = \dfrac{1}{nD} \cdot \sum_{i=1}^n x_i x_i^\dagger$
...
The following proposition shows that the trace part of the log-likelihood remains unchanged by the flip-flop algorithm.
Thus, the proposition below proves to us that $\nabla_0 f_x(\Theta)$ will remain unchanged at each iteration.

\begin{prop}[Trace Invariance]
	Let $\Theta$ and $\Upsilon$ be successive scalings produced by the flip-flop algorithm, and let $\rho_\Theta := \Theta^{1/2} \cdot \sum_{i=1}^n x_i x_i^\dagger \cdot \Theta^{1/2}$ and $\rho_\Upsilon := \Upsilon^{1/2} \cdot \sum_{i=1}^n x_i x_i^\dagger \cdot \Upsilon^{1/2}$. 
	Then $\tr \rho_\Theta = \tr \rho_\Upsilon$.
\end{prop}

\begin{proof}
	Since $\Upsilon$ is the successive scaling from $\Theta$, we have that
	there is $a \in [k]$ such that
	$$\Upsilon_a = \dfrac{\tr \rho_\Theta}{d_a} \cdot \Theta_a^{1/2} (\rho_\Theta^{(a)})^{-1} \Theta_a^{1/2} $$
	...
	\begin{align*}
		\tr \rho_\Upsilon 
		&= \tr\left[ \Upsilon^{1/2} \sum_{i=1}^n x_i x_i^\dagger \cdot \Upsilon^{1/2} \right] 
		= \tr\left[ \Upsilon \sum_{i=1}^n x_i x_i^\dagger \right] \\
		&= \tr\left[ \dfrac{\tr \rho_\Theta}{d_a} \cdot \Theta^{1/2} E_{(a)} \Theta^{1/2} \cdot \sum_{i=1}^n x_i x_i^\dagger \right] = \dfrac{\tr \rho_\Theta}{d_a} \cdot \tr\left[ E_{(a)} \rho_\Theta \right] \\
		&= \dfrac{\tr \rho_\Theta}{d_a} \cdot \tr\left[ (\rho_\Theta^{(a)})^{-1} \rho_\Theta^{(a)} \right] = \tr \rho_\Theta
	\end{align*}
\end{proof}

\RMO{Assuming trace is one for this next lemma - otherwise it gets a little ugly in the end.}
...
\begin{proof}
	...
	As $\Upsilon$ is the successive scaling, and we are assuming that $\tr \rho =1$, we have that $\Upsilon_b = \Theta_b$ when $b \neq a$ and
...
\begin{lemma}[Fast Convergence from Initial Conditions]\label{lem:fast-convergence-initial-generic}
	...
	\begin{enumerate}
		\item $|\nabla_0 f_x(I_D)| \leq 1$
		\item $\norm{\nabla f_x (I_D)}_F \leq \dfrac{r}{4} \leq 1$
...
\begin{lemma}[Initial Conditions]\label{lem:tensor-initial-conditions}
	...
	\begin{enumerate}
		\item $|\nabla_0 f_x(I)| \leq \frac{\gamma}{100 k \dmax^{1/2}}$
...
\begin{proof}
	...
	the first two conditions will fail to hold.

	Letting $\Gamma = \max\{350/\gamma^2, C \}$, having $n \geq \Gamma k^2 \dmax^3/D$ samples gives a sample upper bound that holds for both situations above.
...
\begin{theorem}[Restatement of \cref{thm:tensor-flipflop}]
	...
	$$ D_F(\htheta_a \ || \ \otheta_a) \leq \eps $$
	in $O(k \log(k \dmax/\eps))$ iterations with probability at least
...
We are now ready to state the matrix flip-flop algorithm and prove its fast convergence to the MLE. The proof strategy of this section is similar to the tensor normal model case, except that now we will bound the distance to the MLE in the operator norm. \RMO{I will write more stuff here later}
...
\textbf{Algorithm}:
\begin{enumerate}
\item Preprocess input by scaling sample vectors $x_i$ to become $\dfrac{1}{\tr \rho} \cdot x_i$, where $\displaystyle \rho = \dfrac{1}{n d_1 d_2} \cdot \sum_{i=1}^n x_i x_i^\dagger$ 
...
\item Otherwise, set $\otheta_a \leftarrow \dfrac{\tr \rho}{d_a} \cdot \otheta_a^{1/2} \cdot (\rho^{(a)})^{-1} \cdot \otheta_a^{1/2}$, where $\rho = \otheta^{1/2} \cdot \left( \sum_{i=1}^n x_i x_i^\dagger \right) \cdot \otheta^{1/2}$.
\end{itemize}
\end{enumerate}
\caption{Matrix flip-flop algorithm}\label{alg:flip-flop matrix}
\end{Algorithm}

In the same spirit as in the tensor normal model case, all we need for \cref{alg:flip-flop matrix} to be correct is for our initial samples to satisfy the initial conditions of \cref{lem:fast-convergence-initial-generic}.

\begin{lemma}[Initial Conditions Matrix Normal Model]\label{lem:matrix-normal-initial-conditions}
There exist absolute constants $\Gamma, \gamma > 0$ such that the following holds.
	When the number of samples $n \geq TODO$, with probability at least $TODO$ we have that the following conditions hold:
	\begin{enumerate}
		\item $|\nabla_0 f_x(I)| \leq \frac{\gamma}{100 k \dmax^{1/2}}$
		\item $\norm{\nabla f_x(I)}_F \leq \dfrac{\gamma}{4 \sqrt{(k+1)\dmax}}$ 
		\item $f_x$ is $\frac{1}{2}$-strongly geodesically convex at 
...
Let $(\widehat{\Theta}_1,\widehat{\Theta}_2)$ denote the MLE for $(\Theta_1,\Theta_2)$.
Given $n \geq \TODO{}$ samples of the matrix normal model, the flip-flop algorithm computes $(\overline{\Theta}_1,\overline{\Theta}_2)$ with
\begin{align*}
...
Here we briefly recall and, for completeness, prove well-known lower bounds on the rates for estimating the precision matrix in the relative Frobenius and operator error from independent samples of a Gaussian. The lower bounds follow from Fano's method and the relationship between the relative Frobenius error and the relative entropy (which is proportional to Stein's loss). 
%\CF{define minimax rate}
\begin{prop}[Relative Frobenius error]\label{prp:standard-lower}
There is $c > 0$ such that the following holds. Let $X = X_{\Theta} \in \R^{dn}$ denote $n$ independent samples from a Gaussian with precision matrix $\Theta$. Consider any estimator $\htheta = \htheta(X)$ for the precision $\Theta$. 
\begin{enumerate}
\item 
\begin{align}
\sup_{\Theta \in \PD(d)} \E[ d_F(\htheta||\Theta)^2] = \Omega\left( \min \left\{\frac{d^2}{n},1\right\} \right) .\label{eq:df-lower}
\end{align}
\item 
 \begin{align}
\sup_{\Theta \in \PD(d)} \E[ d_{op}(\htheta||\Theta)^2] = \Omega\left( \min \left\{\frac{d}{n},1\right\} \right). \label{eq:dop-lower}
\end{align}
\end{enumerate}
\end{prop}
The proof uses the local Fano's method, summarized by the following lemma. \CF{cite something that isn't lecture notes??}
\begin{lemma}[Local Fano's method]
...
$$ \max_{i\in [m]} \Pr_{x \sim P_i}[T(x) \neq i] \geq 1 - \frac{ - \log 2 + \frac{1}{m^2}\sum_{i,j \in [m]} D_{KL}(P_i|| P_j)}{\log m}.$$
\end{lemma}

\begin{proof}[Proof of \cref{prp:standard-lower}]
We begin with \cref{eq:df-lower}. We begin by the standard reduction from estimation to testing; let $V_0$ be a $1$-packing of the Frobenius ball $B$ of radius $1$ in the $d\times d$ symmetric matrices, i.e. $B = \{A: A \text{ Symmetric}, \|A\|_F \leq 1\}$ of cardinality $m \geq 2^{d(d+1)/2}$, as guaranteed by the Gilbert-Varshamov bound. Let $V = I_d + \delta V_0 = \{I_d + \delta v: v \in V_0\}$, and write $V = \{\Theta_1, \dots, \Theta_m\}$. Assume $\delta \leq 1/2$, and let $P_i$ to be the distribution of $X(\Theta_i)$ on $\R^{dn}$, i.e $\mathcal{N}(0, \Theta^{-1}_i)^{\ot n}$. Define the estimator $T$ by 
$$T(x) = \arg\min_{i \in [m]} \|\Theta_i - \htheta(x)\|_F,$$
so that $\Pr[T(X) = i] \geq \Pr[\|\htheta -  \Theta_i\|_F \leq \delta/2]$ because $V$ is $\delta$-separated. In order to apply the local Fano's method, we use the well-known fact that $D_{KL}(P_i|| P_j) = n D_{KL}(\mathcal{N}(0, \Theta_i^{-1}), \mathcal{N}(0, \Theta_j^{-1}) = O(nD_{F}(\Theta_j || \Theta_i)^2)$ when $\Theta_i^{-1}\Theta_j$ has eigenvalues uniformly bounded away from zero \CF{cite barsanov}. This condition holds because $(I_d/2 \preceq \Theta_j, \Theta_j \preceq 3I_d/2)$ for $i,j \in [m]$ by our assumption that $\delta \leq 1/2$. The bound of \CF{cite barsanov} can be seen explicitly from the formula 
$$D_{KL}(P_i|| P_j) = \frac{1}{2} \tr \Theta_i^{-1} \Theta_j - \frac{1}{2}\log\det(\Theta_i^{-1} \Theta_j) - \frac{d}{2} = \frac{1}{2} \sum_{i =1}^d (\lambda_i -1 -  \log \lambda_i)$$
 where $\lambda_i \in [1/3, 3]$ are the eigenvalues of $\Theta_i^{-1} \Theta_j$ (or, equivalently, those of $\Theta_i^{-1/2} \Theta_j \Theta_i^{-1/2}$), and observing that $\lambda_i - 1 -\log\lambda_i = O( (\lambda_i - 1)^2)$ on $[1/3, 3]$. 
 
 Moreover, for $i \in [m]$, we have $\|\Theta_i^{-1}\|_{op} \leq 2$ and so $D_F(\Theta_j|| \Theta_i) = O( \|\Theta_i - \Theta_j\|_F) = O(\delta).$ We now have $D_{KL}(P_i|| P_j) \leq Cn \delta^2$ for some absolute constant $C$. By Fano's lemma, 
...
This implies the rate lower bound in $\|\cdot\|_F$. The bound on $D_F(\htheta ||\Theta_i)$ holds because $D_F(\htheta|| \Theta_i) \leq \|\Theta_i\|_{op} \| \Theta - \htheta\|_F \leq 3/2 \| \Theta - \htheta_i\|_F$, so we must have 
$$\min_{i \in [m]} \Pr[ D_F(\htheta || \Theta_i) \leq \delta/3] \leq 1/2.$$
To obtain the rate lower bound in $\| \cdot \|_{op},$ instead start with a packing $V_0$ of the unit operator norm ball of cardinality $m \geq 2^{d(d+1)/2}$. We modify the proof by bounding $D_{KL}(P_i || P_j) = O(n \| \Theta_i - \Theta_j\|_F^2) = O(n d \|\Theta_i - \Theta_j\|_{op}^2) \leq C nd \delta^2.$ Proceeding as before, we find that for $\delta = c \min \{\frac{d}{n}, 1\}$, 
$$\min_{i \in [m]} \Pr[ \|\htheta - \Theta_i\|_{op} \leq \delta/2] \leq 1/2.$$
The bound on $D_{op}(\htheta || \Theta_i)$ follows from the bound $D_{op}(\htheta|| \Theta_i) \leq \|\Theta_i\|_{op} \|\htheta - \Theta_j\|_{op} \leq 3/2 \|\htheta - \Theta_j\|_{op}$.  

%we bound $D_{KL}(P_i|| P_j) \leq O(nD_{F}(\Theta_j || \Theta_i)^2)$ by $O (n d D_{op}(\Theta_j || \Theta_i)^2)$ 
%Indeed, if $D_F (\htheta || \Theta_i) \leq \delta,$ for $\delta \leq 1/2$, then $ D_{op} (\htheta || \Theta_i) \leq 1/2$ and in particular $1/2 I_d \preceq \htheta \preceq 3/2 \Theta_i$, or $\|\htheta\|_{op} \leq  $\|\htheta - \Theta_i\|_F$.
...
\begin{proof}
%If $d_2 \leq nd_1$, we are done. Assume $d_2 > n d_1$. 
