...
We propose a regularizer that is simple to compute and numerically performs well for approximately low-rank covariance matrices (which need not be sparse or have sparse inverses). These low-rank covariance matrices are equal to a low-rank Wishart matrix plus a small multiple of the identity matrix to ensure invertibility; we refer to them as ``spiked" covariance matrices. We show experimentally that, under a natural generative model for \emph{dense} covariance matrices, our regularized estimator outperforms Gemini and KGlasso. We also show the regularized estimator can outputerform Gemini and KGlasso when $\Sigma_2$ is ill-conditioned even when $\Theta_1$ is sparse. Moreover, we observe that our regularized estimator is significantly faster to compute than the GLASSO-type estimators. All three estimators require parameter tuning, so when possible we compare throughout a plausible range of parameters for each of them. We leave determination of the appropriate regularization parameter for our estimator by cross-validation for future work.
...
\begin{figure}
        ...
        %\caption{Three simple graphs}
        %\label{fig:three graphs}
\caption{Average error with $d_1 = 50, d_2 = 50, $ and number of samples $n$ ranging from $1$ to $10$ for both precision matrices sparse. \CF{actually do 10 runs/draws; right now it is 5, Choose regularization better for KGlasso/RegFF so that it actually improves with more samples.}}\label{fig:lc}
\end{figure}
