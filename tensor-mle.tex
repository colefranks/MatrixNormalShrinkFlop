...
\MW{What does the $\Omega$ depend on? Universal constant?}
\MW{And how did the constant $C>0$ in the appendix vanish?}
\MW{Why is the RHS now $\sqrt{d_1} + \sqrt{d_2}$?}
\MW{I think there was some normalization mismatch, please check.}

\begin{theorem}[\cite{pisier2012grothendieck,P14}]\label{thm:hess-pisier}
Let $A_1,\dots,A_N$ be independent $d_2\times d_1$ random matrices with independent standard Gaussian entries.
...
\begin{align*}
  \norm \Phi_A \leq t^{2} \sqrt N \left( d_1 + d_2 \right).
\end{align*}
\end{theorem}
...
\MW{The thing on the right should be $\E \norm{Y_i}_{\op}$ (for any $j$) right?}
\end{theorem}
