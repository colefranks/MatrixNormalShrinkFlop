...
The result is essentially Theorem~16.6 in~\cite{pisier2012grothendieck}, together with a standard symmetrization trick (e.g., Proof of Lemma~4.1 in~\cite{P14}). \MW{Give proof in an appendix?}

\begin{theorem}[Pisier]
There is an absolute constant~$C>0$ such that, for all $n,m\in\N$ and any scalar sequence $a_i \in \R^n$ the following holds with probability at least $1-1/{\color{red}\poly(n,m)}$:
\begin{align*}
  \norm{ \sum_{i=1}^n \alpha_i (Y_i \ot Y_i) (I - P) }_{\op}
\leq C \norm{\alpha}_2 \left( \E \norm{Y}_{\op} \right)^2,
\end{align*}
where $Y_1,\dots,Y_n,Y$ are independent $m\times m$ standard Gaussian random matrices and $P$ denotes the orthogonal projection onto multiples of $\sum_{i=1}^m e_i \ot e_i$.
\end{theorem}
...
\begin{proof}[Proof.] 
\AR{The lemma is $\begin{bmatrix} A & K \\ K^{*} & B \end{bmatrix} \succeq 0$ iff $A, B \succeq 0, \|A^{-1/2} K B^{-1/2}\|_{op} \leq 1$. This means $\begin{bmatrix} 0 & K \\ K^{*} & 0 \end{bmatrix} \preceq \begin{bmatrix} A & 0 \\ 0 & B \end{bmatrix}$ for any $A,B \succeq 0, \|A^{-1/2} K B^{-1/2}\|_{op} \leq 1$ }

\CF{An attempt where I understand the basic premise, but the numbers aren't coming out right} We wish to show that, restricted to the subspace $\oplus_a $ 


We will use the inequality $$- \begin{bmatrix} \sqrt{B B^T} & 0 \\ 0 & \sqrt{B^T B} \end{bmatrix} \preceq \begin{bmatrix} 0  & B \\ B^T & 0 \end{bmatrix} \preceq \begin{bmatrix} \sqrt{B B^T} & 0 \\ 0 & \sqrt{B^T B} \end{bmatrix},$$ for block matrices, which can be seen from computing Schur complements. Applying this to the two-by-two upper left block submatrix of $\nabla^2f$, we see 
\begin{align*}
  \nabla^2 f \succeq \begin{bmatrix}
  \nabla_{11}^2 f - \sqrt{\nabla_{12}^2 f (\nabla_{12}^2 f)^T} & 0 & \hdots &  \nabla_{1k}^2 f  \\
0 & \nabla_{22}^2 f - \sqrt{(\nabla_{12}^2 f)^T \nabla_{12}^2 f} & \ddots & \vdots \\
\vdots & \ddots & \ddots & \vdots\\
  \nabla_{k1}^2 f & \hdots &  \hdots & \nabla_{kk}^2 f \\
  \end{bmatrix}.
\end{align*}
Here $\succeq$ is understood to refer to the Loewner ordering on the space $\oplus_a \Sym^0_{d_a}$ on which $\nabla^2 f$ is a linear operator. \CF{This proof doesn't yet make perfect sense, because we haven't specified what inner product we want for psd-ness. Actually it should be the $d$-norm.}
\end{proof}
