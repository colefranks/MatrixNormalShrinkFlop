...
\begin{lemma} Let $f$ be geodesically convex everywhere, and $\lambda$-geodesically strongly convex w.r.t. norm $\|\cdot\|$ on a ball of radius $\kappa$ about $I$; further assume the geodesic gradient satisfies $\|\nabla f(I)\|_{*} = \eps < \lambda \kappa$. Then there is an optimizer within a $\|\cdot\|$-ball of radius $\eps/\lambda$. 
\end{lemma}
\begin{proof}
...
$$\forall a: \| d_{a} \nabla_a f \|_{op} \leq \epsilon $$
as a consequence, $\|\nabla f(I)\|_{*}^{2} := \sum_{a} d_{a} \|(\nabla f(I))_{a} \|_{F}^{2} \leq k \epsilon^{2}$. 
\end{lemma}

\begin{theorem}\label{thm:tensor-convexity}
Suppose the number of samples is at least $k^{2} \max_{a} d_{a}^{2}/D$. Then with failure probability $\leq 1/poly(D)$, $f_X$ is $\Omega(1)$-strongly convex at $I$ w.r.t. norm $\|Z\|^{2} := \sum_{a} d_{a} \|Z_{a}\|_{F}^{2}$. 
\end{theorem}
...
\begin{proof}[Proof of \cref{lemma:gradient-bound}]Let $N = n D/d_a$ and $d = d_a$. Recall that $\nabla_a f = \rho^a - I_a/d_a$. $\rho^a$ is distributed as $Y Y^\dagger/\|Y\|_F^2$, where $Y$ is a $d \times N$ matrix with independent Gaussian entries.
...
Let $t = \sqrt{N}/\log d$. Because $nD \geq C d_{a}^{2} \log^{2} d_{a}$, both largest and least singular values are
$\sqrt{N} \left( 1 + c/\log d  \right)$. 

Provided $c$ is small enough we then have 
 $$\lambda_1(YY^\dagger), \lambda_{d}(YY^\dagger) \in N(1 + c/\log d).$$ 
 Next, observe that $\|\vec X\|_F^2$ is a $\chi$-squared distribution with $Nd$ degrees of freedom, and thus satisfies $\| \vec X\|_F^2 - Nd \leq t N d$ with probability $1 - 2e^{- Nd t^2/8}$ for $t \leq 1$ \CF{\cite{wainwright}}. Thus 
  $$\lambda_1(\rho^a), \lambda_2(\rho^a) \in  N(1 \pm c/\log d)/ Nd(1 \pm c/\log d)$$ 
with probability $1 - 2 e^{-  N/\log^2 d}- 2 e^{- c Nd /\log d} = 1 - 4 e^{- N/\log^2 d}.$ Provided $c$ is small enough, the bound is proven.\end{proof}
