...
\begin{align*}
  \ef_\samp(\Theta_1, \dots, \Theta_k)
=  \frac{1}{nD}\samp_i^T \left( \textstyle \bigotimes_{a=1}^k \Theta_a \right) \samp_i -  \sum_{a = 1}^k \frac{1}{d_a}\log\det \Theta_a.
\end{align*}
...
\begin{align}\label{eq:mle}
  (\widehat{\Theta}_1, \dots, \widehat{\Theta}_k) := \underset{\det \Theta_1 = \dots = \det \Theta_k}{ \arg\min} f_x(\Theta_1, \dots, \Theta_k)
\end{align}
whenever the minimizer exists and is unique. We write $\widehat\Theta = \widehat\Theta(x)$ when we want to emphasize the dependence of the MLE on the samples~$x$.
...
% , i.e., $-\log \ell_x(\Theta_1^{1/2} e^{H_1t} \Theta_1^{1/2}, \dots, \Theta_k^{1/2} e^{H_kt} \Theta_k^{1/2})$ is convex in $t\in\R$ for any choice of $\Theta_1,\dots,\Theta_k$ and~$H_1,\dots,H_k$.
...
%With this restriction, the log-likelihood function reads \begin{align}\label{eq:reparameterized log likelihood}  \ell_x(\Theta_1, \dots, \Theta_k)&= \frac {nD} 2 k \log \det \Theta_1  - \frac{1}2 \sum_{i=1}^n x_i^T \left( \textstyle\bigotimes_{a=1}^k \Theta_a \right) x_i.\end{align}
...
$$\frac{1}{nD}\|\Phi_{X}\|_0 \leq t^{2} \frac{d_{1}+d_{2}}{\sqrt{d_{1} d_{2} nD}}$$ on traceless symmetric matrices. \CF{I got rid of the big O on the displayed bound...is this ok to be absorbed into the probability? I also got rid of $t > 1$ because the result vacuously holds if $t < 1$. Can \AR{} help?} \AR{Ya this should be fine since $(.1 t)^{\Omega(d)}$ is still $t^{\Omega(d)}$ }\end{theorem} 
...
\end{itemize}
 then $f$ is $(\lambda - O( \|\delta\|_{op}))$-strongly convex at $e^{\delta} := \otimes_{a} e^{\delta_{a}}$.
\end{lemma}
...
\begin{theorem}[\CF{KLR}]\label{thm:klr} There is a constant $c > 0$ such that the following holds for all $d_1 \leq d_2$. Let $\samp$ be a tuple of vectors in $\R^{d_1} \ot \R^{d_2}$ with $\|x\| = \sqrt{nD}$.
If $\Phi_{\samp}$ is $\eps$-doubly balanced and has spectral gap $\lambda$ for $\eps \leq c \lambda^2/\log d_1$, then the maximum likelihood estimator $(\widehat{\Theta}_1(\samp), \widehat{\Theta}_2(\samp))$ is unique and satisfies
$$\left\| \widehat{\Theta}_1(\samp) - I_{d_1}\right\|_{op}, \left\|\widehat{\Theta}_2(\samp) - I_{d_2}\right\|_{op} \leq \frac{\eps \log d_1}{\lambda}.$$
\end{theorem}
By \cref{prp:xnorm}, $|\frac{\|X\|}{\sqrt{nD}} - 1 | \leq \eps$ with probability $e^{- \Omega(\eps^2 nD)}$, so this additional factor will be negligible in our case. \cref{thm:klr}, along with what we've shown so far, already implies a preliminary version of \cref{thm:matrix-normal}. In the proof of \cref{thm:tensor-convexity}, we've already shown that $\Phi$ is an $(\eps \sqrt{d_2/n d_1}, 1 - c)$-quantum expander with failure probability 
...
$Y_i = \frac{1}{\sqrt{d_i}} X_i \Phi_X^*(I_{d_1})^{-1/2} = \frac{1}{\sqrt{d_i}}  X_i (\sum_{i = 1}^n X_i^\dagger X_i)^{-1/2}.$ \CF{modify this to make $Y$ have norm $\sqrt{nD}$? I think yes! DO THIS}
...
By the equivariance \CF{need to make determinants the same!} discussed in \cref{it:reduce} of \cref{subsec:proof-sketch}, we have $\htheta_1(x) = \htheta_1(y)$ and $\htheta_2(x) = \frac{1}{d_2}\Phi^*(I_{d_2})^{-1/2}\htheta_1(y)\Phi^*(I_{d_2})^{-1/2}.$ The bound on $D_{op}(\htheta_1(x)||I_{d_1})$ follows immediately. For $\htheta_2(x)$, by invariance of the relative spectral error we have 
...
We follow the same proof structure as \cref{thm:tensor-convexity}, and analyze the blocks separately. Note the $00$ block is a scalar $\nabla^{2}_{00} f = Tr[\rho]$ and the terms involving the $0$ block are just a vector:
...
\cref{thm:tensor-convexity} gives good bounds on the Hessian of $\samp$; so in order to bound the Hessian at perturbation $\samp' := \prod_{a} (e^{\delta_{a}})_{a} \samp$, it is enough to bound the difference. By the above discussion then we will bound the difference of each under each component-wise perturbation. Note the terms involve $\{\rho^{(a)}\}, \{\rho^{(ab)}\}$, so we prove perturbations on marginals in the following lemmas. 
...
\[ |\nabla^{2}_{00} f(e^{2\delta}) - \nabla^{2}_{00} f(I)| \leq 4 \|\delta\|_{op}     \]
\[ \|\nabla^{2}_{0a} f(e^{2\delta}) - \nabla^{2}_{0a} f(I)\|_{op} \leq 12 \|\delta\|_{op}     \]
\AR{Not really op norm here, just traceless part}
\end{corollary}
\begin{proof}
Recall that the $00$ block of the Hessian is just a scalar:
\[ |\tr[\rho_{\samp'} - \rho_{\samp}]| = |\langle \rho_{\samp}, e^{2\delta} - I \rangle| \leq Tr[\rho_{x}] \|e^{2 \delta} - I\|_{op} \leq 4 \|\delta\|_{op}     \]
In the last step we used the approximation for $e^{\delta}$ given above. For each $a$, the $0a$ block is a vector, so it is enough to bound the inner product with any $Z \perp I_{a}$:
\[ |\langle \rho_{\samp'}^{(a)} - \rho_{\samp}^{(a)}, \sqrt{d_{a}} Z \rangle| \leq \|\rho_{\samp'}^{(a)} - \rho_{\samp}^{(a)}\|_{op} \sqrt{d_{a}} \|Z\|_{1} \leq 10 \|\delta\|_{op} \|\rho^{(a)}_{\samp}\|_{op} d_{a} \|Z\|_{F} \leq 11 \|\delta\|_{op} \|Z\|_{F}   \]
\end{proof}
...
\leq \|\delta\|_{op} (4 z_{0}^{2} + 24 |z_{0}| \sqrt{ k \sum_{a} \|Z_{a}\|_{F}^{2}} )   \]
...
So combining all three terms and using Cauchy-Schwarz, we see:
\[ |\langle Z, (\nabla^{2} f_{\samp'} - \nabla^{2} f_{\samp} ) Z \rangle | \leq \|\delta\|_{op} \left(  4 z_{0}^{2} + 2 \cdot 12 |z_{0}| \sqrt{ k \sum_{a} \|Z_{a}\|_{F}^{2} } + 21 k \sum_{a} \|Z_{a}\|_{F}^{2} \right)    \]
\[ \leq 30 k \|\delta\|_{op} \left( z_{0}^{2} + \sum_{a} \|Z_{a}\|_{F}^{2} \right)    \]
