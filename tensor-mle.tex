...
\item For the tensor normal model, if $n \geq C \max\{d_a^3\}/ \prod_a d_a$, then the MLE for the precision matrices $\Theta_1, \dots, \Theta_k$ has MSE $O(\max \{ d_a^3 \} / (n \prod_a d_a ) )$ in $d_F$.
...
The flip-flop algorithm for this regularized estimator is quite simple. Instead of $\rho_\alpha$, we replace \CF{continue}
