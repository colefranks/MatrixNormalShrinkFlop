...
%\item $X$ for the tensor random variable, $X_1, \dots, X_n$ for each , $\samp = (X_1, \dots, X_n)$ for the random tuple of samples. $\rho = \samp \samp^T/\|\samp\|_F^2$. Lower case $x$ for samples.
...
Recall that for a centered normal distribution~$\cN(0,\Sigma)$ with $D\times D$ covariance matrix~$\Sigma$, the log-likelihood function for given samples $\samp_1,\dots,X_n\in\R^D$ is (up to an additive constant)
\begin{align*}
  \ell_{\samp}(\Theta)
...
\begin{align}\label{eq:tilde ell}
  \tilde\ell_{\samp}(\Theta) = \frac1D\log\det \Theta - \log \sum_{i=1}^n \braket{X_i, \Theta X_i}.
\end{align}
...
The result is $\lambda = n D / \sum_{i=1}^n \braket{X_i, \Theta X_i}$, hence $\sup_{\Theta\in\PD_D} \ell_{\samp}(\Theta) = \sup_{\Theta'\in\PD_D^1} \frac {nD}2 \left( \log(nD) - 1 \right) + \frac{nD}2 \tilde\ell_{\samp}(\Theta')$.}
...
\begin{align*}
  \tilde\ell_{\samp}(\Theta_1,\dots,\Theta_k) = \sum_{i=1}^k \frac1{d_a} \log \det \Theta_a - \log \sum_{i=1}^n \braket{X_i, \left( \textstyle \bigotimes_{a=1}^k \Theta_a \right) X_i},
\end{align*}
...
Given $n$ samples $\rv = (X_1, \dots, X_n)$ in $\R^D$, define the function $f_{\samp}\colon\prod_{a=1}^k \PD_{d_a}^1 \to \R$ by
\begin{align}\label{eq:projective-likelihood}
  f_{\samp}(\Theta_1,\dots,\Theta_k) = \log \sum_{i=1}^n \braket{X_i, \left( \textstyle \bigotimes_{a=1}^k \Theta_a \right) X_i}.
\end{align}
...
We also note that $f$ has a certain useful transformation property or ``equivariance." For all $\Theta \in \PD$, $A \in \SL$, and $\samp=(X_1,\dots,X_n)$, we have that
\begin{align}\label{eq:equivariance}
  f_{A^{-1} \samp}(A^T \Theta A) = f_{\samp}(\Theta),
\end{align}
where we write $A^{-1} \samp = (A^{-1} X_1,\cdots,A^{-1} X_n)$.
...
Let $\rv = (X_1,\dots,X_n)$ be independent standard Gaussian random variables in $\R^D$, where $D=d_1\cdots{}d_k$.
...
There is $c> 0$ such that the following holds: with probability at least $1 - k^{2} n^{c 2 (d_1 + d_2)}$, the function $f_{\rv}$ is $c$-strongly convex at all points $Z\in \PD$ with $\sum_{i = 1}^n \|\log Z_a\|_{op} < c/k.$ 
\end{theorem}
...
Let $\rv = (X_1,\dots,X_n)$ be independent standard Gaussian random variables in $\R^D$, where the number of samples is $N \geq k^2 \max_{a=1}^k d_a^2 / D$. Then, with probability at least $1 - 1/{\color{red}\poly(D)}$, 
...
$$ \|\nabla^{2} f_{\rv} - I\|_{op} \leq (k - 1) \lambda + k \eps^2 + \eps $$
 In particular, $f_{\rv}$ is $\Omega(1)$ strongly convex with probability $\CF{good}$. 
\end{theorem}
...
$$ \|e^{\delta} - I\|_{op} \leq \leq \|\delta\|_{op} + \|\delta\|_{op}^{2} \leq 2 \|\delta\|_{op}.$$ 
\end{fact}
...
For input $\{X_{i}\}$ and perturbation $\|\delta\|_{op} \leq \frac{1}{20}$ giving $\{X'_{i} := (e^{\delta})_{a} X_{i} \}$:
\[ \|\rho_{X'}^{a} - \rho_{X}^{a}\|_{op} \leq 4.5 \|\delta\|_{op} \|\rho_{X}^{a}\|_{op}     \]
\end{lemma}
\begin{proof} By definition, $\|\rho_{X'}^{(a)} - \rho_{X}^{(a)}\|_{op} = \sup_{\|Z\|_{1} \leq 1} \langle Z_{a}, \rho_{X'} - \rho_{X} \rangle $. 


Choose $\eta$ such that  $\|(e^{\delta})_{a} X\|_{2}^{-1} = \|X\|_{2}^{-1} (1 + \eta)$; note that $|\eta| = O(\|\delta\|_{op})$ provided $c$ is small enough. Letting $\delta' := (1+\eta)e^{\delta} - I_{a}$. Assuming without loss of generality that $\|Z\|_{1} = 1$, we have 
...
For input $\{X_{i}\}$ and perturbation $\|\delta\|_{op} \leq \frac{1}{20}$ giving $\{X'_{i} := (e^{\delta})_{b} X_{i} \}$ for $b \neq a$, there is a universal constant $C$:
\[ \|\rho_{X'}^{a} - \rho_{X}^{a}\|_{op} \leq 9.5 \|\delta\|_{op} \|\rho_{X}^{a}\|_{op}      \]
\end{lemma}
\begin{proof} 
Choose $\eta$ such that $\|(e^{\delta})_{b} X\|_{2}^{-2} = (1+\eta) \|X\|_{2}^{-2}$; let $\delta' := (1+\eta)e^{2\delta} - I$. 
...
\[ |\langle Z, \rho_{X'}^{(a)} - \rho_{X}^{(a)} \rangle| \leq \|\delta'\|_{op} (\|Z_{+}\|_{1} + \|Z_{-}\|_{1}) \|\rho_{X}^{(a)}\|_{op}     \]
...
For input $\samp \in \CF{where?}$ such that $d_{a}\|\rho_{X}^{(a)}\|_{op} \leq 1+\frac{1}{20}$; and perturbation $\delta := \sum_{a} (\delta_{a})_{a}$ with $\|\delta\|_{op} = \sum_{a} \|\delta_{a}\|_{op} \leq \frac{1}{20}$ giving $\{X'_{i} := e^{\delta} X_{i} \}$:
\[ \frac{1}{d_{a}} \|\nabla^{2}_{aa} f(e^{2\delta}) - \nabla^{2}_{aa} f(I)\|_{op} \leq 40 \|\delta\|_{op} \|\rho_{X}^{(a)}\|_{op}      \]
\end{corollary}
...
\underset{\ref{atoaaRobustness},\;\ref{btoaaRobustness}}{\leq} \sum_{j=1}^{k}  9.5 \|\delta_{j}\|_{op} \|\rho_{X_{j-1}}^{(a)}\|_{op} \|Y^{2}\|_{1} \\
& \leq \left( \prod_{j=1}^k (1+9.5 \|\delta_{j}\|_{op}) - 1 \right) \|\rho_{X}^{(a)}\|_{op} \|Y\|_{F}^{2} \leq 10 \|\delta\|_{op} \|\rho_{X}^{(a)}\|_{op} \|Y\|_{F}^{2}.   \end{align*}
The term in parenthesis is shown by induction, and in the last step we used $\|\delta\|_{op} \leq \frac{1}{20}$. We next bound the rank-one term $\langle \rho_{X}^{(a)}, Y \rangle^{2}$:
...
\[ \leq 10 \|\delta\|_{op} (2+ 10 \|\delta\|_{op}) (\|\rho_{X}^{(a)}\|_{op} \|Y\|_{1})^{2} \leq 25 \|\delta\|_{op} d_{a} \|\rho_{X}^{(a)}\|_{op}^{2} \|Y\|_{F}^{2}       \]
...
\end{definition}
We use a lemma from \cite{KLR19}.
...
\CF{add quantifiers}
For $\samp \in \CF{somewhere}$ and perturbation $\delta \in \Sym$ with $\|\delta\|_{op} \leq \frac{1}{20}$, if $X' = (e^{\delta})_c X$ for $c \in \{a,b\}$ we have 
%$\{X'_{i} := (e^{\delta})_{c} X_{i} \}$ for $c \in \{a,b\}$:
\[ \|H_{X'}^{ab} - H_{X}^{ab}\|_{0} \leq 4.5 \|\delta\|_{op} \|H_{X}^{ab}\|_{F \to F}    \]
\end{lemma}
\begin{proof}
We will drop the superscript $(ab)$ since it doesn't change. By taking adjoints, we can assume w.l.o.g. that $c = b$. Let $R : \mat(d_{b}) \to \mat(d_{b})$ be defined as $R(Z) := (1+\eta)^{2} e^{\delta} Z e^{\delta}$ for $\eta$ defined by our normalization $\|(e^{\delta})_{b} X\|_{2}^{-1} =: (1+\eta) \|X\|_{2}^{-1}$. The subspace $\Sym_{d_{b}}^{0}$ is not invariant under $R$, but we show $R \approx I$. Let $\delta' := (1+\eta) e^{\delta} - I$. 
\[ \|R(Z) - Z\|_{F} \leq 2 \|\delta' Z\|_{F} + \|\delta' Z \delta'\|_{F} \leq (2 \|\delta'\|_{op} + \|\delta'\|_{op}^{2}) \|Z\|_{F}    \]
So using the definition of $\|\cdot\|_{0}$:
\[ \sup_{Y \in \Sym_{d_{a}}^{0},Z \in \Sym_{d_{b}}^{0}} \frac{\langle Y, (H_{X'} - H_{X}) Z \rangle}{\|Y\|_{F} \|Z\|_{F}} = \frac{ \langle Y, H_{X} (R(Z) - Z) \rangle }{\|Y\|_{F} \|Z\|_{F}} \]
To complete the proof, we use $\|\delta\|_{op} \leq \frac{1}{20}$ and the same calculation as in \ref{atoaaRobustness}.
\end{proof}

\begin{lemma} \label{ctoabRobustness}
For input $\{X_{i}\}$ and perturbation $\|\delta\|_{op} \leq \frac{1}{20}$ giving $\{X'_{i} := (e^{\delta})_{c} X_{i} \}$ for $c \not\in \{a,b\}$:
\[ \|H_{X'}^{ab} - H_{X}^{ab}\|_{0} \leq 19 \|\delta\|_{op} \|H_{X}^{ab}\|_{F \to F}    \]
\end{lemma}
\begin{proof}
...
\[ |\langle Y, (H_{X'} - H_{X}) Z \rangle| \leq \|\delta'\|_{op} \|H_{X}\|_{F \to F} \sum_{s,t \in \{+,-\}} \|Y_{s}\|_{F} \|Z_{t}\|_{F}   \]
...
Again using the definition of $\|\cdot\|_{0}$; and $\|\delta\|_{op} \leq .05$ to give the same calculation from \ref{btoaaRobustness} on $\|\delta'\| \leq 9.5 \|\delta\|_{op}$; we get the lemma. 
\end{proof}
\CF{akshay make consistent with def 4}
\begin{corollary} \label{offdiagRobustness}
For input $\{X_{i}\}$ such that $\|d_{a} \rho_{X}^{(a)}\|_{op}, \|d_{b} \rho_{X}^{(b)}\|_{op} \leq 1+\frac{1}{20}$; perturbation $\delta := \sum_{a} (\delta_{a})_{a}$ with $\|\delta\|_{op} = \sum_{a} \|\delta_{a}\|_{op} \leq \frac{1}{20}$ giving $\{X'_{i} := e^{\delta} X_{i} \}$:
\[ \frac{1}{\sqrt{d_{a} d_{b}}} \|\nabla^{2}_{ab} f(e^{2 \delta}) - \nabla^{2}_{ab} f(I)\|_{op} \leq 100 \|\delta\|_{op} \sqrt{\|\rho_{X}^{(a)}\|_{op} \|\rho_{X}^{(b)}\|_{op}}     \]
\end{corollary}
\begin{proof}
...
\[ |\langle Y, (\rho_{X'}^{(ab)} - \rho_{X}^{(ab)}) Z \rangle| \leq 20 \|\delta\|_{op} \|H_{X}\|_{F \to F} \|Y\|_{F} \|Z\|_{F}    \]
...
\[ 40 \|\delta\|_{op} (1+20\|\delta\|_{op}) \|\rho_{X}^{(a)}\|_{op} \|\rho_{X}^{(b)}\|_{op} \|Y\|_{1} \|Z\|_{1}    \]
\[ \leq 80 \|\delta\|_{op} (\sqrt{d_{a} d_{b}} \|\rho_{X}^{(a)}\|_{op} \|\rho_{X}^{(b)}\|_{op}) \|Y\|_{F} \|Z\|_{F}    \]

In toto this gives the lemma:
\[ \frac{1}{\sqrt{d_{a} d_{b}}} \|\nabla^{2}_{ab} f(e^{2 \delta}) - \nabla^{2}_{ab} f(I)\|_{op} \leq \|\delta\|_{op}((20 \|H_{X}\|_{F \to F}) + 80 (\sqrt{d_{a} d_{b}} \|\rho_{X}^{(a)}\|_{op} \|\rho_{X}^{(b)}\|_{op})) \]
\[ \leq 100 \|\delta\|_{op} \sqrt{\|\rho_{X}^{(a)}\|_{op} \|\rho_{X}^{(b)}\|_{op}}     \]
...
\CF{fix all the $d$-norm stuff in this theorem}
...
We next bound $\kappa$. By \cref{thm:tensor-convexity}, with failure probability $\CF{1/poly(D)}$ we have that $f$ is $1-o_{d}(1)$ strongly convex in norm $\|\cdot\|_{\vec d}$ at $I$, and further by \cref{convexRobustness} $f$ is $1/2$ strongly convex at any point $e^{Z}$ with $\forall a: \|Z_{a}\|_{op} \leq  c/k$ for $c$ a small enough constant. \CF{check that $\|\sqrt{d_{a}} \nabla_{a} f_{X}\|_{op} \leq c$} Because $\|Z_a\|_{op} \leq \|Z_a\|_F \leq \sqrt{d_a} \|Z\|_{\vec d},$ we may take $\kappa = c/\max_a \sqrt{d_a}$ and $\lambda = 1/2$.
...
\begin{Algorithm}
\textbf{Input}: Samples $\samp = (X_1, \ldots, X_n)$ where $X_i \in \R^D$ is sampled from a (unknown) centered normal distribution $\cN(0, \Sigma)$, where each entry of $X_i$ is encoded in binary, with bit size $\le b$. Approximation parameter $\eps > 0$, given in binary representation. \\[.3ex]
