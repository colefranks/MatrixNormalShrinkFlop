...
Show that the gradient $\nabla f_{\vec X}(I)$ (defined below) is small with high probability. \MW{Swap items 2 and 3?}
\end{enumerate}
...
%-----------------------------------------------------------------------------
As suggested by the plan from the previous section, we now compute the gradient of $f_{\vec X}$ and bound it using basic matrix concentration results.
...
\MW{If you prefer we can also write $\rho = \frac{\vec X \vec X^T}{\norm{\vec X}_F^2}$ below, but then we need to say somewhere that we now think of $\vec X$ as a $D\times n$-matrix.}
...
\begin{proof}
  ...
  % \\ &= \tr \rho (I_1 \ot \cdots \ot I_{a-1} \ot Z_a \ot I_{a+1} \ot \cdots \ot I_n)
  = \tr \rho^{(a)} Z_a
  \end{align*}
...
\end{align*}
where $s_{\min}$ and $s_{\max}$ denotes the smallest and largest singular value, respectively.
\end{theorem}
...
Again, $\nabla_a f_{\vec X} = d_a \rho^{(a)} - I_a$ where $\rho^{(a)}$ is distributed as $Y Y^\dagger/\|Y\|_F^2$, where $Y$ is a $d \times N$ matrix with independent Gaussian entries.
...
\end{align*}
\MW{Too tired to check numbers, will continue later.}
