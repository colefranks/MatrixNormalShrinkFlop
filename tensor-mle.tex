...
Let $A$ be an $n \times m$ matrix with independent standard Gaussian entries and $n \geq m$ without loss of generality.
Then for $n = \max\{m,n\}$ large enough
\begin{equation*}
  \E \norm{A}_{\op} \geq \frac{1}{3} (\sqrt{n} + \sqrt{m}).
\end{equation*}
\end{lemma}
\begin{proof}
The moment definition of sub-Gaussianity from \cref{thm:banach conc} shows for $p=2$ and $\sigma^{2} = 1$ (by \cref{l:opNormSubG}) \MW{square root missing? also, already have to plug in $p=2$ here.}
\[ (\E \norm{A}_{\op}^{2})^{1/2} \leq \E \norm{A}_{\op} + C \sqrt{2} \implies (\E \norm{A}_{\op} \geq (\E \norm{A}_{\op}^{2})^{1/2} - C \sqrt{2}    \]
where $C$ the universal constant in \cref{thm:banach conc}. 
...
\[ \E \norm{A}_{\op} \geq \sqrt{ n} - C\sqrt{2} \geq \frac{2}{3} \sqrt{n} \geq \frac{\sqrt{n} + \sqrt{m}}{3} .   \]
Here the lower bounds follow for $n = \max\{m,n\}$ large enough since $C$ is a universal constant \MW{what is $C'$? hasn't been defined yet}.
...
\MW{Isn't $|e^x - 1| \leq 2|x|$ for $|x| \leq 1$?} \AR{Ya my bad I was being dumb}
\end{fact}
