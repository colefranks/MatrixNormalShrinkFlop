...
Let $\Theta$ denote the \emph{precision matrix}, i.e., $\Theta = \bigotimes_{a=1}^k \Theta_a$, where $\Theta_a = \Sigma_a^{-1}$. Let $\P$ denote the manifold of all such $\Theta$, i.e.
...
\end{align}
whenever the minimizer exists and is unique. We write $\widehat\Theta = \widehat\Theta(x)$ when we want to emphasize the dependence of the MLE on the samples~$x$, and we we say $(\htheta_1, \dots, \htheta_k)$ is \emph{an} MLE for $(\Theta_1, \dots, \Theta_k)$ if $\otimes_{a = 1}^k \htheta_a = \htheta$. Note that $\P$ is not a convex domain under the Euclidean geometry on the $D\times D$ matrices.
...
Recall also that $D = \prod_{i=1}^k d_a$. \CF{definitely double check these bounds; esp if we need $n \geq k^2...$ or also a lower bound on $\eps$}
...
\begin{align*}
  D_{\op}(\widehat{\Theta}_1 \Vert \Theta_1) = O\left(\eps \sqrt{\frac{d_1}{nd_2}} \log d_1\right) \text{ and }   D_{\op}(\widehat{\Theta}_2 \Vert \Theta_2) = O\left(\eps \sqrt{\frac{d_2}{nd_1}}\right)
\end{align*}
for $a \in \{1,2\}$ with probability $1 - O(e^{ - \Omega( d_1 \eps^2)})$.
\end{theorem}
...
Rather than simply considering the metric induced by the Euclidean metric on the symmetric matrices, we consider the metric whose geodesics starting at a point $\Theta \in \PD(d)$ are of the form $\Theta^{1/2} e^{Ht} \Theta^{1/2}$ for $t \in \R$ and a symmetric matrix~$H$. % with $\norm H_F=D$.
...
\end{align*}
The direction $(1, 0, \dots, 0)$ changes $\Theta$ by an overall scalar, and tangent directions with supported only in the $a^{th}$ component for $a > 1$ change $\Theta_a$ subject to its determinant staying fixed. We now define the geodesics precisely.
...
\begin{align*}\exp_\Theta(H) &=   e^{\frac{1}{k} H_0}\cdot ( \Theta_1^{1/2} e^{\sqrt{d_1} H_1} \Theta_1^{1/2}) \ot  \dots \ot (\Theta_k^{1/2} e^{\sqrt{d_k} H_k} \Theta_k^{1/2}).\end{align*}

The \emph{geodesics} through $\Theta$ are the curves $t \mapsto \exp_\Theta(t H)$ for $t\in\R$ and $H\in\H$. We take the convention that the geodesics have unit speed if $\norm{H}_F^2 = \sum_{a=0}^k \norm{H_a}_F^2 = 1$.
...
For $\Theta \in \P$, the MLE $\widehat{\Theta}(x')$ is exactly $\Theta^{1/2} \widehat{\Theta}(x) \Theta^{1/2}$.
...
\begin{align*}
  \braket{H, \nabla^2 f(\Theta) K}
% = \sum_{a=1}^k \sum_{b=1}^k \braket{H_a, \nabla^2_{ab} f(P) K_b}
...
\begin{align*}
  ...
  \end{array}\right],
\end{align*}
where for $a, b \in [k]$ (i.e. the submarix to the lower-right of the lines) the components $\nabla_{ab}^2f(\Theta)$ of the Hessian are linear operators from the space of traceless symmetric $d_b\times d_b$-matrices to the space of traceless symmetric $d_a \times d_a$-matrices. The $00$ block, $\nabla^2_{00} f(\Theta),$ is a real number, and $\nabla_{0a}f$ is a linear operator from $\R$ to the space of traceless symmetric $d_1\times d_a$ matrices. Thus $\nabla_{0a}f$ itself can be viewed as a matrix.
\end{definition}
...
\end{align*}
and $\nabla_0 \ef_\samp = \tr \rho - 1 = \frac{1}{nD}\|\samp\|^2 - 1.$ 
\end{lemma}
\begin{proof}
First note that for $a \in [k]$, $\nabla_a \log\det \Theta $ is zero because each component $H_a$ of the tangent vector is traceless for $a \in [k]$. Thus for all $a\in[k]$ the component $\nabla_a f_\samp$ is the same as $\nabla_a F$ where $F:\Theta \mapsto \tr \rho \ot_{a = 1}^k \Theta_a$. Thus
  \begin{align*}
 ...
 % = \sqrt{d_a} \rho^{(a)} - I_a/\sqrt{d_a}.
  \end{align*}
 Finally $\nabla_0 \ell_\samp = \partial_{t=0} (\tr e^{t} \rho - t) = \tr \rho - 1.$
\end{proof}
...
\CF{this is now actually a component of the gradient, so could be moved to gradient bound.}
...
$$ (1 - t) nD \leq \|X\|^2 \leq (1 + t)nD$$ with probability $2e^{-t^2 nD/8}$.
\end{prop}
...
\begin{align*}
\nabla^2_{00} \ef_\samp &=\tr \rho = \frac{1}{nD}\|\samp\|^2 \\
\nabla^2_{a0} \ef_\samp &= \sqrt{d_a}\rho_a
\end{align*}
for all $a \in [k]$. 
\end{lemma}
\begin{proof}
Note that for $a,b \in \{0,\dots, k\}$, $\nabla^2_{ab} f_\samp = \nabla^2_{ab} F$ where $F:\Theta \mapsto \tr \rho \Theta$. 
...
%The quantum expansion we need will then follow from a result of Pisier. 
...
%for $\rho = \rho^{(a,b)}$ is a good \emph{quantum expander}, which or rather that $\Phi_{\samp}$ has a small operator norm on the traceless symmetric matrices. 
...
M_{k1} & \hdots &  \hdots & M_{kk}\\
 ...
 Applying this for all $a < b$ yields the result. The inequality \cref{eq:psd-bound} also applies with the signs and inequalities reversed; applying this inequality for all $a < b$ yields the upper bound on $M$. \end{proof}
  %By a change of basis, \cref{lem:block-matrix} also applies with $\| \cdot \|_{op}$ replaced by $\|\cdot \|_0$ and $\preceq$ with respect to the inner product on $\H$. We will need to supply bounds on $\|\nabla_{ab}^2 f\|_{op}$ and $\| \nabla_{aa}^2 f - I \|_{0}$ for $a \neq b$. First we bound the off-diagonal terms. 
...
$$\frac{1}{nD}\|\Phi_{X}\|_0 \leq t^{2} \frac{d_{1}+d_{2}}{\sqrt{d_{1} d_{2} nD}}$$ on traceless symmetric matrices. \end{theorem} 
...
\begin{proof}[Proof of \cref{thm:tensor-convexity}] \CF{make sure this is in the big theta notation} By \cref{lem:expansion-convexity}, it is enough to prove that $\Phi^{ab}$ is a $(\eps, \lambda):=(1/40 \sqrt{ k}, 1/20k)$-quantum expander for all $a < b \in [k]$ and that $\tr \rho \in (7/8, 9/8)$ with the desired failure probability. Firstly, $\tr \rho = \frac{1}{nD} \|X\|^2$ is in $(7/8, 9/8)$ with failure probability $e^{ -\Omega(nD)} $ by \cref{prp:xnorm}. 
...
\item and $\|\log \Theta\|_{op} \leq c$,
\end{itemize}
 then $f$ is $(\lambda - O( \|\delta\|_{op}))$-strongly convex at $\Theta$.
\end{lemma}
...
If $\Phi_{x}$ is $\eps$-doubly balanced and has spectral gap $\lambda$ for $\eps \leq c \lambda^2/\log d_1$, then there is unique positive definite matrices $P \in \SL(d_1), Q\in \SL(d_2)$ such that $\Phi_{(P \ot Q)x}$ is doubly balanced. Moreover,

$$\| P^2 - I_{d_1}\|_{op}, \| Q^2 - I_{d_2}\|_{op} \leq \frac{\eps \log d_1}{\lambda}.$$
\end{theorem}
The key observation here is that any $(P^2,Q^2)$ such that $\Phi_{(P \ot Q)x}$ is doubly balanced minimize $f_\samp$ subject to $\det P = \det Q = 1$. Thus, finding them captures the key difficulty of the MLE. \cref{thm:klr}, along with what we've shown so far, already implies a preliminary version of \cref{thm:matrix-normal}. For simplicity of the proof sketch, we stick to the case $\det \Theta = 1$ so we can set $\det \Theta_1 = \det \Theta_2 = 1$. In proving \cref{thm:tensor-convexity}, we've already shown that $\Phi$ is an $(\eps \sqrt{d_2/n d_1}, 1 - \lambda)$-quantum expander with failure probability 
$$O(e^{ - \Omega( d_2 \eps^2)}) + (\sqrt{nD}/{d_1})^{ - \Omega(d_1)}.$$ 

By \cref{thm:klr} and the discussion afterwards, we immediately have that with the above failure probability the minimizers $\Theta'_1, \Theta'_2$ of $f_\samp$ over $\SL(d_1)\times \SL(d_2)$ satisfy
\begin{align*}
...
$Y_i =  X_i \Phi_X^*(I_{d_1})^{-1/2} =  X_i (\sum_{i = 1}^n X_i^\dagger X_i)^{-1/2}.$ 
...
\begin{proof}[Proof of \cref{thm:matrix-normal} \CF{Final form}] 
...
We have already shown that the first term is $O(\eps \sqrt{\frac{d_1}{n d_2}}).$ Writing $B^{-2} = \frac{\|x\|^2}{nd_1 d_2} \frac{d_2 \Phi^*(I_{d_1})}{\|x\|^2}$, the second term is $O(\eps \sqrt{\frac{d_2}{n d_1}})$ by \cref{it:norm} and \cref{it:expander}, completing the proof.
\end{proof}

\begin{proof}[Proof of \cref{thm:matrix-normal} \CF{BACK TO SL}] 
By the equivariance discussed in \cref{subsec:proof-sketch}, it is enough to prove \cref{thm:matrix-normal} under the assumption $\Theta_a = I_{d_a}$ for $a \in \{1,2\}$. Let $P, Q$ positive definite be the minimizers of $f_\samp(P^2\otimes Q^2)$ restricted to $(P,Q) \in \SL(d_1)\times\SL(d_2)$. We first show that $P,Q$ are close to identity, and then show that $\htheta_1,\htheta_2$ are close to $P,Q$.
 Consider the following events. 
\begin{enumerate}
\item\label{it:expander} The operator $\Phi_{\rv}$ is a $(\eps_0  \sqrt{{d_2}/{n d_1}},1-\lambda)$-quantum expander.
\item\label{it:flop-balanced} The operator $\tilde{\Phi}$ is $\eps_0 \sqrt{{d_1}/{n d_2}}$-balanced.
\item \label{it:norm} $|1 - \frac{\|X\|}{\sqrt{nD}}| \leq \frac{\eps_0}{\sqrt{d_2}}. $ 
\end{enumerate}
These events occur with failure probability $1 - O(e^{ - \Omega( d_1 \eps_0^2)}).$ Indeed, by the assumption
$$n \geq C (d_2/d_1) \max\{\log (d_2/d_1),  \eps_0^{-2}\} $$
\cref{it:expander} occurs with probability $1 - O(e^{ - \Omega( d_1 \eps_0^2)})$ by \cref{thm:operator-cheeger}. By \cref{lem:flipflop-concentration}, \cref{it:flop-balanced} occurs with probability $1 - O(e^{ - \Omega( d_1 \eps_0^2)}).$ Finally, \cref{it:norm} occurs with probability $1 - e^{- \Omega(n d_1 \eps^2)}$ by \cref{prp:xnorm}. By the union bound all the events occur with probability $1 - O(e^{ - \Omega( d_1 \eps_0^2)})$. Let $\samp$ satisfy all three properties.
 
We now use a lemma relating the quantum expansion of $\tilde{\Phi}$ and $\Phi_\samp$; this is analogous to \cref{convexRobustness} guaranteeing the strong convexity of $f$ in a neighborhood of the origin. Let $z = (I_{d_1} \ot B) x$ where $B = \det \Phi_\samp^*(I_{d_1})^{1/2n} \Phi_\samp(I_{d_1})^{-1/2} \in \SL(d_2).$ Note that the quantum expansion of $\Phi_z$ and $\tilde{\Phi}$ are the same. By Lemma 4.4 in \cite{FM20}, if $\kappa(B^2) - 1 \leq \delta < c$,  then $\Phi_z$ is an $(\eps_0 \sqrt{d_2/n d_1} + O(\delta), 1 - \lambda + O(\delta))$-quantum expander. We have $\kappa(B^2) = \kappa(\|x\|^{-2} \Phi_\samp^*(I_{d_1})) = O(\|\|x\|^{-2} \Phi_\samp^*(I_{d_1}) - I_{d_2}\|_{op}) = O(\eps_0 \sqrt{d_2/nd_1})$ by the balancedness of $\Phi_x$. Thus $\tilde{\Phi}$ is an $(\eps_0 \sqrt{d_1 / n d_2}, 1 - \lambda/2)$-quantum expander provided $c$ is small enough compared to $\lambda$.

By our choice of $n$, we have $\eps_0 \sqrt{d_1/nd_2} \leq c\lambda^2\log d_1$ if we take $\eps_0 = \eps/\log(d_1)$ for $\eps \leq c\lambda^2$. Thus \cref{thm:klr} applies to $\Phi_z$ and hence there are unique minimizers $\tilde{P} \succ 0$ and $\tilde{Q} \succ 0$ of $f_z(\tilde{P}^2\ot \tilde{Q}^2)$ over $\SL(d_1)\times \SL(d_2)$ and they satisfy 
$$\|\tilde{P}^2 - I_{d_1}\|_{op}, \|\tilde{Q}^2 - I_{d_2}\|_{op} \leq \eps_0 \sqrt{\frac{d_1}{n d_2}} \log d_1.$$

By equivariance, this time applied to the minimization problem over $\SL(d_1)\times \SL(d_2)$), we have $P = \tilde{P}$ and $Q^2 = B \tilde{Q}^2 B.$ Now by invariance of $D_{op}$, 
$$D_{op}(Q^2 \rVert I_{d_2}) = D_{op}(\tilde{Q}^2\rVert B^{-2}) \leq D_{op}(\tilde{Q}^2\rVert I_{d_2}) + D_{op}(B^{-2}\rVert I_{d_2}).$$
The inequality follows from the approximate triangle inequality and approximate symmetry for the relative spectral error \CF{cite}. The first term of the right-hand side satisfies 
$$ D_{op}(\tilde{Q}^2\rVert I_{d_2}) = \eps_0 \sqrt{\frac{d_1}{n d_2}} \log d_1 = O\left(\eps \sqrt{\frac{d_2}{n d_1}}\right).$$
To bound the second term, observe that
$$\|\det \Phi_\samp^*(I_{d_1})^{- 1/n} \Phi_\samp(I_{d_1}) - I_{d_2}\|_{op} = O( \|\frac{d_2}{\|x\|^2} \Phi_\samp(I_{d_1}) - I_{d_2}\|_{op}) = O(\eps_0 \sqrt{d_2/nd_1}).$$ The second inequality follows by balancedness of $\Phi_\samp$. The first inequality follows because $\eps_0 \sqrt{d_2/nd_1} \leq c$ and $\det \Phi_\samp^*(I_{d_1})^{- 1/n} \Phi_\samp(I_{d_1})$ are eigenvalues of $\frac{d_2}{\|x\|^2} \Phi_\samp(I_{d_1})$ divided by their geometric mean, which must be between the extremal eigenvalues. Taken together, we have 
$$D_{op}(P^2 \rVert I_{d_2}) = O\left(\eps \sqrt{\frac{d_1}{n d_2}}\right) \text{ and } D_{op}(Q^2 \rVert I_{d_2}) = O\left(\eps \sqrt{\frac{d_2}{n d_1}}\right).$$
 It remains to relate $P^2, Q^2$ to $\htheta_1, \htheta_2$. Fortunately computing the optimum $\htheta$ of $f$ once $\Upsilon:=P^2 \ot Q^2$ is known is a simple one-dimensional optimization problem with a closed form solution. Indeed, we take $\Theta = \alpha \Upsilon$ for some $\alpha$ minimizing $f_x(\alpha \Upsilon) = \alpha f_x(\Upsilon) - \log \alpha,$ which takes its minimum at $\alpha = \frac{1}{f_x(\Upsilon)}$. Thus $\htheta_1 = f_x(\Upsilon)^{-1/2} P^2$ and $\htheta_2 =  f_x(\Upsilon)^{-1/2} Q^2$. But $f_x(\Upsilon) = f_z()$  
...
\end{proof}
\CF{convention to rule them all: only the first determinant is fixed. Will also be equivariant.}




\begin{proof}[Proof of \cref{thm:matrix-normal}] As discussed in \cref{subsec:proof-sketch}, it is enough to prove \cref{thm:matrix-normal} under the assumption $\Theta_a = I_{d_a}$ for $a \in \{1,2\}$. Consider the following events.
\begin{enumerate}
\item\label{it:expander} The operator $\Phi_{\rv}$ is a $(\eps_0  \sqrt{{d_2}/{n d_1}},1-\lambda)$-quantum expander.
\item\label{it:flop-balanced} The operator $\tilde{\Phi}$ is $\eps_0 \sqrt{{d_1}/{n d_2}}$-balanced.
\item \label{it:norm} $|1 - \frac{\|X\|}{\sqrt{nD}}| \leq \frac{\eps_0}{\sqrt{d_2}}. $ 
...
We now use a lemma relating the quantum expansion of $\tilde{\Phi}$ and $\Phi_\samp$; this is analogous to \cref{convexRobustness} guaranteeing the strong convexity of $f$ in a neighborhood of the origin. \CF{simplify.}The operator $\tilde{\Phi}$ is a \emph{scaling} of $\Phi_\samp$, that is for some ``scalings'' $g \in \GL(d_1), h \in \GL(d_2)$ for all $Z$ we have $\tilde{\Phi}(Z) = g^\dagger \Phi_\samp( h Z h^\dagger) g$. In our case we may take $h = \frac{\|x\|}{\sqrt{d_2}}\Phi_\samp^*(I_{d_1})^{-1/2}$ and $g = \frac{1}{\|x\|} I_{d_1}$. By Lemma 4.4 in \cite{FM20}, if $\| h^\dagger h - I_{d_1}\|_{op} \leq \delta$,  then $\tilde{\Phi}$ is an $(\eps \sqrt{d_2/n d_1} + O(\delta), 1 - \lambda + O(\delta))$-quantum expander. Fortunately, $h^\dagger h$ is bounded by the balancedness of $\Phi_\samp$. Indeed, $h^\dagger h - I = \frac{\|x\|^2}{d_2} ( \Phi^*(I_{d_1}))^{-1} - I_{d_2}$. By the balancedness of $\Phi_\samp$ we already know that 
$$\|\frac{d_2}{\|x\|_F^2}\Phi_\samp^*(I_{d_1}) - I_{d_2}\|_{op} \leq \eps_0 \sqrt{d_2/n d_1};$$
 provided $\eps_0 \sqrt{d_2/nd_1} \leq 1/2$ we have $\|h^\dagger h - I\|_{op} = O( \eps_0 \sqrt{d_2/n d_1})$. Thus $\tilde{\Phi}$ is an $(\eps_0 \sqrt{d_1 / n d_2}, 1 - \lambda/2)$-quantum expander provided $c'$ is small enough compared to $c$.
 
By our choice of $n$, we have $\eps \sqrt{d_1/nd_2} \leq c\lambda^2\log d_1$ if we take $\eps_0 = \eps/\log(d_1)$ for $\eps \leq c\lambda^2$. The tuple $y_i =  \sqrt{nd_1} x_i  \,\Phi_\samp^*(I_{d_1})^{-1/2}$ satisfies $\|y\| = 1$, so by \cref{thm:klr} the MLE $\htheta_1(y), \htheta_2(y)$ satisfies
\begin{gather*} \| \htheta_1(y) - I_{d_1}\|_{op}, \| \htheta_2(y) - I_{d_2}\|_{op} \leq C \eps_0 \sqrt{\frac{d_1}{n d_2}} \log d_1 = O\left(\eps \sqrt{\frac{d_1}{n d_2}}\right).\end{gather*}
We may write $y = (A^{-1/2} \ot B^{-1/2}) x_i$ for $\det A = \det B$ by setting 
$$B =  \det\left(\frac{\Phi_\samp^*(I_{d_2})}{nd_1}\right)^{- \frac{1}{2n}} \frac{\Phi_\samp^*(I_{d_2})}{nd_1}\textrm{ and } A = \det\left(\frac{\Phi_\samp^*(I_{d_2})}{nd_1}\right)^{\frac{1}{2n}} I_{d_1}.$$

By the equivariance \CF{need to make determinants the same!} discussed in \cref{it:reduce} of \cref{subsec:proof-sketch}, 


we have $\htheta_1(x) = A^{-1/2}\htheta_1(y)A^{-1/2}$ and $\htheta_2(x) = B^{-1/2}\htheta_1(y)B^{-1/2}.$ To bound $D_{op}(\htheta_1(x)||I_{d_1})$, write 
$$D_{op}(\htheta_1(x)|| I_{d_1}) = D_{op}(\htheta_1(y)|| A)\leq D_{op}(\htheta_1(y)|| I_{d_1}) + D_{op}(A||I_{d_1})$$
by the approximate triangle inequality for the relative spectral error \CF{cite}. Now $D_{op}(A|| I_{d_1}) = |1 - \det\left(\Phi_\samp^*(I_{d_2})/ nd_1\right)^{\frac{1}{2n}}| = O(\eps_0)$ by the $\eps_0$-balancedness of $\Phi_\samp^*(I_{d_2})/ nd_1$ and \cref{it:norm}. \CF{actually this is maybe WRONG or at least not very good.}


For $\htheta_2(x)$, by invariance of the relative spectral error we have 
\begin{align*}D_{op}(\htheta_2(x)|| I_{d_2}) = D_{op} (\htheta_2(y) || d_2\Phi^*(I_{d_2})) &= O( D_{op}(\Theta_2(y) || I_{d_2}) + D_{op}(d_2\Phi_\samp^*(I_{d_2})|| I_{d_2}))\\
& = O\left(\eps \sqrt{\frac{d_2}{n d_1}}\right),
\end{align*} where the penultimate inequality is by the approximate triangle inequality and approximate symmetry for the relative spectral error \CF{cite}. To bound the second term, observe that Our choice $\eps_0 = \eps/\log(d_1)$ gives the failure probability $e^{- \Omega(\eps^2 d_1)/\log^2 d_1}$. By setting the constant $C$ in the statement of the theorem large enough compared to $1/\lambda, 1/c$, all the constraints will be satisfied.
\end{proof}
