...
\begin{theorem}\label{thm:hess-pisier} Let $X = (X_1, \dots, X_N)$ be a tuple of standard Gaussians on $(\R^{d_1}\ot \R^{d_2})^n$. Then $\forall t \geq 1$ with failure probability at most $\exp(-\Omega(d_{a} + d_{b}) \log t)$ \TODO{adjust like in the next corollary}, the random quantum channel 
$$T: X \mapsto \frac{1}{nD}\sum_{i = 1}^N X_i Z X_i^\dagger$$
has operator norm 
$O \left( t^{2} \frac{d_{a}+d_{b}}{\sqrt{d_{a} d_{b} nD}} \right)$\TODO{do similar thing to the next corollary} restricted to the traceless symmetric matrices. \end{theorem}
...
$$ \langle Y,  \left( \nabla^2_{ab} f_{\samp} \right) Z \rangle
= O (\lambda) \left( t^{2} \frac{d_{a}+d_{b}}{\sqrt{nD}} \right) \TODO{O(\lambda)}
$$
\TODO{Akshay, please make the rhs $O(\lambda)$ and then adjust the failure probability accordingly}
with probability $1 - \exp( - \Omega \left( (d_{a} + d_{b}) \log \frac{ \lambda \sqrt{n D} }{d_{a} + d_{b}}  \right) ) \geq 1 - \exp( - \lambda \Omega(d_{a} + d_{b}) )$ for $n D \gg \max_{a} d_{a}^{2}$. 
...
\end{corollary}
\CF{akshay defines a notation for restricting operators to the traceless symmetrics. Maybe we should adopt it here also}
\begin{proof}
...
$$\frac{1}{\sum_{i} \|Y_i\|^2} Y_i Y_i^T$$ where $Y_i$ are $n D/{d_a d_b}$ i.i.d standard Gaussians on $\R^{d_a} \ot \R^{d_b}$. By \cref{lem:hess-pisier}, the operator norm of $T$ restricted to $\Sym^0_{d_b}$ is $O((d_a + d_b) \sqrt{ n D/d_ad_b} / \sum_{i \in \|Y_i\|^2} )$ \TODO{propagate from \cref{cor:off-diag-hess}} 
with probability $\exp(-\Omega(d_{a} + d_{b}))$ \TODO{propagate from \cref{cor:off-diag-hess}} . The denominator is a sum of $nD$ many i.i.d $\chi$-square random variables, and so is $\Theta(nD)$ with failure probability $e^{ -\Omega(nD)} \ll \exp(-\Omega(d_{a} + d_{b}))$ \cite{W19}, so finally we have that the operator norm of $T$ restricted to $\Sym^0_{d_b}$ is $O \left( \frac{d_{a}+d_{b}}{\sqrt{d_{a} d_{b} nD}} \right)$ \TODO{propagate from \cref{cor:off-diag-hess}} .
...
As we have already shown in \cref{prop:gradient-bound}, we already have $\norm{\nabla_a f_{\rv}}_{\op} = \|\sqrt{d_a} \rho^{(a)} - I_a/\sqrt{d_a}\|_{op} \leq \eps/\sqrt{d_a}$ with failure probability at most $\sum_{a=1}^k e^{-N_a \eps^2/2C} \ll $ \TODO{propagate from \cref{cor:off-diag-hess}} , and likewise for $b$. Thus we have$$ |(\tr Y \rho^{(a)})\bigl(\tr Z \rho^{(b)}\bigr)| \leq \left( \frac{\eps \|Y\|_{1}}{d_{a}} \right) \left( \frac{\eps \|Z\|_{1}}{d_{b}} \right) \leq \eps^2 \frac{\|Z\|_F\|Y\|_F}{\sqrt{d_a d_b}} $$
