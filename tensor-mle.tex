...
\end{align*}
for $a \in \{1,2\}$ with probability $1 - O(e^{ - \Omega( d_2 \eps^2/\log^2 d_1)})$ \CF{I think the $d_2$ should actually be a $d_1$}
\end{theorem}
...
\begin{corollary}[Akshay corollary]
If $X$ is an $n$-tuple of i.i.d. standard Gaussians on $\R^{d_1}\ot \R^{d_2}$, then for $\tilde{\Phi}$ as in \cref{eq:one-step} we have
...
\CF{UPDATE OLD PROOF TO USE NEW CONCENTRATION BOUND
