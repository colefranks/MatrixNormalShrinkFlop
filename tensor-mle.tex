...
\end{equation}
implies \cref{thm:hess-pisier}. \AR{Removed $\Theta$}
...
\end{align*}
by two applications of Jensen's inequality, as $\norm{\cdot}_{\op}^p$ is the composition of the norm $\norm{\cdot}_{op}$ with $x \to x^{p}$ which is monotone and convex on $\R_{+}$ \MW{(!)}.
...
\MW{Remind me how we use the 2nd half of the preceding sentence?} \AR{I guess it is enough to say the expectation is non-negative always. It was there before because I did the proof with general coefficients $\vec{\alpha}$ in which case we needed to show any non-vanishing term has coefficient $\alpha$ to some even power so the coefficient is also positive. }
...
\leq 1,
\end{align*}
where we first used that $\braket{\xi,A}$ is distributed the same as $\norm\xi_F A_{11}$ by orthogonal invariance \MW{Why is this not a completely general argument?} \AR{This seems like the most general way to give a sub-Gaussian parameter on a Banach space. Maybe there's a more direct version for operator norms, but I don't see it in Pisier}, and then that the trace norm dominates the Frobenius norm.
...
\end{align*}
Now we choose $q \in \mathbb{N}$ such that $C \sqrt{q} \leq \sqrt m + \sqrt n = \E \|A\|_{op}$, where the last equality is by \cref{eq:op norm bound}. \AR{Changed the choice of constant, hope you don't mind}
