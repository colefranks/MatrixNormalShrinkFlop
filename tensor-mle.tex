...
\end{align*}
By definition, the \emph{geodesics} through $P$ are the curves
$t \mapsto \exp_P(t H)$
for $t\in\R$ and $H\in\H$.
They have unit speed if $\sum_{a=1}^k \norm{H_a}_F^2 = \frac1D$.
\MW{A bit sad.}
\MW{Should we write $\norm H_{\H}$ and $\braket{H,K}_{\H}$??}

\MW{Also sad: The geodesic distance squared between $P,Q\in\P$ is given by
\begin{align*}
  \sum_{a=1}^k \frac D{d_a} \norm{\log(P_a^{-1/2} Q_a P_a^{-1/2})}_F^2.
\end{align*}}
\end{definition}
% Indeed,
% \begin{align*}
...
Assuming $f$ is twice differentiable, this holds if $\partial^2_t f(\exp_P(tH)) \geq 0$ for all~$H\in\H$.
...
Assuming the function is twice differentiable, this holds if $\partial^2_t f(\exp_P(tH)) \geq \lambda \sum_{a=1}^k \norm{H_a}_F^2$ for all~$H\in\H$.
\end{definition}
...
\begin{align*}
  \G = \{ (G_1,\dots,G_k) \in \GL(d_1) \times \dots \times \GL(d_k) \;:\: \det G_1 = \dots = \det G_k = 1 \};
\end{align*}
...
Show that the gradient $\nabla f_x(I)$ (defined below) is small with high probability.
...
To make this discussion more concrete, we now define the gradient and Hessian formall, and state the lemma that we will use to relate the gradient and strong convexity to the distance to the optimizer as in the plan above.

\begin{definition}[Gradient and Hessian]
For $P \in \P$, the \emph{(Riemannian) gradient}~$\nabla f(P)$ is the unique element in $\H$ such that \MW{Need to define inner product.}
\begin{align*}
  \braket{\nabla f(P), H} = \partial_{t=0} f(\exp_P(tH))
\end{align*}
for all $Z\in \H$.
We often abbreviate $\nabla f = \nabla f(I,\dots,I)$ and write $\nabla_a f$ for its components, which are $d_a \times d_a$ traceless symmetric matrices.

The \emph{(Riemannian) Hessian}~$\nabla^2 f(P)$ is the unique linear operator on~$\H$ such that
\begin{align*}
  \braket{H, \nabla^2 f(P) K} = \partial_{s=0} \partial_{t=0} f(\exp_{P}(sH + tK))
\end{align*}
for all $H,K \in \H$.
We often abbreviate $\nabla^2 f = \nabla^2 f(I)$.
As a block matrix,
\begin{align*}
  ...
  \end{bmatrix},
\end{align*}
where $\nabla_{ab}^2f$ is a linear operator from the space of traceless symmetric $d_b\times d_b$-matrices to the space of traceless symmetric  on $\R^{d_b}$ to the traceless $d_a \times d_a$-matrices.
% $\smallSym_{d_b}^0$ to $\smallSym_{d_a}^0$.
\end{definition}
...
Similarly, $f$ is $\lambda$-strongly geodesically convex at $P$ if and only if $\nabla^2 f(P) \succeq \lambda I$, i.e., the Hessian is positive definite with eigenvalues larger than or equal to~$\lambda$.

%%% MICHAEL MARKER %55
\MW{Michael continue here.}

\CF{highlight or put in notation}Now say that $B_\kappa(P) = \{ \exp_{P}(Z) : \norm Z \leq \kappa \}$ is the ball of radius $\kappa>0$ about~$P$.
\MW{Not the right radius.}
