...
%Thus $D_F(\htheta_a\Vert \Theta_a) = O( d(\htheta_a, \Theta_a)) = \norm{\frac{H_0}{d_a k} I_{d_a} + \sqrt{d_a} H_a}_F.$
\begin{align*}
...
\end{customthm}
\begin{proof}
\MW{I'll go through this after the above has been sanity-checked.}
...
$$n \geq C (d_2/d_1) \max\{\log (d_2/d_1), \frac{\log^2 d_1}{\eps^2}, \eps^2\} \geq C (d_2/d_1) \max\{\log (d_2/d_1), \eps^{-2}, \eps^2\},$$
...
%\item $\widehat{\Theta}^\alpha_\samp$ is the maximum a posteriori probability (MAP) estimator for $\Theta$ assuming $\Theta$ is drawn from the Wishart prior with $D + 1$ degrees of freedom and $V = I/\alpha$. That is, $\Theta$ is selected from $P$ with density proportional to $e^{- \alpha \tr \Theta}.$
\end{enumerate}
