...
The flip-flop algorithm for this regularized estimator is quite simple: see \cref{alg:reg-flip-flop}.


\begin{Algorithm}
\textbf{Input}: Samples $\samp = (\samp_1, \ldots, \samp_n)$ where $\samp_i \in \R^D$ is sampled from a (unknown) centered normal distribution $\cN(0, \Sigma)$.  \\[.3ex]
...
\textbf{Output}: Regularized estimators $\otheta \in \SPD$.
% such that $\| \nabla \ell^\alpha_x (\otheta)\|_F < \eps$. \\[.3ex]

\textbf{Algorithm}:
\begin{enumerate}
\item\label{it:flip-flop step 1} Set $\otheta_a = I_a$ for each $a \in [k]$.
\item\label{it:flip-flop step 2} While $\CF{do}$, 
%we need T = \log(1/\epsilon) \cdot 1/\alpha \lambda
\begin{itemize}
%\item Set $\otheta^1$
\item Define $\otheta^a:= \otheta_1 \ot \dots \ot \otheta_{a-1} \ot I_{d_a} \ot \otheta_{a+1} \ot \dots \ot \otheta_{d}$, the tensor product in which the $i^{th}$ factor is $\otheta_i$ except for the $a^{th}$ which is $I_{d_a}$.  Let 
...
\end{itemize}
\end{enumerate}
\caption{Tensor flip-flop algorithm}\label{alg:reg-flip-flop}
\end{Algorithm}
