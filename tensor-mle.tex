...
$$\frac{1}{\sum_{i} \|Y_i\|^2} Y_i Y_i^T$$ where $Y_i$ are $n D/{d_a d_b}$ i.i.d standard Gaussians on $\R^{d_a} \ot \R^{d_b}$. By \cref{cor:hess-pisier}, the operator norm of $T$ restricted to $\Sym^0_{d_b}$ is $O((d_a + d_b) \sqrt{ n D/d_ad_b} / \sum_{i \in \|Y_i\|^2} )$ \TODO{propagate from \cref{cor:off-diag-hess}} 
