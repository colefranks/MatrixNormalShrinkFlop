...
\item I'm going to call $\SL = \oplus \SL_{d_i}, \mathcal{P}= \PD^1 = \oplus \PD_{d_i}^1, \Sym = \oplus \Sym_{d_i}^0$. Explain somewhere how $\Sym$ is the tangent space of $\PD$. \MW{Suppressing the $^1$ and $^0$ is a bit confusing I think. Maybe $\operatorname{SPD}$ for $\PD$ with $\det=1$? I still feel that $\Sym$ looks somewhat horrible (with or without subscript, but I am not sure what would be better).}
...
\end{align*}
where $\Theta$ denotes the precision matrix $\Sigma^{-1}$. \CF{move some flip-flop here}
...
The \emph{geodesics} through $\Theta$ are the curves $t\mapsto \exp_\Theta(tZ)$ for $t \in \R$. These curves are in fact geodesics, or ``straight lines,'' through $\Theta$ for a natural Riemannian metric on~$\PD$~\cite{bhatia2009positive}. \CF{fisher-rao}
...
It is more convenient to maximize a different function than the log-likelihood $\ell_{\samp}$. We may instead maximize the scale-invariant function
