...
\item \CF{\emph{balanced-ness}.}
\end{enumerate}
\end{definition}

The rationale for the above definition is that when $(I_{d_2}/\sqrt{d_2}, I_{d_1}/\sqrt{d_1})$ is in fact a singular pair for $\Phi$, as will approximately be the case in our setting, we have $\sigma_1(\Phi) =  \tr \Phi(I_{d_2})/\sqrt{d_1 d_2}.$ Thus $(1-\lambda)$ is like a spectral gap.
...
If $\Phi$ is an $\eps$-balanced, $(1 - \lambda)$-quantum expander, and $\eps \leq c \lambda^2/\log p$, then there is a scaling solution $L, R$ such that $\|L - I\|_{op} \leq \eps \log p/\lambda$. 
\end{theorem}
...
\begin{theorem}[\CF{check}]\label{thm:operator-cheeger}
...
\CF{todo; not hard, literally stringing the two together.}
...
\end{enumerate}
These three items would complete the proof of the lemma, because every pair of projections is at most $\delta$ from an element of $N$ and if every element of $N$ has $\vol(\Pi_1, \Pi_2) \geq \tr \Phi(I)/{d_1 d_2}$, then by \cref{lem:net-suffices} the two sufficient conditions of \cref{lem:suff} hold with $\delta =  \delta' d_1 d_2/4 \leq c$. \CF{continue here; fix this and make it more transparent.} 
...
To prove \cref{thm:operator-cheeger}, apply \cref{lem:op-cheeger} using \cref{cor:vershynin} to bound $\eps$ and \cref{lem:union} to bound $\ch(\Phi)$. \CF{actually write}.
\end{proof}
...
\exp \left( - \frac{\Omega(n D d_{a} d_{b} t^{2})}{\|y\|_{2}^{2}} & \forall t \lesssim \frac{\|y\|_{2}^{2}}{d_{a} d_{b} \|y\|_{\infty}}   \right)
