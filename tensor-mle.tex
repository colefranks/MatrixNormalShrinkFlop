...
$$n \geq C (d_2/d_1) \max\{\log (d_2/d_1),  \eps_0^{-2}\} $$
 with probability $1 - O(e^{ - \Omega( d_1 \eps_0^2)})$ the operator $\Phi$ is a $(\eps_0  \sqrt{{d_2}/{n d_1}},1-\lambda)$-quantum expander. By \cref{cor:flipflop-concentration}, $\tilde{\Phi}$ is $\eps_0 \sqrt{{d_1}/{n d_2}}$-balanced with probability $1 - O(e^{ - \Omega( d_1 \eps_0^2)});$ by the union bound both events occur with probability $1 - O(e^{ - \Omega( d_1 \eps_0^2)});$ condition on both events.
