...
Let $\underline{\nabla}^2 f$ denote the gradient in the Hilbert-Schmidt inner product, or $\underline{\nabla}^2_{ab} f = \frac{1}{d_a} \nabla_{ab}^2 f$. By \cref{eq:expansion-thing}, with probability $\CF{good}$, the map $Z \to \tr_2 \rho^{(a,b)} (I \ot Z)$ has operator norm at most $\lambda/\sqrt{d_a d_b}$ restricted to traceless $Z$ and $\|\rho^{(a)} - I_{d_a}/{d_a}\|_{op} \leq \eps/d_a, \|\rho^{(b)} - I_{d_b}/d_b\|_{op} \leq \eps/d_b$. Thus for traceless $Z$ we have
$$ \rho^{(a)}\bigl(\tr Z \rho^{(b)}\bigr) \leq \| \rho^{(a)}\|_F \eps /\sqrt{d_a} \leq (1 + \eps) \eps /\sqrt{d_a d_b}.$$
Together, we have 
$\|\underline{\nabla}_{ab}^2 f \|_{op} \leq (\lambda + \eps(1 + \eps))/\sqrt{d_a d_b}.$ We assume $\eps \leq c$, so we may apply the inequality with $A = (\lambda + 2\eps) I_{d_a}/d_{a}$, $B = (\lambda + 2\eps)I_{d_b}/d_{b}$. For $a = 1, b = 2$, we have \begin{align*}
  \underline{\nabla}^2 f \succeq \begin{bmatrix}
  \underline{\nabla}_{11}^2 f - \frac{\lambda + 2\eps}{d_a} I_{d_a} & 0 & \hdots &  \underline{\nabla}_{1k}^2 f  \\
0 & \underline{\nabla}_{22}^2 f - \frac{\lambda + 2\eps}{d_b} I_{d_b} & \ddots & \vdots \\
