...
\begin{lemma}[Riemmanian Hessian]\label{lem:hessian} Let $\rho = \sum_{i=1}^n \samp_i \samp_i^T$. Then the Riemannian Hessian $\nabla^2 \ell_{\samp}$ at the identity is given by 
\begin{align*}
 \langle Y,  \left( \nabla^2_{aa} \ef_{\samp} \right) Y \rangle
&=  \frac{d_a}{nD} \tr \rho^{(a)} Y^2 \\
  \langle Y,  \left( \nabla^2_{ab} \ef_{\samp} \right) Z \rangle
&= \frac{\sqrt{d_a d_b}}{nD} \tr \rho^{(ab)} \left( Y \ot Z \right)
\end{align*}
for all $a\neq b\in[k]$ and traceless symmetric $d_a\times d_a$ matrices $Y$, $d_b\times d_b$ matrices~$Z$, and
\begin{align*}
\nabla^2_{00} \ell_\samp &= \frac{1}{nD}\tr \rho =\frac{1}{nD} \|X\|_F^2 \\
\nabla^2_{a0} \ell_\samp &= \frac{\sqrt{d_a}}{nD}\rho_a
\end{align*}
for all $a \in [k]$. 
\end{lemma}
\begin{proof}
Note that for $a,b \in \{0,\dots, k\}$, $\nabla^2_{ab} f_\samp = \nabla^2_{ab} F$ where $F:\Theta \mapsto \frac{1}{nD}\tr \rho \ot_{a = 1}^k \Theta_a$. 
  ...
  &= \frac{1}{nD}\partial_{s=0} \partial_{t=0} \tr \rho \, e^{(s+t)\sqrt{d_a}  Y_{(a)}} \\
  &= \frac{1}{nD}\partial_{s=0} \partial_{t=0} \tr \rho^{(a)} e^{(s+t)  \sqrt{d_a}Y}
  = \frac{d_a}{nD} \tr \rho^{(a)} Y^2,
  \end{align*}
  while for $a\neq b \in [k]$ we have
  \begin{align*}
    \langle Y,  \left( \nabla^2_{ab} F_{\samp} \right) Z \rangle
  &= \frac{1}{nD}\partial_{s=0} \partial_{t=0} \tr \rho \, e^{s \sqrt{d_a}Y_{(a)} + t \sqrt{d_b} Z_{(b)}} \\
  &= \frac{1}{nD}\partial_{s=0} \partial_{t=0} \tr \rho^{(ab)} \left( e^{s \sqrt{d_a}  Y} \ot e^{t \sqrt{d_b} Z} \right)\\
  &= \frac{\sqrt{d_a d_b}}{nD} \tr \rho^{(ab)} \left( Y \ot Z \right).
  \end{align*}
 Also $\nabla^2_{00} F_{\samp} = \partial^2_{t=0} \tr \rho  e^{t} = \frac{1}{nD}\tr \rho$, and for all $a \in [k],$ 
 \begin{align*}
\langle Z, \nabla^2_{a0} F_{\samp} \lambda \rangle 
  &= \frac{1}{nD} \partial_{s=0} \partial_{t=0} \tr \rho \, e^{s\sqrt{d_a} Z_{(a)}}e^{t\lambda} \\
  &= \frac{\sqrt{d_a}}{nD} \lambda \tr \rho_a Z.
  \end{align*}
