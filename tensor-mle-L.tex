...
\begin{align*}
  ...
  & = e^{\frac{1}{\sqrt{D}} h t} \left( P_1^{1/2} e^{\sqrt{\frac{d_1}{D}} H_1} P_1^{1/2}, \dots, P_k^{1/2} e^{\sqrt{\frac{d_k}{D}} H_k} P_k^{1/2} \right).
\end{align*}
...
For notational convenience we minimize the function $\ef_\samp = - 2 \ell_x$. We write it explicitly below.
...
=  \samp_i^T \left( \textstyle \bigotimes_{a=1}^k P_a \right) \samp_i - \sum_{a = 1}^k \frac{D}{d_a} \log \det(P_a).
\end{align*}
...
\begin{align*}
 \nabla_a \ef_{\samp} = \sqrt{\frac{d_a}{D}}\left(  \rho^{(a)} - \tr(\rho^{(a)})\frac{ I_a}{d_a}\right)
  \qquad \text{ for } a \in [k],
\end{align*}
and $\nabla_0 \ef_\samp = \tr \rho = \|X\|_F^2.$ 
\end{lemma}
\begin{proof}
First note that $\nabla_a \log \det \Theta_a$ is zero because each component $H_a$ of the tangent vector is traceless for $a \in [k]$. Thus for all $a\in[k]$, we have
  \begin{align*}
  ...
  = \sqrt{\frac{d_a}{D}} \tr \rho Z_a
  = \sqrt{\frac{d_a}{D}} \tr \rho^{(a)} Z
  \end{align*}
  for all traceless symmetric $d_a \times d_a$ matrices $Z$.
  Since $\nabla_a f_{\samp}$ is traceless and symmetric by definition, this implies
  \begin{align*}
    \nabla_a f_{\samp}
  = \sqrt{\frac{d_a}{D}}\left(  \rho^{(a)} - \tr(\rho^{(a)})\frac{ I_a}{d_a}\right)
 % = \sqrt{d_a} \rho^{(a)} - I_a/\sqrt{d_a}.
  \end{align*}
 Finally $\nabla_0 \ell_\samp = \partial_t \tr e^{t} \rho - \log (e^{t}) = \tr \rho.$
\end{proof}
...
\begin{prop}\label{prop:gradient-bound}\CF{won't change; update proof.}
...
&=  \frac{d_a}{D} \tr \rho^{(a)} Y^2 \\
  \langle Y,  \left( \nabla^2_{ab} \ef_{\samp} \right) Z \rangle
&= \frac{\sqrt{d_a d_b}}{D} \tr \rho^{(ab)} \left( Y \ot Z \right)
\end{align*}
for all $a\neq b\in[k]$ and traceless symmetric $d_a\times d_a$ matrices $Y$, $d_b\times d_b$ matrices~$Z$, and
\begin{align*}
\nabla^2_{00} \ell_\samp &= \tr \rho = \|X\|_F^2 \\
\nabla^2_{a0} \ell_\samp &= \rho_a
\end{align*}
for all $a \in [k]$. 
\end{lemma}
\begin{proof}
  Let $F_{\samp}(\Theta) = \tr \rho \, \Theta$ as in the proof of \cref{lem:gradient}, and note that the Hessian of $F_\samp$ and $\ell_\samp$ are the same.
  Then for $a \in [k]$, and $Y, Z$ Hermitian we have
  \begin{align*}
   \langle Y,  \left( \nabla^2_{aa} F_{\samp} \right) Y \rangle
  &= \partial_{s=0} \partial_{t=0} \tr \rho \, e^{(s+t)\sqrt{\frac{d_a}{D}}  Y_{(a)}} \\
  &= \partial_{s=0} \partial_{t=0} \tr \rho^{(a)} e^{(s+t)  \sqrt{\frac{d_a}{D}}Y}
  = d_a \tr \rho^{(a)} Y^2,
  \end{align*}
  while for $a\neq b \in [k]$ we have
  \begin{align*}
