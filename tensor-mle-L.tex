...
However, it is never strongly convex. This directly related to the non-identifiability of the $\Theta_a$, which makes the MLE not unique. 
To this end, we parameterize $\Theta_a = P_0^{1/k} P_a$ for a scalar $P_0>0$ and positive definite~$P_a$ of unit determinant, so that $\Theta_1 \ot \cdots \ot \Theta_k = P_0 \cdot P_1 \ot \cdots \ot P_k$ and all parameters are identifiable.
In this parameterization, the log-likelihood function reads
\begin{align}\label{eq:reparameterized log likelihood}
  \ell_x(P_0, P_1,\dots,P_k)
...
&= \frac {nD} 2 \log P_0  - \frac{P_0}2 \sum_{i=1}^n x_i^T \left( \textstyle\bigotimes_{a=1}^k P_a \right) x_i.
\end{align}
%Clearly, the difficulty in finding the MLE is in minimizing the second term over $P_1,\dots,P_k$. 
%Once this is achieved, the correct normalization can be determined by a simple one-dimensional optimization that has a closed form solution. 
%However, 
%Once this is achieved, the correct normalization can be determined by a simple one-dimensional optimization that has a closed form solution (see below).
...
% As above, we can fix this by working with tuples of precision matrices with equal determinant:
\begin{align*}
  \P &= \{ (P_0, P_1,\dots,P_k) \in \R\times \PD(d_1) \times \dots \times \PD(d_k) \;:\; \det P_a = 1 \; \forall a \in [k] \}.
 \end{align*}
%\intertext{The tangent space of this manifold is naturally identified with the real vector space}
The tangent space of this manifold is naturally identified with the real vector space
\begin{align*}
  \H &= \{ (H_0, H_1,\dots,H_k) \;:\; H_0 \in \R \text{ and }H_a \text{ a symmetric $d_a \times d_a$ matrix}, \; \tr H_a = 0\; \forall a \in [k]  \}.
\end{align*}
%$H_0$ is the tangent direction that changes the determinant, and the symmetric traceless matrices are the directions that change in the matrices while keeping the determinant fixed.
The embedding $(P_0, P_1,\dots,P_d) \mapsto P_0 \cdot P_1 \ot \dots \ot P_d$ identifies $\P$ with the set of Kronecker products $\Theta_1 \ot \dots \ot \Theta_k$ in $\PD(D)$, a totally geodesic submanifold. This induces a Riemannian metric with the following geodesics.

\begin{definition}[Geodesics and balls]
Let $P\in\P$.
The \emph{exponential map} $\exp_\Theta \colon \H \to \P$ at~$P$ is defined by
\begin{align*}
  \exp_\Theta(H) &= \exp_{(P_0, P_1,\dots,P_k)}(H_0, H_1,\dots,H_k)\\
  & =  \left(P_0 e^{H_0 t},  P_1^{1/2} e^{\sqrt{d_1} H_1} P_1^{1/2}, \dots, P_k^{1/2} e^{\sqrt{d_k} H_k} P_k^{1/2} \right).
\end{align*}
...
A twice differentiable function $f\colon \P \to \R$ is said to be \emph{geodesically convex} at $\Theta\in\P$ if $\partial^2_{t=0} f(\exp_P(tH)) \geq 0$ for all~$H\in\H$.
...
\begin{align*}
  \G = \{ (G_0, G_1,\dots,G_k) \in \R_x\times \SL(d_1) \times \dots \times \SL(d_k) \}
\end{align*}
where $\SL(d)$ denotes the matrices of determinant $1$. Namely, if $P \in \P$ and $A \in \G$ then the tuple $A P A^T := (|G_0|^2 P_0,  G_1 P_1 G_1^T, \dots, G_k P_k G_k^T)$ is in $\P$. \CF{use either $A$ or $G$ but not both?}
...
\begin{align*}
  \ef_\samp(P)
=  \frac{P_0}{nD}\samp_i^T \left( \textstyle \bigotimes_{a=1}^k P_a \right) \samp_i - \log P_0
\end{align*}
Let $\widehat P = (\widehat P_0, \widehat P_1,\dots,\widehat P_k)$ denote the minimizer of $f_x$ if it exists and is unique.
...
Show that the gradient $\nabla f_x(1, I_{d_1},\dots,I_{d_k})$ (defined below) is small with high probability.
...
\begin{align*}
  ...
  \end{bmatrix},
\end{align*}
where the components $\nabla_{ab}^2f(P)$ of the Hessians are linear operators from the space of traceless symmetric $d_b\times d_b$-matrices to the space of traceless symmetric $d_a \times d_a$-matrices for $a, b \in [k]$, $\nabla^2_{00} f(P)$ is a real number, and $\nabla_{0a}f$ is a linear operator from $\R$ to the space of traceless symmetric $d_1\times d_a$ matrices. Thus $\nabla_{0a}f$ itself can be viewed as a matrix.
\end{definition}
...
\begin{prop}\label{prop:gradient-bound}\CF{won't change with good normalization; update proof.}
...
\begin{align*}
\nabla^2_{00} \ell_\samp &=\tr \rho =\frac{1}{nD} \|X\|_F^2 \\
\nabla^2_{a0} \ell_\samp &= \sqrt{d_a}\rho_a
\end{align*}
for all $a \in [k]$. 
\end{lemma}
\begin{proof}
Note that for $a,b \in \{0,\dots, k\}$, $\nabla^2_{ab} f_\samp = \nabla^2_{ab} F$ where $F:\Theta \mapsto \frac{1}{nD}\tr \rho \ot_{a = 1}^k \Theta_a$. 
 ...
 \begin{align*}
\langle Z, \nabla^2_{a0} F_{\samp} \lambda \rangle 
  &=  \partial_{s=0} \partial_{t=0} \tr \rho \, e^{s\sqrt{d_a} Z_{(a)}}e^{t\lambda} \\
  &= \sqrt{d_a} \lambda \tr \rho_a Z.
  \end{align*}
