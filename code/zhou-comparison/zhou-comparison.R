...
for (j in 1:length(regus)){
  ...
  print(c("trial",i))
    ...
    D = regsinkhorn(X,tol = .1*regu, reg = regu/1000)[[1]]
    D = n*D/tr(D)
    toc()
    
    tic("sinkhorn")
    G = sinkhorn(X,tol = .1*regu)[[1]]
    toc()
    
    tic("glassosinkhorn")
    H = glassosinkhorn(X,tol = .1*regu, reg = regu/1000, glassoreg = rowpen*regu/5)[[1]]
    toc()
    ...
    rmse3 = rmse3 + norm(G-E, "F")^2/norm
    rmse4 = rmse4 + norm(H-E, "F")^2/norm
    
  }

  errors[,j]=c(rmse1/ntrials,
               rmse2/ntrials,
               rmse3/ntrials,
               rmse4/ntrials
               )
